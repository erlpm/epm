-define(ABORT, epm_utils:abort()).
-define(ABORT(Str, Args), epm_utils:abort(Str, Args)).

-define(CONSOLE(Str, Args), io:format(Str++"~n", Args)).

-define(DIAGNOSTIC(Str, Args), epm_log:log(diagnostic, Str, Args)).
-define(DEBUG(Str, Args), epm_log:log(debug, Str, Args)).
-define(INFO(Str, Args), epm_log:log(info, Str, Args)).
-define(WARN(Str, Args), epm_log:log(warn, Str, Args)).
-define(ERROR(Str, Args), epm_log:log(error, Str, Args)).
-define(CRASHDUMP(Str, Args), epm_log:crashdump(Str, Args)).
...
-define(DEFAULT_CDN, "https://erl.pm/repo").
...
-record(resource, {type :: atom(),
                   module :: module(),
                   state :: term(),
                   implementation :: epm_resource | epm_resource_v2}).

-type epm_dict() :: dict:dict().
-type epm_digraph() :: digraph:graph().
-type epm_set() :: sets:set().
