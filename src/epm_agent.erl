%%% @doc Runs a process that holds a epm state and can be used
...
%% @doc boots an agent server; requires a full epm state already.
%% By default (within epm), this isn't called; `epm_prv_shell'
%% enters and transforms into this module
-spec start_link(epm_state:t()) -> {ok, pid()}.
start_link(State) ->
...
-spec run(atom(), atom(), string(), epm_state:t(), file:filename()) ->
    {ok, epm_state:t()} | {{error, term()}, epm_state:t()}.
run(Namespace, Command, StrArgs, RState, Cwd) ->
    try
        case epm_dir:get_cwd() of
            Cwd ->
                PArgs = getopt:tokenize(StrArgs),
                Args = [atom_to_list(Namespace), atom_to_list(Command)] ++ PArgs,
                CmdState0 = refresh_state(RState, Cwd),
                CmdState1 = epm_state:set(CmdState0, task, atom_to_list(Command)),
                CmdState = epm_state:set(CmdState1, caller, api),
                case epm:run(CmdState, Args) of
                    {ok, TmpState} ->
...
%% @private based on a epm state term, reload paths in a way
%% that makes sense.
-spec refresh_paths(epm_state:t()) -> ok.
refresh_paths(RState) ->
    RefreshPaths = application:get_env(epm, refresh_paths, [all_deps, test]),
    ToRefresh = parse_refresh_paths(RefreshPaths, RState, []),
    %% Modules from apps we can't reload without breaking functionality
    ShellOpts = epm_state:get(RState, shell, []),
    ShellBlacklist = proplists:get_value(app_reload_blacklist, ShellOpts, []),
    Blacklist = lists:usort(
        application:get_env(epm, refresh_paths_blacklist, ShellBlacklist)
        ++ [epm, erlware_commons, providers, cf, cth_readable]),
    %% Similar to epm_utils:update_code/1, but also forces a reload
...
%% run by no processes get unloaded by epm_paths, without being loaded back in.
refresh_path_blacklisted(Path) ->
...
parse_refresh_paths([all_deps | RefreshPaths], RState, Acc) ->
    Paths = epm_state:code_paths(RState, all_deps),
    parse_refresh_paths(RefreshPaths, RState, Paths ++ Acc);
parse_refresh_paths([project_apps | RefreshPaths], RState, Acc) ->
    Paths = [filename:join([epm_app_info:out_dir(App), "ebin"])
        || App <- epm_state:project_apps(RState)],
    parse_refresh_paths(RefreshPaths, RState, Paths ++ Acc);
parse_refresh_paths([test | RefreshPaths], RState, Acc) ->
    Paths = [filename:join([epm_app_info:out_dir(App), "test"])
        || App <- epm_state:project_apps(RState)],
    parse_refresh_paths(RefreshPaths, RState, Paths ++ Acc);
...
-spec refresh_state(epm_state:t(), file:filename()) -> epm_state:t().
refresh_state(RState, _Dir) ->
    lists:foldl(
        fun(F, State) -> F(State) end,
        epm:init_config(),
        [fun(S) -> epm_state:apply_profiles(S, epm_state:current_profiles(RState)) end]
    ).
