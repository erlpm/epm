%%% @doc Packages epm.hrl features and macros into a more generic API
...
-export_type([epm_dict/0, epm_digraph/0]).
...
expand_env_variable(InStr, VarName, RawVarValue) ->
    epm_utils:expand_env_variable(InStr, VarName, RawVarValue).
...
-spec set_paths(epm_paths:targets(), epm_state:t()) -> ok.
set_paths(List, State) ->
    epm_paths:set_paths(List, State).

%% @doc Unsets code paths. Takes arguments of the form
%% `[plugins, deps]' or `[deps, plugins]' and ensures the
%% paths are no longer active.
-spec unset_paths(epm_paths:targets(), epm_state:t()) -> ok.
unset_paths(List, State) ->
    epm_paths:unset_paths(List, State).

%% @doc Add deps to the code path
-spec add_deps_to_path(epm_state:t()) -> ok.
...
%% plugins in the path
-spec restore_code_path(epm_state:t()) -> true | {error, term()}.
restore_code_path(State) ->
  epm_utils:cleanup_code_path(epm_state:code_paths(State, default)).
...
processing_base_dir(State) ->
    epm_dir:processing_base_dir(State).
