...
-type project_type() :: epm | mix | undefined.
...
-record(app_info_t, {name               :: binary() | undefined,
                     ...
                     default=dict:new() :: epm_dict(),
                     opts=dict:new()    :: epm_dict(),
                     dep_level=0        :: integer(),
                     fetch_dir          :: file:name(),
                     dir                :: file:name(),
                     out_dir            :: file:name(),
...
-spec new(atom() | binary() | string()) ->
                 {ok, t()}.
new(AppName) ->
    {ok, #app_info_t{name= epm_utils:to_binary(AppName)}}.
...
new(AppName, Vsn, Dir) ->
    {ok, #app_info_t{name= epm_utils:to_binary(AppName),
                     vsn=Vsn,
                     original_vsn=Vsn,
                     fetch_dir= epm_utils:to_list(Dir),
                     dir= epm_utils:to_list(Dir),
                     out_dir= epm_utils:to_list(Dir),
                     ebin_dir=filename:join(epm_utils:to_list(Dir), "ebin")}}.
...
%% file for the app
-spec update_opts(t(), epm_dict(), [any()]) -> t().
...
deps_from_config(Dir, ConfigDeps) ->
    case epm_config:consult_lock_file(filename:join(Dir, ?LOCK_FILE)) of
        [] ->
...
-spec discover(file:filename_all(), epm_state:t()) -> {ok, t()} | not_found.
discover(Dir, State) ->
    case epm_app_discover:find_app(Dir, all, State) of
        {true, AppInfo} ->
            {ok, AppInfo};
        false ->
            not_found
...
app_file_src(#app_info_t{app_file_src=undefined, dir=Dir, name=Name, opts=Opts}) ->
    CandidatePaths = [filename:join([epm_utils:to_list(Dir), Src, epm_utils:to_list(Name)++".app.src"])
                      || Src <- epm_opts:get(Opts, src_dirs, ["src"])],
...
app_file_src(AppInfo=#app_info_t{}, AppFileSrc) ->
    AppInfo#app_info_t{app_file_src= epm_utils:to_list(AppFileSrc)}.
...
app_file_src_script(#app_info_t{app_file_src_script=undefined, dir=Dir, name=Name}) ->
    AppFileSrcScript = filename:join([epm_utils:to_list(Dir), "src", epm_utils:to_list(Name)++".app.src.script"]),
...
app_file_src_script(#app_info_t{app_file_src_script=AppFileSrcScript}) ->
    epm_utils:to_list(AppFileSrcScript).
...
app_file_src_script(AppInfo=#app_info_t{}, AppFileSrcScript) ->
    AppInfo#app_info_t{app_file_src_script= epm_utils:to_list(AppFileSrcScript)}.
...
app_file(#app_info_t{app_file=undefined, out_dir=Dir, name=Name}) ->
    AppFile = filename:join([epm_utils:to_list(Dir), "ebin", epm_utils:to_list(Name)++".app"]),
...
app_details(AppInfo=#app_info_t{app_details=[]}) ->
    case app_file(AppInfo) of
        undefined ->
            try epm_config:consult_app_file(app_file_src(AppInfo)) of
                [] -> [];
        ...
        AppFile ->
            try epm_file_utils:try_consult(AppFile) of
                [] -> [];
                [{application, _Name, AppDetails}] -> AppDetails
            catch
                throw:{error, {Module, Reason}} ->
                    ...
                    case epm_config:consult_app_file(app_file_src(AppInfo)) of
                        [] -> [];
                        [{application, _Name, AppDetails}] -> AppDetails
                    end
            end
...
dir(AppInfo=#app_info_t{out_dir=undefined}, Dir) ->
    AppInfo#app_info_t{dir= epm_utils:to_list(Dir),
                       out_dir= epm_utils:to_list(Dir)};
...
out_dir(AppInfo=#app_info_t{}, OutDir) ->
    AppInfo#app_info_t{out_dir= epm_utils:to_list(OutDir),
                       ebin_dir=filename:join(epm_utils:to_list(OutDir), "ebin")}.
...
ebin_dir(#app_info_t{ebin_dir=undefined,
                     out_dir=OutDir}) ->
    filename:join(epm_utils:to_list(OutDir), "ebin");
...
priv_dir(#app_info_t{out_dir=OutDir}) ->
    epm_utils:to_list(filename:join(OutDir, "priv")).
...
valid(AppInfo=#app_info_t{valid=undefined}) ->
    case epm_app_utils:validate_application_info(AppInfo) =:= true
...
%% epm will do the detection of the status itself.
...
verify_otp_vsn(AppInfo) ->
    epm_utils:check_min_otp_version(epm_app_info:get(AppInfo, minimum_otp_vsn, undefined)),
    epm_utils:check_blacklisted_otp_versions(epm_app_info:get(AppInfo, blacklisted_otp_vsns, [])).
...
has_all_artifacts(AppInfo) ->
    Artifacts = epm_app_info:get(AppInfo, artifacts, []),
    OutDir = out_dir(AppInfo),
    Context = [{base_dir, epm_app_info:get(AppInfo, base_dir, ?DEFAULT_BASE_DIR)}
              ,{profile_dir, epm_dir:profile_dir(opts(AppInfo), profiles(AppInfo))}
              ,{out_dir, OutDir}],
...
apply_overrides(Overrides, AppInfo) ->
    Name = binary_to_atom(epm_app_info:name(AppInfo), utf8),
    Opts = epm_opts:apply_overrides(opts(AppInfo), Name, Overrides),
...
add_to_profile(AppInfo, Profile, KVs) when is_atom(Profile), is_list(KVs) ->
    Opts = epm_opts:add_to_profile(opts(AppInfo), Profile, KVs),
    AppInfo#app_info_t{opts=Opts}.
...
apply_profiles(AppInfo=#app_info_t{default = Defaults, profiles=CurrentProfiles}, Profiles) ->
    AppliedProfiles = case Profiles of
                          %% Head of list global profile is special, only for use by epm
                          %% It does not clash if a user does `epm as global...` but when
    ...
    ConfigProfiles = epm_app_info:get(AppInfo, profiles, []),
                       ...
                       (Profile, OptsAcc) ->
                            case proplists:get_value(Profile, ConfigProfiles, []) of
                                OptsList when is_list(OptsList) ->
                                    ...
                                    epm_opts:merge_opts(Profile, ProfileOpts, OptsAcc);
                                Other ->
