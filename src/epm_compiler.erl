...
-callback context(epm_app_info:t()) -> #{src_dirs     => [DirName :: file:filename()], % mandatory
...
-callback compile(file:filename(), out_mappings(), epm_dict(), list()) ->
    ok | {ok, [string()]} | error | {error, [string()], [string()]} | skipped.
-callback compile_and_track(file:filename(), out_mappings(), epm_dict(), list()) ->
    {ok, [{file:filename(), file:filename(), term()}]} |
    {ok, [{file:filename(), file:filename(), term()}], [string()]} |
    {error, [string()], [string()]} | error.
-callback clean([file:filename()], epm_app_info:t()) -> _.
...
analyze_all({Compiler, G}, Apps) ->
    ...
    epm_compiler_dag:prune(
        G, SrcExt, OutExt, lists:append(AbsSources), lists:append(AppOutPaths)
    ),
    epm_compiler_dag:populate_deps(G, SrcExt, OutExt),
    epm_compiler_dag:propagate_stamps(G),

    [$a, Sep, $b] = filename:join("a", "b"),
    AppPaths = [{epm_app_info:name(AppInfo),
                 epm_utils:to_list(epm_app_info:dir(AppInfo)) ++ [Sep]}
                || AppInfo <- Apps],
    AppNames = epm_compiler_dag:compile_order(G, AppPaths, SrcExt, OutExt),
...
-spec compile_analyzed({module(), digraph:graph()}, epm_app_info:t(), map()) -> ok.
compile_analyzed({Compiler, G}, AppInfo, Contexts) -> % > 3.13.2
    run(G, Compiler, AppInfo, Contexts),
    ok.

-spec compile_all([module(), ...], epm_app_info:t()) -> ok.
...
%% epm_base_compiler. This can't be done because of existing plugins.
...
error_tuple(Source, Es, Ws, Opts) ->
    epm_base_compiler:error_tuple(Source, Es, Ws, Opts).

maybe_report(Reportable) ->
    epm_base_compiler:maybe_report(Reportable).

format_error_source(Path, Opts) ->
    epm_base_compiler:format_error_source(Path, Opts).

report(Messages) ->
    epm_base_compiler:report(Messages).
...
gather_contexts(Compiler, Apps) ->
    Default = default_ctx(),
    Contexts = [{epm_app_info:name(AppInfo),
...
gather_in_dirs([{AppInfo, Ctx} | Rest], Acc) ->
    ...
    BaseDir = epm_utils:to_list(epm_app_info:dir(AppInfo)),
...
analyze_app({Compiler, G}, Contexts, AppInfo) ->
    AppName = epm_app_info:name(AppInfo),
    BaseDir = epm_utils:to_list(epm_app_info:dir(AppInfo)),
    OutDir = epm_utils:to_list(epm_app_info:out_dir(AppInfo)),
    BaseOpts = epm_app_info:opts(AppInfo),
    #{src_dirs := SrcDirs,
      src_ext := SrcExt,
    ...
    %% Run the analysis
    epm_compiler_dag:populate_sources(
...
sort_apps(Names, Apps) ->
    NamedApps = [{epm_app_info:name(App), App} || App <- Apps],
    [App || Name <- Names,
            {_, App} <- [lists:keyfind(Name, 1, NamedApps)]].

prepare_compiler_env(Compiler, Apps) ->
    EpmLibs = [epm_utils:to_binary(Atom)
                 || {ok, EpmApps} <- [application:get_key(epm, applications)],
                    Atom <- EpmApps],
    lists:foreach(
        fun(AppInfo) ->
            EbinDir = epm_utils:to_list(epm_app_info:ebin_dir(AppInfo)),
            %% Make sure that outdir is on the path
            ok = epm_file_utils:ensure_dir(EbinDir),
            ...
            %% use code:add_patha to go above epm own dependencies
            %% when they clash to avoid overtaking the project's
            %% path for includes and priv/
            case lists:member(epm_app_info:name(AppInfo), EpmLibs) of
...
do_compile(CompilerMod, Source, Outs, Config, Opts) ->
    case CompilerMod:compile(Source, Outs, Config, Opts) of
        ok ->
            ?DEBUG("~tsCompiled ~ts", [epm_utils:indent(1), filename:basename(Source)]);
        {ok, Warnings} ->
            report(Warnings),
            ?DEBUG("~tsCompiled ~ts", [epm_utils:indent(1), filename:basename(Source)]);
        skipped ->
            ?DEBUG("~tsSkipped ~ts", [epm_utils:indent(1), filename:basename(Source)]);
        Error ->
...
store_artifacts(G, [{Source, Target, Meta}|Rest]) ->
    ...
    epm_compiler_dag:store_artifact(G, Source, Target, Meta),
...
compile_handler({ok, Source}, _Args) ->
    ?DEBUG("~sCompiled ~s", [epm_utils:indent(1), filename:basename(Source)]),
    ok;
...
compile_handler({skipped, Source}, _Args) ->
    ?DEBUG("~sSkipped ~s", [epm_utils:indent(1), filename:basename(Source)]),
    ok;
...
clean_(CompilerMod, AppInfo, _Label) ->
    ...
    BaseDir = epm_app_info:dir(AppInfo),
    Opts = epm_app_info:opts(AppInfo),
...
annotate_extras(AppInfo) ->
    ...
    ExtraDirs = epm_dir:extra_src_dirs(AppOpts, []),
    OldSrcDirs = epm_dir:src_dirs(AppOpts, ["src"]),
    ...
    %% profiles. All of this must be done with the epm_dir functionality
    %% which properly tracks and handles the various legacy formats for
    %% recursion setting (erl_opts vs. dir options and profiles)
    ExtraDirsOpts = [case epm_dir:recursive(AppOpts, Dir) of
                         false -> {Dir, [{recursive, false}]};
                         true -> Dir
                     end || Dir <- ExtraDirs],
    OldSrcDirsOpts = [case epm_dir:recursive(AppOpts, Dir) of
                          false -> {Dir, [{recursive, false}]};
                          true -> Dir
                      end || Dir <- OldSrcDirs],
    AppDir = epm_app_info:dir(AppInfo),
    lists:map(fun({DirOpt, Dir}) ->
        EbinDir = filename:join(epm_app_info:out_dir(AppInfo), Dir),
        %% need a unique name to prevent lookup issues that clobber entries
        AppName = unicode:characters_to_binary(
            [epm_app_info:name(AppInfo), "_", Dir]
        ),
        AppInfo0 = epm_app_info:name(AppInfo, AppName),
        AppInfo1 = epm_app_info:ebin_dir(AppInfo0, EbinDir),
        AppInfo2 = epm_app_info:set(AppInfo1, src_dirs, [DirOpt]),
        AppInfo3 = epm_app_info:set(AppInfo2, extra_src_dirs, OldSrcDirsOpts),
...
find_source_files(BaseDir, SrcExt, SrcDirs, Opts) ->
    ...
    lists:flatmap(fun(SrcDir) ->
                      Recursive = epm_dir:recursive(Opts, SrcDir),
                      epm_utils:find_files_in_dirs([filename:join(BaseDir, SrcDir)], SourceExtRe, Recursive)
                  end, SrcDirs).

add_to_includes(AppInfo, Dirs) ->
    Opts = epm_app_info:opts(AppInfo),
    List = epm_opts:get(Opts, erl_opts, []),
    NewErlOpts = [{i, Dir} || Dir <- Dirs] ++ List,
    NewOpts = epm_opts:set(Opts, erl_opts, NewErlOpts),
    epm_app_info:opts(AppInfo, NewOpts).
