...
-module(epm_compiler_dag).
-export([init/4, status/4, maybe_store/5, terminate/1
    ,prune/5, populate_sources/5, populate_deps/3, propagate_stamps/1,
...
%% epm.config deps order, and enforce the sorted app
...
dag_file(Dir, CompilerMod, undefined) ->
    filename:join([epm_dir:local_cache_dir(Dir), CompilerMod,
                   ?DAG_ROOT ++ ?DAG_EXT]);
