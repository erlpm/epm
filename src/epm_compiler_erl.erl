...
context(AppInfo) ->
    ...
    OutDir = epm_app_info:dir(AppInfo),
    SrcDirs = epm_dir:src_dirs(epm_app_info:opts(AppInfo), ["src"]),
    ...
    AbsIncl = [filename:join(OutDir, "include") | % standard include path
              ...
              lists:append([
                  find_recursive_incl(OutDir, Src, EpmOpts) ||
...
needed_files(Graph, FoundFiles, _, AppInfo) ->
    ...
    NeededErlFiles = case needed_files(Graph, ErlOpts, EpmOpts, OutDir, EbinDir, ParseTransforms) of
                         [] ->
                             needed_files(Graph, ErlOpts, EpmOpts, OutDir, EbinDir, Rest);
                         _ ->
                             %% at least one parse transform in the opts needs updating, so recompile all
                             FoundFiles
                     end,
    {ErlFirstFiles, ErlOptsFirst} = erl_first_files(EpmOpts, ErlOpts, Dir, NeededErlFiles),
...
dependencies(Source, _SourceDir, Dirs, DepOpts) ->
    epm_compiler_epp:ensure_started(),
    OptPTrans = proplists:get_value(parse_transforms, DepOpts, []),
    try epm_compiler_epp:deps(Source, DepOpts) of
        #{include := AbsIncls,
          ...
          behaviour := Behaviours} ->
            ...
            lists:filtermap(
                fun (Mod) -> epm_compiler_epp:resolve_source(Mod, Dirs) end,
...
compile(Source, [{_, OutDir}], Config, ErlOpts) ->
    case compile:file(Source, [{outdir, OutDir}, no_spawn_compiler_process | ErlOpts]) of
        ...
        {ok, _Mod, Ws} ->
            ...
            epm_compiler:ok_tuple(Source, FormattedWs);
        {error, Es, Ws} ->
...
compile_and_track(Source, [{Ext, OutDir}], Config, ErlOpts) ->
    ...
    case compile:file(Source, BuildOpts) of
        ...
        {ok, _Mod, Ws} ->
            FormattedWs = format_error_sources(Ws, Config),
            {ok, Warns} = epm_compiler:ok_tuple(Source, FormattedWs),
...
error_tuple(Module, Es, Ws, AllOpts, Opts) ->
    ...
    epm_compiler:error_tuple(Module, FormattedEs, FormattedWs, Opts).

format_error_sources(Es, Opts) ->
    [{epm_compiler:format_error_source(Src, Opts), Desc}
     || {Src, Desc} <- Es].

find_recursive_incl(Base, Src, Opts) ->
    find_recursive_incl(Base, Src, Opts, epm_dir:recursive(Opts, Src)).
...
needed_files(Graph, ErlOpts, EpmOpts, Dir, OutDir, SourceFiles) ->
      ...
      fun(Incl) ->
              case filelib:is_regular(Incl) of
                  true ->
                      [Incl];
                  false ->
                      Res = epm_utils:find_files_in_dirs(Dirs, [$^, Incl, $$], true),
                      case Res of
                          [] ->
