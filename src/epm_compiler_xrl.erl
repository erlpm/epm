...
needed_files(_, FoundFiles, Mappings, AppInfo) ->
    ...
    Opts = epm_opts:get(epm_app_info:opts(AppInfo), xrl_opts, []),
...
compile(Source, [{_, _}], _, Opts) ->
    case leex:file(Source, [{return, true} | Opts]) of
        ...
        {error, Es, Ws} ->
            epm_compiler:error_tuple(Source, Es, Ws, Opts)
    end.

clean(XrlFiles, _AppInfo) ->
    epm_file_utils:delete_each(
      [epm_utils:to_list(re:replace(F, "\\.xrl$", ".erl", [unicode]))
       || F <- XrlFiles]).
