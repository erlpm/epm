...
-spec compile_order([epm_app_info:t()]) ->
    {ok, [epm_app_info:t()]} | {error, no_sort | {cycles, [[binary(),...]]}}.
compile_order(Apps) ->
    Graph = digraph:new(),
...
%% for them.
-spec names_to_apps([atom()], [epm_app_info:t()]) -> [epm_app_info:t()].
...
-spec find_app_by_name(atom(), [epm_app_info:t()]) -> {ok, epm_app_info:t()} | error.
...
%% the deps listed in its epm.config is all deps that may be needed
%% for building the app.
-spec all_apps_deps(epm_app_info:t()) -> [binary()].
all_apps_deps(App) ->
    Applications = lists:usort([atom_to_binary(X, utf8) || X <- epm_app_info:applications(App)]),
    Deps = lists:usort(lists:map(fun({Name, _}) -> Name; (Name) -> Name end, epm_app_info:deps(App))),
