...
-spec base_dir(epm_state:t()) -> file:filename_all().
base_dir(State) ->
    profile_dir(epm_state:opts(State), epm_state:current_profiles(State)).

%% @doc returns the directory root for build artifacts for a given set
%% of profiles.
-spec profile_dir(epm_dict(), [atom(), ...]) -> file:filename_all().
profile_dir(Opts, Profiles) ->
    BasePath =
        case Profiles of
            [global | _] -> ?MODULE:global_cache_dir(Opts);
            [_|_] -> epm_opts:get(Opts, base_dir, ?DEFAULT_BASE_DIR)
        end,
...
root_dir(State) ->
    filename:absname(epm_state:get(State, root_dir, ?DEFAULT_ROOT_DIR)).

%% @doc returns the expected location of the `_checkouts' directory.
-spec checkouts_dir(epm_state:t()) -> file:filename_all().
checkouts_dir(State) ->
    epm_file_utils:canonical_path(filename:join(root_dir(State), epm_state:get(State, checkouts_dir, ?DEFAULT_CHECKOUTS_DIR))).

%% @doc returns the expected location of a given app in the checkouts
%% directory for the project.
-spec checkouts_dir(epm_state:t(), file:filename_all()) -> file:filename_all().
...
-spec checkouts_out_dir(epm_state:t()) -> file:filename_all().
checkouts_out_dir(State) ->
    filename:join(base_dir(State), epm_state:get(State, checkouts_out_dir, ?DEFAULT_CHECKOUTS_OUT_DIR)).

%% @doc returns the expected location of a given app in the checkouts
%% directory for the project.
-spec checkouts_out_dir(epm_state:t(), file:filename_all()) -> file:filename_all().
checkouts_out_dir(State, App) ->
    filename:join(checkouts_out_dir(State), App).

%% @doc Returns the directory where plugins are located.
-spec plugins_dir(epm_state:t()) -> file:filename_all().
plugins_dir(State) ->
    case lists:member(global, epm_state:current_profiles(State)) of
        true ->
            filename:join([base_dir(State), global_config_dir(State), epm_state:get(State, plugins_dir, ?DEFAULT_PLUGINS_DIR)]);
        false ->
            filename:join(base_dir(State), epm_state:get(State, plugins_dir, ?DEFAULT_PLUGINS_DIR))
    end.

%% @doc returns the list of relative path where the project applications can
%% be located.
-spec lib_dirs(epm_state:t()) -> file:filename_all().
lib_dirs(State) ->
    epm_state:get(State, project_app_dirs, ?DEFAULT_PROJECT_APP_DIRS).
...
%% @doc returns the directory where the global configuration files for epm
%% may be stored.
-spec global_config_dir(epm_state:t()) -> file:filename_all().
global_config_dir(State) ->
    filename:join([epm_config_dir(State), ".config", "epm"]).

epm_config_dir(State) ->
    case os:getenv("EPM_GLOBAL_CONFIG_DIR") of
        false ->
            epm_state:get(State, global_epm_dir, home_dir());
        ConfDir ->
            ConfDir
    end.

%% @doc returns the path of the global epm.config file
-spec global_config(epm_state:t()) -> file:filename_all().
global_config(State) ->
    filename:join(global_config_dir(State), "epm.config").

%% @doc returns the default path of the global epm.config file
-spec global_config() -> file:filename_all().
global_config() ->
    Home = home_dir(),
    filename:join([Home, ".config", "epm", "epm.config"]).

%% @doc returns the location for the global cache directory
-spec global_cache_dir(epm_dict()) -> file:filename_all().
global_cache_dir(Opts) ->
    Home = home_dir(),
    epm_opts:get(Opts, global_epm_dir, filename:join([Home, ".cache", "epm"])).
...
-spec template_globals(epm_state:t()) -> file:filename_all().
...
%% the project.
-spec processing_base_dir(epm_state:t(), file:filename()) -> boolean().
processing_base_dir(State, Dir) ->
    AbsDir = filename:absname(Dir),
    AbsDir =:= epm_state:get(State, base_dir).
...
%% Example:
%% ```
%% 1> epm_dir:make_relative_path("a/b/c/d/file", "a/b/file").
%% "c/d/file"
%% 2> epm_dir:make_relative_path("a/b/file", "a/b/c/d/file").
%% "../../file"
%% '''
-spec make_relative_path(file:filename(), file:filename()) -> file:filename().
make_relative_path(Source, Target) ->
    AbsSource = epm_file_utils:normalized_path(Source),
    AbsTarget = epm_file_utils:normalized_path(Target),
...
src_dirs(Type, Opts, Default) ->
    lists:usort([
        case D0 of
            {D,_} -> epm_file_utils:normalize_relative_path(D);
            _ -> epm_file_utils:normalize_relative_path(D0)
...
raw_src_dirs(Type, Opts, Default) ->
    ErlOpts = epm_opts:erl_opts(Opts),
    Vs = proplists:get_all_values(Type, ErlOpts),
    case lists:append([epm_opts:get(Opts, Type, []) | Vs]) of
        []   -> Default;
        Dirs -> Dirs
    end.
...
%% `extra_src_dirs').
-spec all_src_dirs(epm_dict()) -> list(file:filename_all()).
all_src_dirs(Opts) -> all_src_dirs(Opts, [], []).

%% @doc returns all the source directories (`src_dirs' and
%% `extra_src_dirs') while being able to configure defaults for both.
-spec all_src_dirs(epm_dict(), list(file:filename_all()), list(file:filename_all())) ->
...
-spec src_dir_opts(epm_dict(), file:filename_all()) -> [{atom(),term()}].
...
-spec recursive(epm_dict(), file:filename_all()) -> boolean().
recursive(Opts, Dir) ->
    DirOpts = src_dir_opts(Opts, Dir),
    Default = proplists:get_value(recursive,
                                  epm_opts:get(Opts, erlc_compiler, []),
                                  true),
...
-spec retarget_path(epm_state:t(), string()) -> string().
...
%% project root
retarget_path(State, Path, []) ->
    case epm_file_utils:path_from_ancestor(epm_file_utils:canonical_path(Path), epm_state:dir(State)) of
...
retarget_path(State, Path, [App|Rest]) ->
    case epm_file_utils:path_from_ancestor(epm_file_utils:canonical_path(Path), epm_app_info:dir(App)) of
        {ok, NewPath}      -> filename:join([epm_app_info:out_dir(App), NewPath]);
...
format_source_file_name(Path, Opts) ->
    Type = case epm_opts:get(Opts, compiler_source_format,
    ...
    case Type of
        ...
        relative ->
            Cwd = epm_dir:get_cwd(),
            epm_dir:make_relative_path(resolve_linked_source(Path), Cwd)
    end.
...
warn_source_format_once(Format) ->
    Warn = application:get_env(epm, warn_source_format) =/= {ok, false},
    application:set_env(epm, warn_source_format, false),
    case Warn of
        false ->
            ok;
        true ->
...
resolve_linked_source(Src) ->
    {Dir, Base} = epm_file_utils:split_dirname(Src),
    filename:join(epm_file_utils:resolve_link(Dir), Base).
