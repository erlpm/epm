...
from_state(BaseConfig, State) ->
    HexConfig = epm_state:get(State, hex, []),
    ...
    %% add base config entries that are specific to use by epm and not overridable
    Repos1 = merge_with_base_and_auth(Repos, BaseConfig, Auth),
    %% merge organizations parent repo options into each organization repo
    update_organizations(maybe_override_default_repo_url(Repos1, State)).

-spec get_repo_config(unicode:unicode_binary(), epm_state:t() | [repo()])
                     -> {ok, repo()} | error.
...
update_organizations(Repos) ->
    lists:map(fun(Repo=#{repo_name := RepoName,
                      ...
                      ParentRepoUrl = epm_utils:to_list(maps:get(repo_url, Parent)),
                      {ok, _RepoUrl} =
                          epm_uri:append_path(ParentRepoUrl,
                                                filename:join("repos", epm_utils:to_list(RepoName))),
                      %% still let the organization config override this constructed repo url
                      maps:merge(Parent#{repo_url => epm_utils:to_binary(ParentRepoUrl)}, Repo);
                 (Repo) ->
                      Repo
              end, Repos).
...
maybe_override_default_repo_url(Repos, State) ->
    lists:map(fun(Repo=#{repo_name := ?PUBLIC_HEX_REPO}) ->
                      Repo#{repo_url => epm_state:default_hex_repo_url_override(State)};
...
default_repo() ->
    HexDefaultConfig = erlpm_core:default_config(),
    HexDefaultConfig#{name => ?PUBLIC_HEX_REPO, repo_verify_origin => repo_verify_origin()}.

repo_verify_origin() ->
    case os:getenv("EPM_NO_VERIFY_REPO_ORIGIN") of
        "1" -> false;
        _ -> true
    end.
...
-spec auth_config_file(epm_state:t()) -> file:filename_all().
auth_config_file(State) ->
    filename:join(epm_dir:global_config_dir(State), ?HEX_AUTH_FILE).

-spec auth_config(epm_state:t()) -> map().
auth_config(State) ->
