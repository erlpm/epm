...
download_(Dir, {hg, Url, {branch, Branch}}, _State) ->
    ...
    epm_utils:sh(?FMT("hg clone -q -b ~ts ~ts ~ts",
...
download_(Dir, {hg, Url, {tag, Tag}}, _State) ->
    ok = filelib:ensure_dir(Dir),
    maybe_warn_local_url(Url),
    epm_utils:sh(?FMT("hg clone -q -u ~ts ~ts ~ts",
...
download_(Dir, {hg, Url, {ref, Ref}}, _State) ->
    ok = filelib:ensure_dir(Dir),
    maybe_warn_local_url(Url),
    epm_utils:sh(?FMT("hg clone -q -r ~ts ~ts ~ts",
...
make_vsn(AppInfo, _) ->
    check_type_support(),
    make_vsn_(epm_app_info:dir(AppInfo)).

make_vsn_(Dir) ->
    BaseHg = "hg -R \"" ++ epm_utils:escape_double_quotes(Dir) ++ "\" ",
    Ref = get_ref(Dir),
    ...
    RawVsn = epm_string:trim(VsnString, both, "\n"),
...
compare_url(Dir, Url) ->
    CurrentUrl = epm_string:trim(os:cmd("hg -R \"" ++ epm_utils:escape_double_quotes(Dir) ++"\" paths default"), both, "\n"),
    CurrentUrl1 = epm_string:trim(CurrentUrl, both, "\r"),
    parse_hg_url(CurrentUrl1) =:= parse_hg_url(Url).

get_ref(Dir) ->
    AbortMsg = io_lib:format("Get ref of hg dependency failed in ~ts", [Dir]),
    {ok, RefString} =
        epm_utils:sh("hg -R \"" ++ epm_utils:escape_double_quotes(Dir) ++ "\" --debug id -i",
                       [{use_stdout, false}, {debug_abort_on_error, AbortMsg}]),
    epm_string:trim(RefString, both, "\n").

get_tag_distance(Dir, Ref) ->
    AbortMsg = io_lib:format("Get tag distance of hg dependency failed in ~ts", [Dir]),
    {ok, LogString} =
        epm_utils:sh("hg -R \"" ++ epm_utils:escape_double_quotes(Dir) ++ "\" "
                      "log --template \"{latesttag}-{latesttagdistance}\n\" "
                      "--rev " ++ epm_utils:escape_chars(Ref),
                      [{use_stdout, false}, {debug_abort_on_error, AbortMsg}]),
    Log = epm_string:trim(LogString,
                            both, "\n"),
...
get_branch_ref(Dir, Branch) ->
    ...
    {ok, BranchRefString} =
        epm_utils:sh("hg -R \"" ++ epm_utils:escape_double_quotes(Dir) ++
                       "\" log --template \"{node}\n\" --rev " ++ epm_utils:escape_chars(Branch),
                       [{use_stdout, false}, {debug_abort_on_error, AbortMsg}]),
    epm_string:trim(BranchRefString, both, "\n").
...
parse_hg_url("ssh://" ++ HostPath) ->
    [Host | Path] = epm_string:lexemes(HostPath, "/"),
...
check_type_support() ->
    case get({is_supported, ?MODULE}) of
        true ->
            ok;
        _ ->
            case epm_utils:sh("hg --version", [{return_on_error, true},
                                                 {use_stdout, false}]) of
                {error, _} ->
