...
-spec run_all_hooks(file:filename_all(), pre | post,
                   ...
                   [providers:t()], epm_app_info:t(), epm_state:t()) -> epm_app_info:t().
run_all_hooks(Dir, Type, Command, Providers, AppInfo, State) ->
    ?DEBUG("Running hooks for ~p in app ~ts (~ts) with configuration:",
           [Command, epm_app_info:name(AppInfo), Dir]),
    State1 = epm_state:current_app(State, AppInfo),
    State2 = run_provider_hooks(Dir, Type, Command, Providers, epm_app_info:opts(AppInfo), State1),
    run_hooks(Dir, Type, Command, epm_app_info:opts(AppInfo), State1),
    epm_state:current_app(State2).

run_all_hooks(Dir, Type, Command, Providers, State) ->
    ?DEBUG("Running hooks for ~p with configuration:", [Command]),
    run_provider_hooks(Dir, Type, Command, Providers, epm_state:opts(State), State),
    run_hooks(Dir, Type, Command, epm_state:opts(State), State).
...
run_project_and_app_hooks(Dir, Type, Command, Providers, State) ->
    ...
    [epm_hooks:run_all_hooks(Dir, Type, Command, Providers, AppInfo, State) || AppInfo <- ProjectApps],
...
run_provider_hooks(Dir, Type, Command, Providers, Opts, State) ->
    case epm_opts:get(Opts, provider_hooks, []) of
        [] ->
            State;
        AllHooks ->
            TypeHooks = proplists:get_value(Type, AllHooks, []),
            run_provider_hooks_(Dir, Type, Command, Providers, TypeHooks, epm_state:opts(State, Opts))
    end.
...
run_provider_hooks_(Dir, Type, Command, Providers, TypeHooks, State) ->
    case proplists:get_all_values(Command, TypeHooks) of
        ...
        HookProviders ->
            ...
            Providers1 = epm_state:providers(State),
            %% Drop CLI arguments from the command for the hook.
            State0 = epm_state:command_parsed_args(
                       epm_state:command_args(State, []),
                       {[], []}
            ),
            State1 = epm_state:providers(epm_state:dir(State0, Dir), Providers++Providers1),
            ?DEBUG("\t{provider_hooks, [{~p, ~p}]}.",
                   [Type, HookProviders]),
            case epm_core:do(HookProviders, State1) of
                {error, ProviderName} ->
                    ?DEBUG(format_error({bad_provider, Type, Command, ProviderName}), []),
                    throw(?PRV_ERROR({bad_provider, Type, Command, ProviderName}));
                {ok, State2} ->
                    epm_paths:set_paths([deps], State2),
                    State2
            end
    end.
...
run_hooks(Dir, Type, Command, Opts, State) ->
    case epm_opts:get(Opts, Type, []) of
        [] ->
            ?DEBUG("\t{~p, []}.", [Type]),
            ?DIAGNOSTIC("run_hooks(~p, ~p, ~p) -> no hooks defined\n", [Dir, Type, Command]),
            ok;
        Hooks ->
            Env = epm_env:create_env(State, Opts),
...
apply_hook(Dir, Env, {Arch, Command, Hook}) ->
    case epm_utils:is_arch(Arch) of
        true ->
...
apply_hook(Dir, Env, {Command, Hook}) ->
    ...
    epm_utils:sh(Hook, [use_stdout, {cd, Dir}, {env, Env}, {abort_on_error, Msg}]).
