...
%% to ensure epm at least maintains its old behaviour.
del_dep(OldValue, OldValue, Value) ->
    OldValue--Value;
...
merge_opt(relx, NewValue, OldValue) ->
    ...
    epm_utils:tup_umerge(NewOverlays, OldOverlays)
    ++ epm_utils:tup_umerge(OldOther, NewOther);
...
merge_opt(_Key, NewValue, OldValue) when is_list(NewValue) ->
    case io_lib:printable_list(NewValue) of
        ...
        true ->
            NewValue;
        false ->
            epm_utils:tup_umerge(NewValue, OldValue)
    end;
...
filter_defines([{platform_define, ArchRegex, Key} | Rest], Acc) ->
    case epm_utils:is_arch(ArchRegex) of
        true ->
