...
format_error({missing_package, Name, Vsn}) ->
    io_lib:format("Package not found in any repo: ~ts ~ts", [epm_utils:to_binary(Name),
                                                             epm_utils:to_binary(Vsn)]);
format_error({missing_package, Pkg}) ->
    io_lib:format("Package not found in any repo: ~p", [Pkg]).

-spec get(epm_hex_repos:repo(), binary()) -> {ok, map()} | {error, term()}.
get(Config, Name) ->
    try erlpm_api_package:get(Config, Name) of
...
-spec get_all_names(epm_state:t()) -> [binary()].
get_all_names(State) ->
    verify_table(State),
...
-spec get_package_versions(unicode:unicode_binary(), ec_semver:semver(),
                           ...
                           ets:tid(), epm_state:t()) -> [vsn()].
get_package_versions(Dep, {_, AlphaInfo}, Repo, Table, State) ->
    ?MODULE:verify_table(State),
    AllowPreRelease = epm_state:get(State, deps_allow_prerelease, false)
        orelse AlphaInfo =/= {[],[]},
...
-spec get_package(unicode:unicode_binary(), unicode:unicode_binary(),
                  binary() | undefined | '_',
                  [unicode:unicode_binary()] | ['_'], ets:tab(), epm_state:t())
        ...
        {ok, _} ->
            case ets:lookup_element(?PACKAGE_TABLE, package_index_version, 1) of
                ...
                V ->
                    ...
                    ?DEBUG("Package index version mismatch. Current version ~p, this epm expecting ~p",
...
registry_dir(State) ->
    CacheDir = epm_dir:global_cache_dir(epm_state:opts(State)),
...
-spec package_dir(epm_hex_repos:repo(), epm_state:t()) -> {ok, file:filename_all()}.
package_dir(Repo, State) ->
    {ok, RegistryDir} = registry_dir(State),
    RepoName = maps:get(name, Repo),
    PackageDir = filename:join([RegistryDir, epm_utils:to_list(RepoName), "packages"]),
    ok = filelib:ensure_dir(filename:join(PackageDir, "placeholder")),
    {ok, PackageDir}.


%% Hex supports use of ~> to specify the version required for a dependency.
%% Since epm requires exact versions to choose from we find the highest
...
update_package(Name, RepoConfig=#{name := Repo}, State) ->
    ...
    ?DEBUG("Getting definition for package ~ts from repo ~ts",
           [Name, epm_hex_repos:format_repo(RepoConfig)]),
    try erlpm_repo:get_package(get_package_repo_config(RepoConfig), Name) of
        {ok, {200, _Headers, Releases}} ->
            _ = insert_releases(Name, Releases, Repo, ?PACKAGE_TABLE),
            {ok, RegistryDir} = epm_packages:registry_dir(State),
        ...
        Error ->
            ...
            %% TODO: add better log message. erlpm_core should export a format_error
...
unverified_repo_message() ->
    ...
    "You can disable this check by setting EPM_NO_VERIFY_REPO_ORIGIN=1".
...
resolve_version(Dep, DepVsn, _OldHash, Hash, HexRegistry, State) when is_binary(Hash) ->
    ...
    case get_package(Dep, DepVsn, Hash, RepoNames, HexRegistry, State) of
        {ok, Package=#package{key={_, _, RepoName}}} ->
            {ok, RepoConfig} = epm_hex_repos:get_repo_config(RepoName, RepoConfigs),
