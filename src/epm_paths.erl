...
-spec set_paths(targets(), epm_state:t()) -> ok.
...
-spec unset_paths(targets(), epm_state:t()) -> ok.
...
-spec clashing_apps(targets(), epm_state:t()) -> [{target(), [binary()]}].
clashing_apps(Targets, State) ->
    AppGroups = app_groups(Targets, State),
    AppNames = [{G, sets:from_list(
                    [epm_app_info:name(App) || App <- Apps]
                )} || {G, Apps} <- AppGroups],
...
normalize_targets(List) ->
    ...
    %% from future versions of epm, possibly from plugins and so on,
...
purge_and_load([{_Group, Apps}|Rest], Seen) ->
    ...
    [begin
         ...
         case application:unload(AtomApp) of
            ...
            %% Shouldn't unload ourselves; epm runs without ever
    ...
    CandidateMods = lists:append(
        ...
        [case application:get_key(AppName, modules) of
             ...
             undefined ->
                 ...
                 case epm_app_info:app_details(epm_app_info:app_details(App, [])) of
                     [] -> [];
                     Details -> proplists:get_value(modules, Details, [])
                 end
         end || App <- GoodApps,
                AppName <- [binary_to_atom(epm_app_info:name(App), utf8)]]
    ),
...
get_paths(deps, State) ->
    epm_state:code_paths(State, all_deps);
get_paths(plugins, State) ->
    epm_state:code_paths(State, all_plugin_deps);
get_paths(runtime, State) ->
    RuntimeApps = get_apps(runtime, State),
    [epm_app_info:ebin_dir(App) || App <- RuntimeApps].
...
get_apps(deps, State) ->
    ...
    epm_state:project_apps(State) ++ epm_state:all_deps(State);
get_apps(plugins, State) ->
    epm_state:all_plugin_deps(State);
get_apps(runtime, State) ->
    ...
    AppsList = epm_state:project_apps(State) ++ epm_state:all_deps(State),
...
get_runtime_apps([App|Rest0], AppsAcc0, AppsList) ->
    Apps = epm_app_info:applications(App),
    IncludedApps = epm_app_info:included_applications(App),
    TotalApps0 = [atom_to_binary(A, utf8) || A <- (Apps ++ IncludedApps)],
    TotalApps = TotalApps0 -- [epm_app_info:name(A) || A <- sets:to_list(AppsAcc0)],

    {Rest1, AppsAcc1} =
        lists:foldl(
            fun(AppName, {Rest, Acc}) ->
                %% We only care about those apps we ccould find in the state.
                case epm_app_utils:find(AppName, AppsList) of
