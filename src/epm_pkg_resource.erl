...
-type package() :: {pkg, binary(), binary(), binary(), binary(), epm_hex_repos:repo()}.
...
init(Type, State) ->
    ...
    BaseConfig = #{http_adapter => {epm_httpc_adapter, #{profile => epm}},
                   http_user_agent_fragment =>
                       <<"(epm/", (list_to_binary(Vsn))/binary, ") (httpc)">>},
    Repos = epm_hex_repos:from_state(BaseConfig, State),
    Resource = epm_resource_v2:new(Type, ?MODULE, #{repos => Repos,
...
-spec lock(AppInfo, ResourceState) -> Res when
      ...
      ResourceState :: epm_resource_v2:resource_state(),
      Res :: {atom(), string(), any(), binary(), binary()}.
lock(AppInfo, _) ->
    {pkg, Name, Vsn, OldHash, Hash, _RepoConfig} = epm_app_info:source(AppInfo),
...
needs_update(AppInfo, _) ->
    {pkg, _Name, Vsn, _OldHash, _Hash, _} = epm_app_info:source(AppInfo),
    epm_utils:to_binary(epm_app_info:original_vsn(AppInfo)) =/= epm_utils:to_binary(Vsn).
...
download(TmpDir, AppInfo, State, ResourceState) ->
    case download(TmpDir, epm_app_info:source(AppInfo), State, ResourceState, true) of
        ok ->
            ok;
        Error ->
            {error, Error}
...
download(TmpDir, Pkg={pkg, Name, Vsn, _OldHash, _Hash, Repo}, State, _ResourceState, UpdateETag) ->
    {ok, PackageDir} = epm_packages:package_dir(Repo, State),
...
%%------------------------------------------------------------------------------
%% @doc
%% Implementation of epm_resource make_vsn callback.
...
format_error({bad_registry_checksum, Name, Vsn, Expected, Found}) ->
    io_lib:format("The checksum for package at ~ts-~ts (~ts) does not match the "
                  ...
                  "Run `epm do unlock ~ts, update` and then try again.",
...
%% @end
%%------------------------------------------------------------------------------
-spec request(epm_hex_repos:repo(), binary(), binary(), false | binary())
             -> {ok, cached} | {ok, binary(), binary()} | error.
request(Config, Name, Version, ETag) ->
    Config1 = Config#{http_etag => ETag},
    try erlpm_repo:get_tarball(Config1, Name, Version) of
...
serve_from_memory(TmpDir, Binary, {pkg, _Name, _Vsn, OldHash, Hash, _RepoConfig}) ->
    ...
    case erlpm_tarball:unpack(Binary, TmpDir) of
...
anon({pkg, PkgName, PkgVsn, OldHash, Hash, RepoConfig}) ->
    {pkg, PkgName, PkgVsn, OldHash, Hash,
     epm_hex_repos:anon_repo_config(RepoConfig)}.
