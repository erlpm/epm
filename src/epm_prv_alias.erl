...
%%% from a plugin at https://github.com/tsloughter/epm_alias after
%%% years of stability. Only some error checks were added
-module(epm_prv_alias).
...
init(State) ->
    Aliases = epm_state:get(State, alias, []),
...
init_alias(Alias, Cmds, State) ->
    Module = list_to_atom("epm_prv_alias_" ++ atom_to_list(Alias)),
...
validate_provider(Alias, Cmds, State) ->
    ...
    %% is friendlier
    case providers:get_provider(Alias, epm_state:providers(State)) of
        not_found ->
...
example(Alias) ->
    "epm " ++ atom_to_list(Alias).

-dialyzer({no_unused, desc/1}). % required since we suppress warnings for init_alias/3
desc(Cmds) ->
    "Equivalent to running: epm do "
        ++ epm_string:join(lists:map(fun to_desc/1, Cmds), ",").
...
do_func(Cmds) ->
    {function, 1, do, 1,
     [{clause, 1,
       ...
       [],
       [{call, 1,
         {remote, 1, {atom, 1, epm_prv_do}, {atom, 1, do_tasks}},
