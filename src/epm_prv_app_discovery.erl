%% -*- erlang-indent-level: 4;indent-tabs-mode: nil -*-
%% ex: ts=4 sw=4 et

-module(epm_prv_app_discovery).

-behaviour(provider).

-export([init/1,
         do/1,
         format_error/1]).

-include("epm.hrl").
-include_lib("providers/include/providers.hrl").

-define(PROVIDER, app_discovery).
-define(DEPS, []).

%% ===================================================================
%% Public API
%% ===================================================================

-spec init(epm_state:t()) -> {ok, epm_state:t()}.
init(State) ->
    State1 = epm_state:add_provider(State, providers:create([{name, ?PROVIDER},
                                                               {module, ?MODULE},
                                                               {bare, false},
                                                               {deps, ?DEPS},
                                                               {example, ""},
                                                               {short_desc, ""},
                                                               {desc, ""},
                                                               {opts, []}])),
    {ok, State1}.

-spec do(epm_state:t()) -> {ok, epm_state:t()} | {error, string()}.
do(State) ->
    LibDirs = epm_dir:lib_dirs(State),
    try
        State1 = epm_app_discover:do(State, LibDirs),
        State2 = epm_plugins:project_apps_install(State1),
        {ok, State2}
    catch
        throw:{error, {epm_packages, Error}} ->
            {error, {epm_packages, Error}};
        throw:{error, {epm_app_utils, Error}} ->
            {error, {epm_app_utils, Error}};
        throw:{error, {epm_app_discover, Error}} ->
            {error, {epm_app_discover, Error}};
        throw:{error, Error} ->
            ?PRV_ERROR(Error)
    end.

-spec format_error(any()) -> iolist().
format_error({multiple_app_files, Files}) ->
    io_lib:format("Multiple app files found in one app dir: ~ts", [epm_string:join(Files, " and ")]);
format_error({invalid_app_file, File, Reason}) ->
    case Reason of
        {Line, erl_parse, Description} ->
            io_lib:format("Invalid app file ~ts at line ~b: ~p",
                [File, Line, lists:flatten(Description)]);
        _ ->
            io_lib:format("Invalid app file ~ts: ~p", [File, Reason])
    end;
%% Provide a slightly more informative error message for consult of app file failure
format_error({epm_file_utils, {bad_term_file, AppFile, Reason}}) ->
    io_lib:format("Error in app file ~ts: ~ts", [epm_dir:make_relative_path(AppFile,
                                                                              epm_dir:get_cwd()),
                                                 file:format_error(Reason)]);
format_error(Reason) ->
    io_lib:format("~p", [Reason]).
