...
do(State) ->
    ...
    %% that it knows older epm version will ignore so we play nice and
    %% honor it.
    DefaultOutDir = os:getenv("EPM_BARE_COMPILER_OUTPUT_DIR", epm_dir:get_cwd()),
    OutDir = proplists:get_value(outdir, RawOpts, DefaultOutDir),
    [ code:add_pathsa(filelib:wildcard(PathWildcard))
      || PathWildcard <- epm_string:lexemes(Paths, Sep) ],

    case epm_state:project_apps(State) of
        [] ->
            {error, {?MODULE, {not_an_application_structure, OutDir}}};
        [AppInfo] ->
            AppInfo1 = epm_app_info:out_dir(AppInfo, OutDir),

            %% run compile in the default namespace
            epm_prv_compile:compile(epm_state:namespace(State, default), AppInfo1),

            epm_utils:cleanup_code_path(OrigPath),
