...
do(State) ->
    ...
    case compile(State, Tests) of
        ...
        {ok, S} ->
            {RawOpts, _} = epm_state:command_parsed_args(S),
...
run_tests(State, Opts) ->
    ...
    {RawOpts, _} = epm_state:command_parsed_args(State),
...
-spec symlink_to_last_ct_logs(epm_state:t(), list()) -> ok.
symlink_to_last_ct_logs(State, Opts) ->
    LogDir = case proplists:get_value(logdir, Opts) of
        undefined -> filename:join([epm_dir:base_dir(State), "logs"]);
        Dir -> Dir
    end,
    ...
    case CtRunDirs of
        ...
        _ ->
            ...
            case epm_file_utils:symlink_or_copy(Existing, Target) of
                ok -> ok;
                exists ->
                    %% in case the symlink already exists we remove it
                    %% and make a new updated one
                    epm_file_utils:rm_rf(Target),
                    epm_file_utils:symlink_or_copy(Existing, Target);
                Reason -> ?DIAGNOSTIC("Warning, couldn't make a symlink to ~ts, reason: ~p.", [Target, Reason])
            end
    end.

setup_name(State) ->
    {Long, Short, Opts} = epm_dist_utils:find_options(State),
    epm_dist_utils:either(Long, Short, Opts).

prepare_tests(State) ->
    %% command line test options
    CmdOpts = cmdopts(State),
    %% epm.config test options
...
transform_retry(Opts, State) ->
    case proplists:get_value(retry, Opts, false) andalso
        ...
        false ->
            Opts;
        true ->
            Path = filename:join([epm_dir:base_dir(State), "logs", "retry.spec"]),
            filelib:is_file(Path) andalso [{spec, Path}|Opts]
    end.

split_string(String) ->
    epm_string:lexemes(String, [$,]).

transform_group(String) ->
    case epm_string:consult([$[, String, $], $.]) of
...
cfgopts(State) ->
    case epm_state:get(State, ct_opts, []) of
...
ensure_opts([{cover, _}|Rest], Acc) ->
    ?WARN("Cover specs not supported. See https://www.erldoc.com/erlpm/testing/ct/", []),
...
select_tests(State, ProjectApps, CmdOpts, CfgOpts) ->
    ...
    Configs = lists:flatmap(fun(Filename) ->
                                epm_file_utils:consult_config(State, Filename)
                            end, SysConfigs),
    ...
    epm_utils:reread_config(Configs, [update_logger]),

    Opts = merge_opts(CmdOpts,CfgOpts),
    discover_tests(State, ProjectApps, Opts).

%% Merge the option lists from command line and epm.config:
%%
...
default_tests(State, ProjectApps) ->
    BareTest = filename:join([epm_state:dir(State), "test"]),
...
application_dirs([App|Rest], Acc) ->
    TestDir = filename:join([epm_app_info:dir(App), "test"]),
...
inject_ct_state(State, Tests, [App|Rest], Acc) ->
    case inject(epm_app_info:opts(App), State, Tests) of
        {error, _} = Error -> Error;
        NewOpts            ->
            NewApp = epm_app_info:opts(App, NewOpts),
            inject_ct_state(State, Tests, Rest, [NewApp|Acc])
    end;
inject_ct_state(State, Tests, [], Acc) ->
    case inject(epm_state:opts(State), State, Tests) of
        {error, _} = Error -> Error;
        NewOpts            ->
          {ok, {epm_state:opts(State, NewOpts), lists:reverse(Acc)}}
    end.

opts(Opts, Key, Default) ->
    case epm_opts:get(Opts, Key, Default) of
...
erl_opts(Opts, State, Tests) ->
    ...
    case add_transforms(append(CTOpts, ErlOpts), State) of
        {error, _} = Error -> Error;
        NewErlOpts         -> first_files(epm_opts:set(Opts, erl_opts, NewErlOpts), Tests)
    end.
...
first_files(Opts, Tests) ->
    ...
    case append(CTFirstFiles, FirstFiles) of
        {error, _} = Error -> Error;
        NewFirstFiles      -> include_files(epm_opts:set(Opts, erl_first_files, NewFirstFiles), Tests)
    end.
...
include_files(Opts, Tests) ->
    ...
    case append(Is, ErlOpts) of
        {error, _} = Error -> Error;
        NewIncludes        -> ct_macro(epm_opts:set(Opts, erl_opts, NewIncludes))
    end.

ct_macro(Opts) ->
    ErlOpts = opts(Opts, erl_opts, []),
    NewOpts = safe_define_ct_macro(ErlOpts),
    epm_opts:set(Opts, erl_opts, NewOpts).
...
readable(State) ->
    ...
    case proplists:get_value(readable, RawOpts) of
        ...
        "false" -> false;
        "compact" -> compact;
        undefined -> epm_state:get(State, ct_readable, compact)
    end.
    ...
    case proplists:get_value(spec, Opts) of
        undefined ->
            case {proplists:get_value(suite, Opts), proplists:get_value(dir, Opts)} of
                {undefined, undefined}  ->
                    {ok, epm_state:project_apps(State, Apps)};
...
spec_test_dirs(State, Apps, Specs0) ->
    case get_dirs_from_specs(Specs0) of
        {ok,{Specs,SuiteDirs}} ->
            ...
            {ok, epm_state:project_apps(State2, Apps2)};
        Error ->
            Error
    end.
...
set_compile_dirs(State, Apps, What) ->
    ...
    {ok, epm_state:project_apps(NewState, NewApps)}.
...
maybe_inject_test_dir(State, AppAcc, [App|Rest], Dir) ->
    case epm_file_utils:path_from_ancestor(Dir, epm_app_info:dir(App)) of
        {ok, []}   ->
            ...
            ok = copy_bare_suites(Dir, epm_app_info:out_dir(App)),
            Opts = inject_test_dir(epm_state:opts(State), epm_app_info:out_dir(App)),
            {epm_state:opts(State, Opts), AppAcc ++ [App]};
        {ok, Path} ->
            Opts = inject_test_dir(epm_app_info:opts(App), Path),
            {State, AppAcc ++ [epm_app_info:opts(App, Opts)] ++ Rest};
        {error, badparent} ->
            maybe_inject_test_dir(State, AppAcc ++ [App], Rest, Dir)
    end;
maybe_inject_test_dir(State, AppAcc, [], Dir) ->
    case epm_file_utils:path_from_ancestor(Dir, epm_state:dir(State)) of
        {ok, []}   ->
            %% normal operation involves copying the entire directory a
            ...
            ExtrasDir = filename:join([epm_dir:base_dir(State), "extras"]),
            ok = copy_bare_suites(Dir, ExtrasDir),
            Opts = inject_test_dir(epm_state:opts(State), ExtrasDir),
            {epm_state:opts(State, Opts), AppAcc};
        {ok, Path} ->
            Opts = inject_test_dir(epm_state:opts(State), Path),
            {epm_state:opts(State, Opts), AppAcc};
        {error, badparent} ->
            {State, AppAcc}
    end.
...
copy_bare_suites(From, To) ->
    ...
    SrcFiles = epm_utils:find_files(From, ".*\\.[e|h]rl\$", false),
    DataDirs = lists:filter(fun filelib:is_dir/1,
                            filelib:wildcard(filename:join([From, "*_SUITE_data"]))),
    ok = epm_file_utils:cp_r(SrcFiles, To),
    epm_file_utils:cp_r(DataDirs, To).

maybe_copy_spec(State, [App|Apps], Spec) ->
    case epm_file_utils:path_from_ancestor(filename:dirname(Spec), epm_app_info:dir(App)) of
        {ok, []}   ->
            ok = epm_file_utils:cp_r([Spec], epm_app_info:out_dir(App));
        {ok,_} ->
            ok;
        {error,badparent} ->
...
maybe_copy_spec(State, [], Spec) ->
    case epm_file_utils:path_from_ancestor(filename:dirname(Spec), epm_state:dir(State)) of
        {ok, []}   ->
            ExtrasDir = filename:join([epm_dir:base_dir(State), "extras"]),
            ok = epm_file_utils:cp_r([Spec],ExtrasDir);
        _R ->
            ok
    end.
...
inject_test_dir(Opts, Dir) ->
    ...
    ExtraSrcDirs = epm_opts:get(Opts, extra_src_dirs, []),
    epm_opts:set(Opts, extra_src_dirs, ExtraSrcDirs ++ [Dir]).
...
translate(State, [App|Rest], Path) ->
    case epm_file_utils:path_from_ancestor(Path, epm_app_info:dir(App)) of
        {ok, P}            -> filename:join([epm_app_info:out_dir(App), P]);
        {error, badparent} -> translate(State, Rest, Path)
    end;
translate(State, [], Path) ->
    case epm_file_utils:path_from_ancestor(Path, epm_state:dir(State)) of
        {ok, P}            -> filename:join([epm_dir:base_dir(State), "extras", P]);
        ...
        {ok, Filenames} ->
            ...
            case Dirs of
                ...
                _ when length(Dirs) >= N ->
                    ...
                    [epm_file_utils:rm_rf(filename:join([LogDir, Dir])) || Dir <- Discard],
                    ok;
...
maybe_cover_compile(State) ->
    ...
    State1 = case proplists:get_value(cover, RawOpts, false) of
        true  -> epm_state:set(State, cover_enabled, true);
        false -> State
    end,
    epm_prv_cover:maybe_cover_compile(State1).
...
maybe_write_coverdata(State) ->
    ...
    epm_prv_cover:maybe_write_coverdata(State1, Name).
...
help(fail_fast) ->
    ...
    "support this natively, we abort the epm run on a failure. This May break CT's disk logging and "
    "other epm features.";
help(_) ->
    "".
