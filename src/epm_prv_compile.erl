...
do(State) ->
    ...
    epm_paths:set_paths([deps], State),

    Providers = epm_state:providers(State),
    MustBuildDeps = epm_state:deps_to_build(State),
    Deps = epm_state:all_deps(State),
    CompiledDeps = copy_and_build_deps(State, Providers, MustBuildDeps, Deps),
    State0 = epm_state:merge_all_deps(State, CompiledDeps),
    ...
    epm_paths:set_paths([plugins], State1),
...
handle_project_apps(Providers, State) ->
    ...
    {ok, ProjectApps1} = epm_digraph:compile_order(ProjectApps),
    ...
    State2 = epm_state:project_apps(State, ProjectApps2),
    ...
    epm_hooks:run_all_hooks(Cwd, post, ?PROVIDER, Providers, State3),
    case epm_state:has_all_artifacts(State3) of
        {false, File} ->
...
-spec compile(epm_state:t(), [epm_app_info:t()]) -> [epm_app_info:t()]
      ;      (epm_state:t(), epm_app_info:t()) -> epm_app_info:t().
compile(State, AppInfo) ->
    compile(State, epm_state:providers(State), AppInfo).

-spec compile(epm_state:t(), [providers:t()],
              [epm_app_info:t()]) -> [epm_app_info:t()]
      ;      (epm_state:t(), [providers:t()],
              epm_app_info:t()) -> epm_app_info:t().
...
-spec compile(epm_state:t(), [providers:t()],
              [epm_app_info:t()], atom() | undefined) -> [epm_app_info:t()].
...
prepare_project_app(_State, _Providers, AppInfo) ->
    ...
    code:add_pathsz([epm_app_info:dir(AppInfo)]),
    AppInfo.

prepare_compile(State, Providers, AppInfo) ->
    AppDir = epm_app_info:dir(AppInfo),
    epm_hooks:run_all_hooks(AppDir, pre, ?PROVIDER,  Providers, AppInfo, State).

prepare_compilers(State, Providers, AppInfo) ->
    AppDir = epm_app_info:dir(AppInfo),
    epm_hooks:run_all_hooks(AppDir, pre, ?ERLC_HOOK, Providers, AppInfo, State).
...
run_compilers(State, _Providers, Apps, Tag) ->
    ...
    lists:foreach(fun({Mod, {G, {Dir, DAGLabel, CritMeta}}}) ->
        epm_compiler_dag:maybe_store(G, Dir, Mod, DAGLabel, CritMeta),
        epm_compiler_dag:terminate(G)
    end, DAGsInfo),
    Apps.

load_dags(State, Tag) ->
    F = fun(Dir, Mod, DAGLabel, CritMeta) ->
            {epm_compiler_dag:init(Dir, Mod, DAGLabel, CritMeta),
             {Dir, DAGLabel, CritMeta}}
        end,
    map_dags(F, State, Tag).

check_dags(State, Tag) ->
    map_dags(fun epm_compiler_dag:status/4, State, Tag).
...
map_dags(F, State, Tag) ->
    ...
    Dir = epm_dir:deps_dir(State),
    SharedCritMeta = [],
    [{Mod, F(Dir, Mod, DAGLabel, mod_meta(Mod, SharedCritMeta))}
     || Mod <- epm_state:compilers(State)].

mod_meta(epm_compiler_erl, CritMeta) ->
...
finalize_compilers(State, Providers, AppInfo) ->
    AppDir = epm_app_info:dir(AppInfo),
    epm_hooks:run_all_hooks(AppDir, post, ?ERLC_HOOK, Providers, AppInfo, State).

prepare_app_file(State, Providers, AppInfo) ->
    AppDir = epm_app_info:dir(AppInfo),
    epm_hooks:run_all_hooks(AppDir, pre, ?APP_HOOK, Providers, AppInfo, State).

compile_app_files(State, Providers, Apps) ->
    %% Load plugins back for make_vsn calls in custom resources.
    %% The epm_otp_app compilation step is safe regarding the
    %% overall path management, so we can just load all plugins back
    %% in memory.
    epm_paths:set_paths([plugins], State),
...
compile_app_file(State, _Providers, AppInfo) ->
    case epm_otp_app:compile(State, AppInfo) of
...
finalize_app_file(State, Providers, AppInfo) ->
    AppDir = epm_app_info:dir(AppInfo),
    epm_hooks:run_all_hooks(AppDir, post, ?APP_HOOK, Providers, AppInfo, State).

finalize_compile(State, Providers, AppInfo) ->
    AppDir = epm_app_info:dir(AppInfo),
    AppInfo2 = epm_hooks:run_all_hooks(AppDir, post, ?PROVIDER, Providers, AppInfo, State),
...
build_root_extras(State, Apps) ->
    ...
    F = fun(App) -> epm_app_info:dir(App) == BaseDir end,
    case lists:any(F, Apps) of
        true ->
            [];
        false ->
            ProjOpts = epm_state:opts(State),
            Extras = epm_dir:extra_src_dirs(ProjOpts, []),
            {ok, VirtApp} = epm_app_info:new("extra", "0.0.0", BaseDir, []),
...
extra_virtual_apps(State, VApp0, [Dir|Dirs]) ->
    SrcDir = filename:join([epm_state:dir(State), Dir]),
    case ec_file:is_dir(SrcDir) of
        false ->
            extra_virtual_apps(State, VApp0, Dirs);
        true ->
            BaseDir = filename:join([epm_dir:base_dir(State), "extras"]),
            OutDir = filename:join([BaseDir, Dir]),
            copy(epm_state:dir(State), BaseDir, Dir),
            VApp1 = epm_app_info:out_dir(VApp0, BaseDir),
            VApp2 = epm_app_info:ebin_dir(VApp1, OutDir),
            Opts = epm_state:opts(State),
            VApp3 = epm_app_info:opts(VApp2, Opts),
            %% ensure we don't end up running a similar extra fake app while
            %% compiling root-level extras by marking it explicitly null
            VApp4 = epm_app_info:set(VApp3, extra_src_dirs, []),
            %% need a unique name to prevent lookup issues that clobber entries
            AppName = unicode:characters_to_binary(["extra_", Dir]),
            VApp5 = epm_app_info:name(VApp4, AppName),
            [epm_app_info:set(VApp5, src_dirs, [OutDir])
             | extra_virtual_apps(State, VApp0, Dirs)]
    end.
...
build_apps(DAGs, Apps, State) ->
    {Epm, Custom} = lists:partition(
        fun(AppInfo) ->
            Type = epm_app_info:project_type(AppInfo),
            Type =:= epm orelse Type =:= undefined
        end,
        Apps
    ),
    [build_custom_builder_app(AppInfo, State) || AppInfo <- Custom],
    build_epm_apps(DAGs, Epm, State).

build_custom_builder_app(AppInfo, State) ->
    ?INFO("Compiling ~ts", [epm_app_info:name(AppInfo)]),
    Type = epm_app_info:project_type(AppInfo),
    ProjectBuilders = epm_state:project_builders(State),
    case lists:keyfind(Type, 1, ProjectBuilders) of
        ...
        _ ->
            throw(?PRV_ERROR({unknown_project_type, epm_app_info:name(AppInfo), Type}))
    end.

build_epm_apps(DAGs, Apps, _State) when DAGs =:= []; Apps =:= [] ->
    %% No apps to actually build, probably just other compile phases
    %% to run for non-epm apps, someone wanting .app files built,
    %% or just needing the hooks to run maybe.
    ok;
build_epm_apps(DAGs, Apps, State) ->
    epm_paths:set_paths([deps], State),
    ...
    [begin
         {Ctx, ReorderedApps} = epm_compiler:analyze_all(DAG, Apps),
         lists:foreach(
             fun(AppInfo) ->
                DAG =:= LastDAG andalso
                  ?INFO("Compiling ~ts", [epm_app_info:name(AppInfo)]),
                epm_compiler:compile_analyzed(DAG, AppInfo, Ctx)
             end,
             ReorderedApps
         ),
         {ExtraCtx, ReorderedExtraApps} = epm_compiler:analyze_all_extras(DAG, Apps),
         lists:foreach(
             fun(AppInfo) ->
                epm_compiler:compile_analyzed(DAG, AppInfo, ExtraCtx)
             end,
             ReorderedExtraApps
         )
     end || DAG <- DAGs],
...
update_code_paths(State, ProjectApps) ->
    ...
    DepsPaths = epm_state:code_paths(State, all_deps),
    epm_state:code_paths(State, all_deps, DepsPaths ++ ProjAppsPaths ++ ExtrasPaths).

paths_for_apps(Apps) -> paths_for_apps(Apps, []).

paths_for_apps([], Acc) -> Acc;
paths_for_apps([App|Rest], Acc) ->
    {_SrcDirs, ExtraDirs} = resolve_src_dirs(epm_app_info:opts(App)),
    Paths = [filename:join([epm_app_info:out_dir(App), Dir]) || Dir <- ["ebin"|ExtraDirs]],
    FilteredPaths = lists:filter(fun ec_file:is_dir/1, Paths),
    paths_for_apps(Rest, Acc ++ FilteredPaths).

paths_for_extras(State, Apps) ->
    F = fun(App) -> epm_app_info:dir(App) == epm_state:dir(State) end,
...
paths_for_extras(State) ->
    {_SrcDirs, ExtraDirs} = resolve_src_dirs(epm_state:opts(State)),
    Paths = [filename:join([epm_dir:base_dir(State), "extras", Dir]) || Dir <- ExtraDirs],
    lists:filter(fun ec_file:is_dir/1, Paths).

has_all_artifacts(AppInfo1) ->
    case epm_app_info:has_all_artifacts(AppInfo1) of
        {false, File} ->
            throw(?PRV_ERROR({missing_artifact, File}));
        true ->
            epm_app_utils:lint_app_info(AppInfo1),
            true
    end.

copy_app_dirs(AppInfo, OldAppDir, AppDir) ->
    case epm_utils:to_binary(filename:absname(OldAppDir)) =/=
        epm_utils:to_binary(filename:absname(AppDir)) of
        true ->
            ...
            case filelib:is_dir(EbinDir) of
                true ->
                    ...
                    epm_file_utils:cp_r(filelib:wildcard(filename:join([EbinDir, "*"])), OutEbin);
                false ->
                    ok
            end,
            ...
            {SrcDirs, ExtraDirs} = resolve_src_dirs(epm_app_info:opts(AppInfo)),
...
symlink_or_copy(OldAppDir, AppDir, Dir) ->
    ...
    epm_file_utils:symlink_or_copy(Source, Target).

symlink_or_copy_existing(OldAppDir, AppDir, Dir) ->
    Source = filename:join([OldAppDir, Dir]),
    Target = filename:join([AppDir, Dir]),
    case ec_file:is_dir(Source) of
        true -> epm_file_utils:symlink_or_copy(Source, Target);
        false -> ok
    end.
...
copy(Source, Target) ->
    ...
    {ok, Files} = epm_utils:list_dir(Source),
    case [filename:join([Source, F]) || F <- Files] of
        []    -> ok;
        Paths -> epm_file_utils:cp_r(Paths, Target)
    end.
...
resolve_src_dirs(Opts) ->
    SrcDirs = epm_dir:src_dirs(Opts, ["src"]),
    ExtraDirs = epm_dir:extra_src_dirs(Opts, []),
