...
-spec maybe_cover_compile(epm_state:t()) -> ok.
maybe_cover_compile(State) ->
    maybe_cover_compile(State, apps).

-spec maybe_cover_compile(epm_state:t(), [file:name()] | apps) -> ok.
maybe_cover_compile(State, Dirs) ->
    case epm_state:get(State, cover_enabled, false) of
        true  -> cover_compile(State, Dirs);
        false -> ok
    end.

-spec maybe_write_coverdata(epm_state:t(), atom()) -> ok.
...
get_all_coverdata(CoverDir) ->
    ...
    {ok, Files} = epm_utils:list_dir(CoverDir),
    epm_utils:filtermap(fun(FileName) ->
...
analysis(State, Task) ->
    ...
    Analysis = lists:map(fun(Mod) ->
              ...
              Mods),
    true = epm_utils:cleanup_code_path(OldPath),
    lists:sort(Analysis).
...
restore_cover_paths(State) ->
    ...
    end, epm_state:project_apps(State)),
...
cover_compile(State, apps) ->
    ExclApps = [epm_utils:to_binary(A) || A <- epm_state:get(State, cover_excl_apps, [])],
    Apps = filter_checkouts_and_excluded(epm_state:project_apps(State), ExclApps),
...
cover_compile(State, Dirs) ->
    ...
    %% redirect cover output
    true = redirect_cover_output(State, CoverPid),
    ExclMods = epm_state:get(State, cover_excl_mods, []),
...
filter_checkouts_and_excluded([App|Rest], ExclApps, Acc) ->
    case epm_app_info:is_checkout(App) orelse lists:member(epm_app_info:name(App), ExclApps) of
...
write_coverdata(State, Name) ->
    DataDir = cover_dir(State),
    ok = filelib:ensure_dir(filename:join([DataDir, "dummy.log"])),
    ExportFile = filename:join([DataDir, epm_utils:to_list(Name) ++ ".coverdata"]),
...
config_opts(State) ->
    epm_state:get(State, cover_opts, []).
...
cover_dir(State) ->
    filename:join([epm_dir:base_dir(State), "cover"]).
