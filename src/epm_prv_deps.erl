...
display_profile_deps(State, Profile) ->
    ...
    ProfileDeps = epm_state:get(State, {deps, Profile}, []),
    % ProfileDeps include those deps from epm.lock that have been
    % removed from epm.config
...
parse_dep_without_locks(DepsDir, Dep, State) ->
    ParsedDep = epm_app_utils:parse_dep(Dep, root, DepsDir, State, [], 0),
    case Dep of
        {_Name, Src, Level} when is_tuple(Src), is_integer(Level) ->
            % This Dep is not in epm.config but in epm.lock
            epm_app_info:source(ParsedDep, undefined);
        _ ->
            epm_app_utils:expand_deps_sources(ParsedDep, State)
    end.
...
locked_deps_map(State, Profile) ->
    ...
    lists:foldl(fun(Dep, DepsIn) ->
                        case epm_app_info:is_lock(Dep) of
                            true ->
                                DepName = epm_app_info:name(Dep),
                                maps:put(epm_utils:to_binary(DepName), Dep, DepsIn);
                            _ ->
                                DepsIn
                        end
...
display_dep(State, Dep, LockedDeps) ->
    Name = epm_utils:to_binary(epm_app_info:name(Dep)),
    NeedsUpdate = epm_fetch:needs_update(Dep, State),
    Source = epm_app_info:source(Dep),
    LockedSource = case maps:get(Name, LockedDeps, undefined) of
                       undefined -> undefined;
                       LockedDep -> epm_app_info:source(LockedDep)
                   end,
...
source_text({pkg, _Name, Vsn, _Hash}) ->
    [<<"package">>, " ", epm_utils:to_binary(Vsn)];
...
source_text(Source) when is_tuple(Source) ->
    ...
    case element(3, Source) of
        {ref , Ref} ->
            SmallRef = case epm_utils:to_binary(Ref) of
                           <<R:7/binary, _/binary>> -> <<R/binary, "...">>;
                           R -> R
                       end,
            [atom_to_binary(Type, unicode), " source ", SmallRef];
        {_ , Vsn} ->
            [atom_to_binary(Type, unicode), " source ", epm_utils:to_binary(Vsn)];
        _ ->
