...
init(State) ->
    Opts = [{update_plt, $u, "update-plt", boolean, "Enable updating the PLT. Default: true"},
            ...
            {base_plt_location, undefined, "base-plt-location", string, "The location of base PLT file, defaults to $HOME/.cache/epm"},
            {plt_location, undefined, "plt-location", string, "The location of the PLT file, defaults to the profile's base directory"},
            {plt_prefix, undefined, "plt-prefix", string, "The prefix to the PLT file, defaults to \"epm\"" },
            {app, $a, "app", string, "Perform success typing analysis of a single application"},
            {base_plt_prefix, undefined, "base-plt-prefix", string, "The prefix to the base PLT file, defaults to \"epm\"" },
...
desc() ->
    ...
    "options `dialyzer` in epm.config:\n"
    ...
    "`plt_prefix` - the prefix to the PLT file, defaults to \"epm\"**\n"
    ...
    "$HOME/.cache/epm (default) or  a custom directory***\n"
...
do(State) ->
    ...
    epm_paths:unset_paths([plugins], State), % no plugins in analysis
...
%% https://github.com/erlpm/epm/pull/489#issuecomment-107953541
%% Dialyzer gets default plt location wrong way by peeking HOME environment
%% variable which usually is not defined on Windows.
maybe_fix_env() ->
    os:putenv("DIALYZER_PLT", filename:join(epm_dir:home_dir(), ".dialyzer_plt")).
...
plt_name(Prefix) ->
    Prefix ++ "_" ++ epm_utils:otp_release() ++ "_plt".
...
do(Args, State, Plt) ->
    ...
    case PltWarnings + Warnings of
        0 ->
            {ok, State2};
        TotalWarnings ->
            ?INFO("Warnings written to ~ts", [epm_dir:format_source_file_name(Output)]),
...
default_output_file() ->
    epm_utils:otp_release() ++ ".dialyzer_warnings".
...
proj_apps(State) ->
    [ec_cnv:to_atom(epm_app_info:name(App)) ||
...
proj_plt_apps(State) ->
    ...
    DepApps = lists:flatmap(
               fun(App) ->
                       epm_app_info:applications(App) ++
                           epm_app_info:included_applications(App)
               end, Apps),
...
get_base_plt(Args, State) ->
    ...
    case proplists:get_value(base_plt_location, Args, get_config(State, base_plt_location, global)) of
        global ->
            GlobalCacheDir = epm_dir:global_cache_dir(epm_state:opts(State)),
...
proj_files(SingleApp, State) ->
    ...
    BasePltMods = get_config(State, base_plt_mods, []),
    PltMods = get_config(State, plt_extra_mods, []) ++ BasePltMods,
    ExtraDirs = epm_dir:extra_src_dirs(epm_state:opts(State)),
...
run_dialyzer(State, Opts, Output) ->
    ...
    case proplists:get_bool(get_warnings, Opts) of
        true ->
            ...
            Warnings = format_warnings(epm_state:opts(State),
...
format_warnings(Opts, Output, Warnings) ->
    Warnings1 = epm_dialyzer_format:format_warnings(Opts, Warnings),
    console_warnings(Warnings1),
    Config = epm_opts:get(Opts, dialyzer, []),
...
get_config(State, Key, Default) ->
    Config = epm_state:get(State, dialyzer, []),
    proplists:get_value(Key, Config, Default).

debug_info(State) ->
    Config = epm_state:get(State, erl_opts, []),
...
-spec collect_nested_dependent_apps([atom()], epm_state:t()) -> [atom()].
collect_nested_dependent_apps(RootApps, State) ->
    Deps = collect_nested_dependent_apps(RootApps, sets:new(), State),
    sets:to_list(Deps).

-spec collect_nested_dependent_apps([atom()], epm_set(), epm_state:t()) -> epm_set().
...
-spec collect_nested_dependent_app(atom(), epm_set(), epm_state:t()) -> epm_set().
collect_nested_dependent_app(App, Seen, State) ->
        ...
        false ->
            ...
            case code:lib_dir(App) of
                {error, _} ->
                    throw({unknown_application, App});
                AppDir ->
                    case epm_app_discover:find_app(AppDir, all, State) of
                        false ->
                            throw({unknown_application, App});
                        {true, AppInfo}  ->
                            collect_nested_dependent_apps(epm_app_info:applications(AppInfo), Seen1, State)
                    end
            end
    end.
...
dialyzer_version() ->
    ...
    case epm_string:lexemes(Vsn, ".") of
        [Major, Minor] ->
...
format_path(Path) ->
    Normalized = epm_dir:format_source_file_name(Path),
    case filelib:is_file(Normalized) of
        true -> Normalized;
        false -> epm_dir:format_source_file_name(Path, abs_path_opts())
    end.
