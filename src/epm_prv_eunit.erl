        ...
        {ok, T} ->
            case run_tests(State, T) of
                {ok, State1} ->
                    ...
                    epm_hooks:run_project_and_app_hooks(Cwd, post, ?PROVIDER, Providers, State1),
...
cfg_tests(State) ->
    case epm_state:get(State, eunit_tests, []) of
...
set_apps([App|Rest], Acc) ->
    AppName = list_to_atom(binary_to_list(epm_app_info:name(App))),
    set_apps(Rest, [{application, AppName}|Acc]).

set_modules(Apps, State) -> set_modules(Apps, State, {[], []}).

set_modules([], State, {AppAcc, TestAcc}) ->
    Regex = epm_state:get(State, eunit_test_regex, ?DEFAULT_TEST_REGEX),
    BareTestDir = [filename:join([epm_state:dir(State), "test"])],
    TestSrc = gather_src(BareTestDir, Regex),
    dedupe_tests({AppAcc, TestAcc ++ TestSrc});
set_modules([App|Rest], State, {AppAcc, TestAcc}) ->
    F = fun(Dir) -> filename:join([epm_app_info:dir(App), Dir]) end,
    AppDirs = lists:map(F, epm_dir:src_dirs(epm_app_info:opts(App), ["src"])),
    Regex = epm_state:get(State, eunit_test_regex, ?DEFAULT_TEST_REGEX),
    AppSrc = gather_src(AppDirs, Regex),
    TestDirs = [filename:join([epm_app_info:dir(App), "test"])],
...
gather_src([Dir|Rest], Regex, Srcs) ->
    gather_src(Rest, Regex, Srcs ++ epm_utils:find_files(Dir, Regex, true)).
...
dedupe_tests({AppMods, TestMods}) ->
    ...
    epm_utils:filtermap(F, UniqueTestMods).
...
inject_eunit_state(State, [App|Rest], Acc) ->
    case inject(epm_app_info:opts(App)) of
        {error, _} = Error -> Error;
        NewOpts            ->
...
inject_eunit_state(State, [], Acc) ->
    case inject(epm_state:opts(State)) of
        {error, _} = Error -> Error;
        NewOpts            -> {ok, {epm_state:opts(State, NewOpts), lists:reverse(Acc)}}
    end.
...
erl_opts(Opts) ->
    ...
    case append(EUnitOpts, ErlOpts) of
        {error, _} = Error -> Error;
        NewErlOpts         -> first_files(epm_opts:set(Opts, erl_opts, NewErlOpts))
    end.
...
first_files(Opts) ->
    ...
    case append(EUnitFirstFiles, FirstFiles) of
        {error, _} = Error -> Error;
        NewFirstFiles  -> eunit_macro(epm_opts:set(Opts, erl_first_files, NewFirstFiles))
    end.
...
test_dirs(State, Apps, []) -> epm_state:project_apps(State, Apps);
...
%% and epm.config may contain arbitrarily complex tests that are effectively
%% unvalidatable
...
validate_app(State, [App|Rest], AppName) ->
    case AppName == binary_to_atom(epm_app_info:name(App), unicode) of
        true  -> ok;
        false -> validate_app(State, Rest, AppName)
    end.

validate_dir(State, Dir) ->
    case ec_file:is_dir(filename:join([epm_state:dir(State), Dir])) of
        true  -> ok;
        false -> {error, lists:concat(["Directory `", Dir, "' not found."])}
    end.

validate_file(State, File) ->
    case ec_file:exists(filename:join([epm_state:dir(State), File])) of
        true  -> ok;
...
resolve_eunit_opts(State) ->
    {Opts, _} = epm_state:command_parsed_args(State),
    EUnitOpts = epm_state:get(State, eunit_opts, []),
...
translate(State, [App|Rest], {dir, Dir}) ->
    case epm_file_utils:path_from_ancestor(Dir, epm_app_info:dir(App)) of
        {ok, Path}         -> {dir, filename:join([epm_app_info:out_dir(App), Path])};
...
translate(State, [App|Rest], {file, FilePath}) ->
    ...
    case epm_file_utils:path_from_ancestor(Dir, epm_app_info:dir(App)) of
        {ok, Path}         -> {file, filename:join([epm_app_info:out_dir(App), Path, File])};
        {error, badparent} -> translate(State, Rest, {file, FilePath})
    end;
translate(State, [], {dir, Dir}) ->
    case epm_file_utils:path_from_ancestor(Dir, epm_state:dir(State)) of
        {ok, Path}         -> {dir, filename:join([epm_dir:base_dir(State), "extras", Path])};
...
translate(State, [], {file, FilePath}) ->
    ...
    File = filename:basename(FilePath),
    case epm_file_utils:path_from_ancestor(Dir, epm_state:dir(State)) of
        {ok, Path}         -> {file, filename:join([epm_dir:base_dir(State), "extras", Path, File])};
        %% not relative, leave as is
        {error, badparent} -> {file, FilePath}
    end.

apply_sys_config(State) ->
    CfgSysCfg = case epm_state:get(State, eunit_opts, []) of
    ...
    {RawOpts, _} = epm_state:command_parsed_args(State),
    SysCfgs = epm_string:lexemes(
        proplists:get_value(sys_config, RawOpts, ""),
        [$,]
    ) ++ CfgSysCfg,
    Configs = lists:flatmap(
        fun(Filename) -> epm_file_utils:consult_config(State, Filename) end,
        SysCfgs
    ),
