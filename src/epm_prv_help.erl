...
do(State) ->
    case epm_state:command_args(State) of
        [] ->
            help(State),
            {ok, State};
...
%%
help(State) ->
    ?CONSOLE("Epm is a tool for working with Erlang projects.~n", []),
    OptSpecList = epm:global_option_spec_list(),
    getopt:usage(OptSpecList, "epm", "", []),
    ?CONSOLE("  Set the environment variable DEBUG=1 for detailed output.~n", []),
    ?CONSOLE("Several tasks are available:~n", []),

    providers:help(epm_state:providers(State)),

    ?CONSOLE("~nRun 'epm help <TASK>' for details.", []).
