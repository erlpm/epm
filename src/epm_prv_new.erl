...
do(State) ->
    case strip_flags(epm_state:command_args(State)) of
        ["help"] ->
            ?CONSOLE("Call `epm new help <template>` for a detailed description~n", []),
...
info() ->
    io_lib:format(
      "Create epm project based on template and vars.~n"
      "~n"
      "Valid command line options:~n"
      "  <template> [var=foo,...]~n"
      "~n"
      "See available templates with: `epm new help`~n", []).
