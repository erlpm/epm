...
init(State) ->
    State1 = epm_state:add_provider(State,
                                      providers:create([{name, ?PROVIDER},
                                                        {module, ?MODULE},
                                                        {bare, true},
                                                        {deps, ?DEPS},
                                                        {example, "epm pkgs elli"},
...
do(State) ->
    ...
    case proplists:get_value(package, Args, undefined) of
        ...
        Name ->
            ...
            Results = get_package(epm_utils:to_binary(Name), Repos),
...
get_package(Name, Repos) ->
    lists:foldl(fun(RepoConfig, Acc) ->
                        [{maps:get(name, RepoConfig), epm_packages:get(RepoConfig, Name)} | Acc]
                end, [], Repos).


-spec format_error(any()) -> iolist().
format_error(no_package_arg) ->
    "Missing package argument to `epm pkgs` command.";
