...
filter_apps(RawOpts, State) ->
    ...
    Apps = lists:foldl(fun(String, Acc) -> epm_string:lexemes(String, ",") ++ Acc end, [], RawApps),
    case Apps of
        [] ->
            ProjectDeps = project_deps(State),
            ProjectApps = epm_state:project_apps(State),
            lists:map(fun(A) -> binary_to_list(epm_app_info:name(A)) end, ProjectApps) ++ ProjectDeps;
        _  -> Apps
    end.
...
base_dir(State) -> io_lib:format("~ts", [epm_dir:base_dir(State)]).
bin_dir(State)  -> io_lib:format("~ts/bin", [epm_dir:base_dir(State)]).
lib_dir(State)  -> io_lib:format("~ts", [epm_dir:deps_dir(State)]).
rel_dir(State)  -> io_lib:format("~ts/rel", [epm_dir:base_dir(State)]).

ebin_dirs(Apps, State) ->
    lists:flatmap(fun(App) -> [io_lib:format("~ts/~ts/ebin", [epm_dir:checkouts_out_dir(State), App]),
                               io_lib:format("~ts/~ts/ebin", [epm_dir:deps_dir(State), App]) ] end, Apps).
priv_dirs(Apps, State) ->
    lists:flatmap(fun(App) -> [io_lib:format("~ts/~ts/priv", [epm_dir:checkouts_out_dir(State), App]),
                               io_lib:format("~ts/~ts/priv", [epm_dir:deps_dir(State), App]) ] end, Apps).
src_dirs(Apps, State) ->
    lists:flatmap(fun(App) -> [io_lib:format("~ts/~ts/src", [epm_dir:checkouts_out_dir(State), App]),
                               io_lib:format("~ts/~ts/src", [epm_dir:deps_dir(State), App]) ] end, Apps).
...
print_paths_if_exist(Paths, State) ->
    ...
    io:format("~ts", [epm_string:join(RealPaths, Sep)]).

project_deps(State) ->
    Profiles = epm_state:current_profiles(State),
    DepList = lists:foldl(fun(Profile, Acc) -> epm_state:get(State, {deps, Profile}, []) ++ Acc end, [], Profiles),
    LockList = lists:foldl(fun(Profile, Acc) -> epm_state:get(State, {locks, Profile}, []) ++ Acc end, [], Profiles),
