...
do(State) ->
    ...
    GlobalConfig = epm_state:new(epm_config:consult_file(GlobalConfigFile)),
    GlobalPlugins = epm_state:get(GlobalConfig, plugins, []),
    GlobalSrcDirs = epm_state:get(GlobalConfig, src_dirs, ["src"]),
    GlobalPluginsDir = filename:join([epm_dir:global_cache_dir(epm_state:opts(State)), "plugins", "*"]),
    GlobalApps = epm_app_discover:find_apps([GlobalPluginsDir], GlobalSrcDirs, all, State),
    ...
    PluginsDirs = filelib:wildcard(filename:join(epm_dir:plugins_dir(State), "*")),
    ...
    CheckoutsDirs = filelib:wildcard(filename:join(epm_dir:checkouts_dir(State), "*")),
    Apps = epm_app_discover:find_apps(CheckoutsDirs++PluginsDirs, SrcDirs, all, State),
...
list_local_plugins(State) ->
    LocalPluginsDefs = epm_state:get(State, plugins, [])
                       ++ epm_state:get(State, project_plugins, []),
...
display_plugins(Apps, Plugins) ->
    lists:foreach(fun(Plugin) ->
                          Name = if is_atom(Plugin) -> atom_to_binary(Plugin, unicode);
                                    is_tuple(Plugin) -> epm_utils:to_binary(element(1, Plugin))
                                 end,
                          case epm_app_utils:find(Name, Apps) of
                              {ok, App} ->
                                  ?CONSOLE("~ts (~s)", [Name, epm_app_info:original_vsn(App)]);
                              error ->
