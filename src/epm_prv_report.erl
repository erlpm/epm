...
-define(DEPS, []).
-define(ISSUES_URL, "https://github.com/erlang/epm/issues").
...
init(State) ->
    State1 = epm_state:add_provider(State, providers:create([{name, ?PROVIDER},
                                                               ...
                                                               {bare, true},
                                                               {deps, ?DEPS},
                                                               {example, "epm report \"<task>\""},
                                                               {short_desc, "Provide a crash report to be sent to the epm issues page."},
                                                               {desc, "Provide a crash report to be sent to the epm issues page."},
                                                               {opts, [
...
do(State) ->
    %% Show command
    Task = epm_state:command_args(State),
    Command = parse_task(Task),
    %% Show command version (if a plugin?)
    %% ...
    %% Show app versions (including epm)
    {ok, Vsn} = application:get_key(epm, vsn),
    {ok, Apps} = application:get_key(epm, applications),
    ...
    ?CONSOLE(
        ...
        [Vsn, time_to_string(UTC),
         ...
         [providers:format(P)++" "
          || P <- lists:sort(epm_state:providers(State))]
        ]
    ),
    {ok, State}.
