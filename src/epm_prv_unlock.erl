...
do(State) ->
    ...
    case file:consult(LockFile) of
        ...
        {ok, _} ->
            ...
            {ok, epm_state:set(State, {locks, default}, NewLocks)}
    end.
...
handle_unlocks(State, Locks, LockFile) ->
    ...
    Names = parse_names(epm_utils:to_binary(proplists:get_value(package, Args, <<"">>))),
    case [Lock || Lock = {Name, _, _} <- Locks, not lists:member(Name, Names)] of
        ...
        _ when Names =:= [] -> % implicitly all locks
            ...
            {ok, []};
        NewLocks ->
            epm_config:write_lock_file(LockFile, NewLocks),
            {ok, NewLocks}
    end.
