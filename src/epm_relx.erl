...
-spec do(atom(), epm_state:t()) -> {ok, epm_state:t()} | {error, string()}.
do(Provider, State) ->
    {Opts, _} = epm_state:command_parsed_args(State),
    RelxConfig = read_relx_config(State, Opts),

    ProfileString = epm_dir:profile_dir_name(State),
    ...
    XrefIgnores = epm_state:get(State, xref_ignores, []),
    RelxState = rlx_state:filter_xref_warning(RelxState_0,
        fun(Warnings) ->
            epm_prv_xref:filter_xref_results(undefined_function_calls, XrefIgnores, Warnings) end),
...
read_relx_config(State, Options) ->
    ...
    case ConfigFile of
        "" ->
            ConfigPath = filename:join([epm_dir:root_dir(State), "relx.config"]),
            case {epm_state:get(State, relx, []), file:consult(ConfigPath)} of
                {[], {ok, Config}} ->
...
parallel_run(Provider, Releases, AllApps, RelxState) ->
    epm_parallel:queue(Releases, fun rel_worker/2, [Provider, AllApps, RelxState], fun rel_handler/2, []).
...
%% Returns a map of all apps that are part of the epm project.
