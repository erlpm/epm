...
-callback lock(file:filename_all(), tuple()) ->
    source().
-callback download(file:filename_all(), tuple(), epm_state:t()) ->
...
download(Module, TmpDir, AppInfo, State) ->
    case Module:download(TmpDir, epm_app_info:source(AppInfo), State) of
        {ok, _} ->
            ok;
        Error ->
            Error
...
needs_update(Module, AppInfo) ->
    Module:needs_update(epm_app_info:dir(AppInfo), epm_app_info:source(AppInfo)).

make_vsn(Module, AppInfo) ->
    Module:make_vsn(epm_app_info:dir(AppInfo)).
