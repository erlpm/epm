...
-record(state_t, {dir                               :: file:name(),
                  ...
                  current_app                       :: undefined | epm_app_info:t(),
                  project_apps        = []          :: [epm_app_info:t()],
                  deps_to_build       = []          :: [epm_app_info:t()],
                  all_plugin_deps     = []          :: [epm_app_info:t()],
                  all_deps            = []          :: [epm_app_info:t()],

                  compilers           = []          :: [module()],
                  project_builders    = []          :: [{epm_app_info:project_type(), module()}],
                  resources           = [],
                  providers           = [],
...
new() ->
    ...
    BaseState#state_t{dir = epm_dir:get_cwd()}.
...
new(ParentState=#state_t{}, Config) ->
    %% Load terms from epm.config, if it exists
    Dir = epm_dir:get_cwd(),
...
has_all_artifacts(State) ->
    Artifacts = epm_state:get(State, artifacts, []),
    Dir = epm_dir:base_dir(State),
    all(Dir, Artifacts).
...
add_to_profile(State, Profile, KVs) when is_atom(Profile), is_list(KVs) ->
    Opts = epm_opts:add_to_profile(opts(State), Profile, KVs),
    State#state_t{opts=Opts}.
...
apply_profiles(State=#state_t{default = Defaults, current_profiles=CurrentProfiles}, Profiles) ->
    ...
    ConfigProfiles = epm_state:get(State, profiles, []),
...
deps_names(Deps) when is_list(Deps) ->
    lists:map(fun(Dep) when is_tuple(Dep) ->
                      epm_utils:to_binary(element(1, Dep));
                 (Dep) when is_atom(Dep) ->
                      epm_utils:to_binary(Dep)
              end, Deps);
deps_names(State) ->
    Deps = epm_state:get(State, deps, []),
    deps_names(Deps).
...
project_apps(State=#state_t{project_apps=Apps}, App) ->
    State#state_t{project_apps=lists:keystore(epm_app_info:name(App), 2, Apps, App)}.
...
deps_to_build(State=#state_t{deps_to_build=Apps}, App) ->
    State#state_t{deps_to_build=lists:keystore(epm_app_info:name(App), 2, Apps, App)}.
...
all_checkout_deps(#state_t{all_deps=Apps}) ->
    [App || App <- Apps, epm_app_info:is_checkout(App)].
...
-spec resources(t()) -> [{epm_resource_v2:type(), module()}].
resources(#state_t{resources=Resources}) ->
    Resources.

-spec set_resources(t(), [{epm_resource_v2:type(), module()}]) -> t().
set_resources(State, Resources) ->
    State#state_t{resources=Resources}.

-spec resources(t(), [{epm_resource_v2:type(), module()}]) -> t().
resources(State, NewResources) ->
    lists:foldl(fun(Resource, StateAcc) ->
                        add_resource(StateAcc, Resource)
                end, State, NewResources).

-spec add_resource(t(), {epm_resource_v2:type(), module()}) -> t().
...
warn_old_resource(ResourceModule) ->
    ?WARN("Using custom resource ~s that implements a deprecated api. "
          "It should be upgraded to epm_resource_v2.", [ResourceModule]).

%% @doc get a list of all registered compiler modules, which should implement
%% the `epm_compiler' behaviour
...
%% module should implement the `epm_compiler' behaviour. Use this when
...
%% modules that implement the `epm_compiler' behaviour.
...
default_hex_repo_url_override(State) ->
    CDN = epm_state:get(State, epm_packages_cdn, ?DEFAULT_CDN),
    epm_utils:to_binary(CDN).
...
create_logic_providers(ProviderModules, State0) ->
    try
        ...
        ?WITH_STACKTRACE(C,T,S)
            ...
            throw({error, "Failed creating providers. Run with DIAGNOSTIC=1 for stacktrace or consult epm.crashdump."})
    end.
