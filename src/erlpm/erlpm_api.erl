...
-type response() :: {ok, {erlpm_http:status(), erlpm_http:headers(), body() | nil}} | {error, term()}.
...
request(Config, Method, Path, Body) when is_binary(Path) and is_map(Config) ->
    ...
    case erlpm_http:request(Config, Method, build_url(Path, Config), ReqHeaders2, Body) of
...
%% TODO: copy-pasted from erlpm_repo
make_headers(Config) ->
