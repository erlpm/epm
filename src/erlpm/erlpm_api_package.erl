%% Vendor from epm_core v0.7.1, do not edit manually

-module(erlpm_api_package).

-export([get/2, search/3]).
%% @doc
%% Gets a package.
%%
%% Examples:
%%
%% ```
%% > erlpm_api_package:get(erlpm_core:default_config(), <<"package">>).
%% {ok, {200, ..., #{
%%     <<"name">> => <<"package1">>,
%%     <<"meta">> => #{
%%         <<"description">> => ...,
%%         <<"licenses">> => ...,
%%         <<"links">> => ...,
%%         <<"maintainers">> => ...
%%     },
%%     ...,
%%     <<"releases">> => [
%%         #{<<"url">> => ..., <<"version">> => <<"0.5.0">>}],
%%         #{<<"url">> => ..., <<"version">> => <<"1.0.0">>}],
%%         ...
%%     ]}}}
%% '''
%% @end
-spec get(erlpm_core:config(), binary()) -> erlpm_api:response().
get(Config, Name) when is_map(Config) and is_binary(Name) ->
    Path = erlpm_api:build_repository_path(Config, ["packages", Name]),
    erlpm_api:get(Config, Path).

%% @doc
%% Searches packages.
%%
%% Examples:
%%
%% ```
%% > erlpm_api_package:search(erlpm_core:default_config(), <<"package">>, []).
%% {ok, {200, ..., [
%%     #{<<"name">> => <<"package1">>, ...},
%%     ...
%% ]}}
%% '''
-spec search(erlpm_core:config(), binary(), list(binary())) -> erlpm_api:response().
search(Config, Query, SearchParams) when is_map(Config) and is_binary(Query) and is_list(SearchParams) ->
    QueryString = erlpm_api:encode_query_string([{search, Query} | SearchParams]),
    Path = erlpm_api:join_path_segments(erlpm_api:build_repository_path(Config, ["packages"])),
    PathQuery = <<Path/binary, "?", QueryString/binary>>,
    erlpm_api:get(Config, PathQuery).
