...
-module(erlpm_api_package_owner).
-export([
    add/5, delete/3, get/3, list/2
]).
...
%%
%% ```
%% > erlpm_api_package_owner:list(erlpm_core:default_config(), <<"package">>).
%% {ok, {200, ..., [#{
...
%% '''
%% @end
-spec list(erlpm_core:config(), binary()) -> erlpm_api:response().
list(Config, PackageName) when is_binary(PackageName) ->
    Path = erlpm_api:build_repository_path(Config, ["packages", PackageName, "owners"]),
...
%%
%% ```
%% > erlpm_api_package_owner:get(erlpm_core:default_config(), <<"package">>, <<"user">>).
%% {ok, {200, ..., #{
%%      <<"email">> => <<"user@example.com">>,
%%      <<"full_name">> => <<"John Doe">>,
%%      <<"handles">> => #{...},
...
%%
%% ```
%% > erlpm_api_package_owner:add(erlpm_core:default_config(), <<"package">>, <<"user">>, <<"full">>, false).
%% {ok, {200, ..., #{
%%      <<"email">> => <<"user@example.com">>,
%%      <<"full_name">> => <<"John Doe">>,
%%      <<"handles">> => #{...},
...
%%
%% ```
%% > erlpm_api_package_owner:delete(erlpm_core:default_config(), <<"package">>, <<"user">>).
%% {ok, {204, ..., nil}}
%% '''
%% @end
