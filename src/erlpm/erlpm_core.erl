...
%% @doc
%% epm_core entrypoint module.
%%
%% ### Config
%%
%% Most functions in the epm_core API takes a configuration. The configuration sets things
...
%% * `api_url' - URL to the HTTP API (default: `https://erl.pm/api').
%% * `http_adapter' - A tuple of a callback module used for HTTP requests, see [`erlpm_http'](erlpm_http.html)
%%   (default: `erlpm_http_httpc') and the configuration to pass to the HTTP adapter.
...
%%   (defaults to hexpm public key `https://erl.pm/docs/public_keys').
%% * `repo_url' - URL to the repository (default: `https://erl.pm/repo').
...
%%   requires protobuf messages as of epm_core v0.4.0 (default: `true').
...
default_config() ->
    #{
        ...
        api_url => <<"https://erl.pm/api">>,
        http_adapter => {erlpm_http_httpc, #{profile => default}},
        http_etag => undefined,
        http_headers => #{},
