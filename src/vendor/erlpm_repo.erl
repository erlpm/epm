...
%%
%% ```
%% > erlpm_repo:get_names(erlpm_core:default_config()).
%% {ok, {200, ...,
%%     [
%%         #{name => <<"package1">>},
%%         #{name => <<"package2">>},
...
get_names(Config) when is_map(Config) ->
    ...
    Decoder = fun(Data) ->
        case Verify of
            true -> erlpm_registry:decode_names(Data, repo_name(Config));
            false -> erlpm_registry:decode_names(Data, no_verify)
        end
    end,
...
%%
%% ```
%% > erlpm_repo:get_versions(Config).
%% {ok, {200, ...,
%%     [
...
get_versions(Config) when is_map(Config) ->
    ...
    Decoder = fun(Data) ->
        case Verify of
            true -> erlpm_registry:decode_versions(Data, repo_name(Config));
            false -> erlpm_registry:decode_versions(Data, no_verify)
        end
    end,
...
%%
%% ```
%% > erlpm_repo:get_package(erlpm_core:default_config(), <<"package1">>).
%% {ok, {200, ...,
%%     {
...
get_package(Config, Name) when is_binary(Name) and is_map(Config) ->
    ...
    Decoder = fun(Data) ->
        case Verify of
            true -> erlpm_registry:decode_package(Data, repo_name(Config), Name);
            false -> erlpm_registry:decode_package(Data, no_verify, no_verify)
        end
    end,
...
%%
%% ```
%% > {ok, {200, _, Tarball}} = erlpm_repo:get_tarball(erlpm_core:default_config(), <<"package1">>, <<"1.0.0">>),
%% > {ok, #{metadata := Metadata}} = erlpm_tarball:unpack(Tarball, memory).
%% '''
%% @end
...
get(Config, URI, Headers) ->
    erlpm_http:request(Config, get, URI, Headers, undefined).
...
decode(Signed, PublicKey, Decoder, Config) ->
    ...
    case Verify of
        true ->
            case erlpm_registry:decode_and_verify_signed(Signed, PublicKey) of
                {ok, Payload} ->
                    Decoder(Payload);
                Other ->
                    Other
        ...
        false ->
            #{payload := Payload} = erlpm_registry:decode_signed(Signed),
            Decoder(Payload)
    end.
