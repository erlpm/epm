...
-module(erlpm_tarball).
-export([
    create/2, create/3, create_docs/1, create_docs/2, unpack/2, unpack/3
...
%% > erlpm_tarball:create(Metadata, Files).
%% {ok, #{tarball => <<86,69,...>>,
...
%% > erlpm_tarball:create_docs(Files).
%% {ok, <<86,69,...>>}
%% '''
%% @end
-spec create_docs(files(), erlpm_core:config()) -> {ok, tarball()} | {error, term()}.
...
create_docs(Files) ->
    create_docs(Files, erlpm_core:default_config()).

%% @doc
%% Unpacks a package tarball.
%%
%% Remember to verify the outer tarball checksum against the registry checksum
%% returned from `erlpm_repo:get_package(Config, Package)`.
%%
%% Examples:
%%
%% ```
%% > erlpm_tarball:unpack(Tarball, memory).
%% {ok,#{outer_checksum => <<...>>,
%%       contents => [{"src/foo.erl",<<"-module(foo).">>}],
%%       metadata => #{<<"name">> => <<"foo">>, ...}}}
%%
%% > erlpm_tarball:unpack(Tarball, "path/to/unpack").
%% {ok,#{outer_checksum => <<...>>,
%%       metadata => #{<<"name">> => <<"foo">>, ...}}}
%% '''
...
unpack(Tarball, Output, _Config) ->
    case erlpm_erl_tar:extract({binary, Tarball}, [memory]) of
        {ok, []} ->
            {error, {tarball, empty}};
...
%%
%% ```
%% > erlpm_tarball:unpack_docs(Tarball, memory).
%% {ok, [{"index.html", <<"<!doctype>">>}, ...]}
%%
%% > erlpm_tarball:unpack_docs(Tarball, "path/to/unpack").
%% ok
%% '''
...
format_error({tarball, Reason}) -> "tarball error, " ++ erlpm_erl_tar:format_error(Reason);
format_error({inner_tarball, Reason}) -> "inner tarball error, " ++ erlpm_erl_tar:format_error(Reason);
...
format_error({metadata, Reason}) -> "error reading package metadata" ++ erlpm_safe_erl_term:format_error(Reason);
...
do_decode_metadata(Binary) when is_binary(Binary) ->
    ...
    case erlpm_safe_erl_term:string(String) of
        {ok, Tokens, _Line} ->
            try
                Terms = erlpm_safe_erl_term:terms(Tokens),
...
unpack_tarball(ContentsBinary, memory) ->
    erlpm_erl_tar:extract({binary, ContentsBinary}, [memory, compressed]);
unpack_tarball(ContentsBinary, Output) ->
    filelib:ensure_dir(filename:join(Output, "*")),
    case erlpm_erl_tar:extract({binary, ContentsBinary}, [{cwd, Output}, compressed]) of
        ok ->
...
create_memory_tarball(Files) ->
    Path = tmp_path(),
    {ok, Tar} = erlpm_erl_tar:open(Path, [write]),

    try
        add_files(Tar, Files)
    after
        ok = erlpm_erl_tar:close(Tar)
    end,
...
add_file(Tar, {Filename, Contents}) when is_list(Filename) and is_binary(Contents) ->
    ok = erlpm_erl_tar:add(Tar, Contents, Filename, tar_opts());
...
add_file(Tar, {Filename, AbsFilename}) when is_list(Filename), is_list(AbsFilename) ->
    ...
    case FileInfo#file_info.type of
        symlink ->
            ok = erlpm_erl_tar:add(Tar, {Filename, AbsFilename}, tar_opts());
        directory ->
            case file:list_dir(AbsFilename) of
                {ok, []} ->
                    erlpm_erl_tar:add(Tar, {Filename, AbsFilename}, tar_opts());
        ...
        _ ->
            ...
            ok = erlpm_erl_tar:add(Tar, Contents, Filename, Mode, tar_opts())
    end.
