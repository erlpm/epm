...
init_per_group(basic_app, Config) ->
    ...
    Name = epm_test_utils:create_random_name("app1"),
...
init_per_group(checkout_apps, Config) ->
    ...
    Name1 = epm_test_utils:create_random_name("checkapp1_"),
    Vsn1 = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name1, Vsn1, [kernel, stdlib]),
...
build_basic_srcdirs(Config) ->
    ...
    EpmConfig = [{erl_opts, [{src_dirs, ["src", "extra"]}]}],
...
build_basic_extra_dirs(Config) ->
    ...
    EpmConfig = [{erl_opts, [{extra_src_dirs, ["extra"]}]}],
...
paths_checkout_deps(Config) ->
    ...
    %% epm_test_utils:init_epm_state/1,2 uses epm_state:new/3 which
    ...
    {ok, EpmConfig} = file:consult(filename:join([AppDir, "epm.config"])),

    {ok, State} = epm_test_utils:run_and_check(Config, EpmConfig, ["compile"], return),

    [AppName2] = epm_state:all_checkout_deps(State),
    Name2Bin = binary:list_to_bin(Name2),
    Name2Bin = epm_app_info:name(AppName2),
...
clean_basic_app(Config) ->
    ...
    epm_test_utils:run_and_check(Config, [], ["clean"], {ok, [{app, Name, invalid}]}).
...
recompile_when_opts_change(Config) ->
    ...
    epm_test_utils:create_config(AppDir, [{erl_opts, [{d, some_define}]}]),

    epm_test_utils:run_and_check(Config, [{erl_opts, [{d, some_define}]}], ["compile"], {ok, [{app, Name}]}),
...
recompile_when_dag_opts_change(Config) ->
    ...
    DepsDir = filename:join([AppDir, "_build", "default", "lib"]),
    G = epm_compiler_dag:init(DepsDir, epm_compiler_erl, "project_apps", []),
    ...
    %% the epm_compiler_erl module is annotated with a compiler version
    %% to help rebuild deps
    {ok, CompileVsn} = application:get_key(compiler, vsn),
    CritMeta = [{compiler, CompileVsn}],

    epm_compiler_dag:maybe_store(G, DepsDir, epm_compiler_erl, "project_apps", CritMeta),
    epm_compiler_dag:terminate(G),
    %% ... but don't change the actual epm config...
    epm_test_utils:run_and_check(Config, [], ["compile"], {ok, [{app, Name}]}),
...
dont_recompile_yrl_or_xrl(Config) ->
    ...
    EpmConfig = [{yrl_opts, [{includefile, "include/some_header.hrl"}]}],
...
parse_transform_test(Config) ->
    ...
    EpmConfig = [{erl_opts, [{parse_transform, pascal}]}],
...
erl_first_files_test(Config) ->
    ...
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),
    epm_test_utils:write_src_file(AppDir, "a.erl"),
    epm_test_utils:write_src_file(AppDir, "b.erl"),
    epm_test_utils:write_src_file(AppDir, "d.erl"),
...
mib_test(Config) ->
    ...
    EpmConfig = [{mib_first_files, ["mibs/SIMPLE-MIB.mib"]}],
...
"END\n">>,
    ...
    SuccessEpmConfig = [{mib_first_files, ["mibs/BEXPORTER-MIB.mib"]}],
    epm_test_utils:run_and_check(Config, SuccessEpmConfig, ["compile"], {ok, [{app, Name}]}),
...
only_default_transitive_deps(Config) ->
    ...
    GitDeps = epm_test_utils:expand_deps(git, [{"a", "1.0.0", []}]),
    PkgName = epm_test_utils:create_random_name("pkg1_"),
    ...
    Deps = epm_test_utils:top_level_deps(GitDeps),
    RConfFile = epm_test_utils:create_config(AppDir, [{deps, Deps}]),
    {ok, RConf} = file:consult(RConfFile),
...
git_subdir_deps(Config) ->
    Deps = epm_test_utils:expand_deps(git_subdir, [{"some_dep", "0.0.1", [{"other_dep", "0.0.1", []}]}]),
    TopDeps = epm_test_utils:top_level_deps(Deps),
...
override_del_pkg_deps(Config) ->
    Deps = epm_test_utils:expand_deps(pkg, [{"some_dep", "0.0.1", [{"other_dep", "0.0.1", []}]}]),
    TopDeps = epm_test_utils:top_level_deps(Deps),

    {_, PkgDeps} = epm_test_utils:flat_deps(Deps),
...
%% test for fix of https://github.com/erlpm/epm/issues/1801
...
only_deps(Config) ->
    ...
    RConfFile = epm_test_utils:create_config(AppDir, [{deps, TopDeps}]),
    {ok, RConf} = file:consult(RConfFile),
    epm_test_utils:run_and_check(
...
deps_build_in_prod(Config) ->
    ...
    GitDeps = epm_test_utils:expand_deps(git, [{"asdf", "1.0.0", []}]),
    PkgName = epm_test_utils:create_random_name("pkg1_"),
    {SrcDeps, _} = epm_test_utils:flat_deps(GitDeps),
    mock_git_resource:mock([{deps, SrcDeps},
...
include_file_in_src_test_multiapp(Config) ->
    ...
    Name2 = epm_test_utils:create_random_name("app1_"),
    AppDir1 = filename:join([?config(apps, Config), "lib", Name1]),
    AppDir2 = filename:join([?config(apps, Config), "lib", Name2]),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir1, Name1, Vsn, [kernel, stdlib, list_to_atom(Name2)]),
...
dont_recompile_when_erl_compiler_options_env_does_not_change(Config) ->
    ...
    Name = epm_test_utils:create_random_name("erl_compiler_options_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),
...
epm_config_os_var(Config) ->
    ...
    Name = epm_test_utils:create_random_name("epm_config_os_var_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),

    epm_test_utils:create_config(AppDir, [{erl_opts, []}]),

    AltConfig = filename:join(AppDir, "test.epm.config"),
    file:write_file(AltConfig, "{erl_opts, [compressed]}."),
    true = os:putenv("EPM_CONFIG", AltConfig),
...
recompile_when_parse_transform_inline_changes(Config) ->
    ...
    Name = epm_test_utils:create_random_name("parse_transform_inline_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),
...
recompile_when_parse_transform_as_opt_changes(Config) ->
    ...
    Name = epm_test_utils:create_random_name("parse_transform_opt_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),
...
extra_recursion(Config) ->
    ...
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),
    epm_test_utils:write_src_file(filename:join([AppDir, "src", "src2"]), "rec.erl"),
    epm_test_utils:write_src_file(filename:join([AppDir, "test", "test2"]), "rectest.erl"),
...
app_file_linting(Config) ->
    ...
    AppDir = ?config(apps, Config),
    Name = epm_test_utils:create_random_name("app_file_linting"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [foo]),

    _ = epm_test_utils:run_and_check(Config, [], ["compile"], return),
    History = meck:history(epm_log),
    Warnings = [{Str, Args} || {_, {epm_log, log, [warn, Str, Args]}, _} <- History],
