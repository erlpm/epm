...
init_per_suite(Config) ->
    epm_compiler_erl:module_info(), % ensure it is loaded
    Config.
...
init_per_group(with_project, Config) ->
    NewConfig = epm_test_utils:init_epm_state(Config, "apps"),
    ...
    Name3 = epm_test_utils:create_random_name("app3_"),
    Vsn3 = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(filename:join([AppDir,"apps",Name3]), Name3, Vsn3, [kernel, stdlib]),
...
exists(Config) ->
    ...
    G = epm_compiler_dag:init(Priv, compilermod, "label", [crit_meta]),
    epm_compiler_dag:store_artifact(G, "somefile", "someartifact", [written]),
    epm_compiler_dag:maybe_store(G, Priv, compilermod, "label", [crit_meta]),
    epm_compiler_dag:terminate(G),

    ?assertEqual(valid,     epm_compiler_dag:status(Priv, compilermod, "label", [crit_meta])),
    ?assertEqual(not_found, epm_compiler_dag:status(Priv, compilermad, "label", [crit_meta])),
    ?assertEqual(not_found, epm_compiler_dag:status(Priv, compilermod, "lobel", [crit_meta])),
    ?assertEqual(bad_meta,  epm_compiler_dag:status(Priv, compilermod, "label", [crit_zeta])),
    ok.
...
app_sort(Config) ->
    ...
    ?assertEqual([lists:nth(2, AppNames),
                 ...
                 epm_compiler_dag:compile_order(G, AppPaths, ".erl", ".beam")),
    ok.
...
analyze_apps(G, AppNames, AppDir) ->
    ...
    epm_compiler_dag:populate_deps(G, ".erl", [{".beam", "ebin/"}]),
