...
init_per_testcase(_, Config) ->
    epm_test_utils:init_epm_state(Config, "cover_").
...
flag_coverdata_written(Config) ->
    ...
    Name = epm_test_utils:create_random_name("cover_"),
...
config_coverdata_written(Config) ->
    ...
    EpmConfig = [{erl_opts, [{d, some_define}]}, {cover_enabled, true}],
    epm_test_utils:run_and_check(Config,
                                   EpmConfig,
                                   ["eunit"],
                                   {ok, [{app, Name}]}),
...
basic_extra_src_dirs(Config) ->
    ...
    Name = epm_test_utils:create_random_name("cover_extra_"),
    Vsn = epm_test_utils:create_random_vsn(),
    ...
    EpmConfig = [{erl_opts, [{d, some_define}]}, {extra_src_dirs, ["extra"]}],
    epm_test_utils:run_and_check(Config,
                                   EpmConfig,
                                   ["eunit", "--cover"],
                                   {ok, [{app, Name}]}),
...
config_verbose(Config) ->
    ...
    EpmConfig = [{erl_opts, [{d, some_define}]}, {cover_opts, [verbose]}],
    epm_test_utils:run_and_check(Config,
                                   EpmConfig,
                                   ["do", "eunit", "--cover", ",", "cover"],
                                   {ok, [{app, Name}]}),
...
excl_mods_and_apps(Config) ->
    ...
    Name3 = epm_test_utils:create_random_name("excludeme_"),
...
coverdata_is_reset_on_write(Config) ->
    ...
    Name = epm_test_utils:create_random_name("coverdata_is_reset_on_write_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_eunit_app(AppDir, Name, Vsn, [kernel, stdlib]),
...
flag_min_coverage(Config) ->
    ...
    Name = epm_test_utils:create_random_name("min_cover_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_eunit_app(AppDir, Name, Vsn, [kernel, stdlib]),
    ...
    ?assertMatch({error,{epm_prv_cover,{min_coverage_failed,{65,_}}}},
                 epm_test_utils:run_and_check(
                   Config, EpmConfig,
...
config_min_coverage(Config) ->
    ...
    EpmConfig1 = [{erl_opts, [{d, some_define}]}, {cover_opts, [{min_coverage,5}]}],
    ?assertMatch({ok, _},
                 epm_test_utils:run_and_check(
                   Config, EpmConfig1,
                   ["do", "eunit", "--cover", ",", "cover"],
    ...
    EpmConfig2 = [{erl_opts, [{d, some_define}]}, {cover_opts, [{min_coverage,65}]}],
    ?assertMatch({error,{epm_prv_cover,{min_coverage_failed,{65,_}}}},
                 epm_test_utils:run_and_check(
                   Config, EpmConfig2,
                   ["do", "eunit", "--cover", ",", "cover"],
