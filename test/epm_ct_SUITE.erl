...
init_per_group(basic_app, Config) ->
    C = epm_test_utils:init_epm_state(Config, "ct_"),

    AppDir = ?config(apps, C),

    Name = epm_test_utils:create_random_name(atom_to_list(basic_app) ++ "_"),
    ...
    {ok, State} = epm_test_utils:run_and_check(C, [], ["as", "test", "lock"], return),

    Tests = epm_prv_common_test:prepare_tests(State),
    {ok, NewState} = epm_prv_common_test:compile(State, Tests),
    {ok, T} = Tests,
    Opts = epm_prv_common_test:translate_paths(NewState, T),
...
init_per_group(dirs_and_suites, Config) ->
    ...
    Name1 = epm_test_utils:create_random_name(atom_to_list(dirs_and_suites) ++ "_"),
    Vsn1 = epm_test_utils:create_random_vsn(),
    AppDir1 = filename:join([AppDir, "apps", Name1]),
    epm_test_utils:create_app(AppDir1, Name1, Vsn1, [kernel, stdlib]),
    ...
    Name2 = epm_test_utils:create_random_name(atom_to_list(dir_and_suites) ++ "_"),
    Vsn2 = epm_test_utils:create_random_vsn(),
    AppDir2 = filename:join([AppDir, "apps", Name2]),
    epm_test_utils:create_app(AppDir2, Name2, Vsn2, [kernel, stdlib]),
...
init_per_group(ct_opts, Config) ->
    C = epm_test_utils:init_epm_state(Config, "ct_opts"),

    AppDir = ?config(apps, C),

    Name = epm_test_utils:create_random_name("ct_opts_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),
...
multi_app_ct_macro(Config) ->
    ...
    Apps = epm_state:project_apps(State),
...
no_ct_suite(Config0) ->
    ...
    Name = epm_test_utils:create_random_name("no_ct_suite_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),

    {ok, State} = epm_test_utils:run_and_check(Config, [], ["as", "test", "lock"], return),

    {ok, Opts} = epm_prv_common_test:prepare_tests(State),
...
single_app_dir(Config) ->
    ...
    LibDirs = epm_dir:lib_dirs(State),
    State1 = epm_app_discover:do(State, LibDirs),

    Providers = epm_state:providers(State1),
    Namespace = epm_state:namespace(State1),
    ...
    State2 = epm_state:command_parsed_args(State1, GetOptResult),

    Tests = epm_prv_common_test:prepare_tests(State2),
    {ok, NewState} = epm_prv_common_test:compile(State2, Tests),
    {ok, T} = Tests,
    Opts = epm_prv_common_test:translate_paths(NewState, T),
...
suite_at_root(Config) ->
    ...
    epm_file_utils:rm_rf("_build"),

    {ok, GetOptResult2} = getopt:parse(GetOptSpec, ["--suite=" ++ "root_SUITE"]),

    State3 = epm_state:command_parsed_args(State1, GetOptResult2),

    Tests2 = epm_prv_common_test:prepare_tests(State3),
    {ok, NewState2} = epm_prv_common_test:compile(State3, Tests2),
    {ok, T2} = Tests2,
    Opts2 = epm_prv_common_test:translate_paths(NewState2, T2),
...
%% this test probably only fails when this suite is run via epm with the --cover flag
data_dir_correct(Config) ->
    DataDir = ?config(data_dir, Config),
    Parts = filename:split(DataDir),
    ["epm_ct_SUITE_data","test","epm","lib",_,"_build"|_] = lists:reverse(Parts).
...
cmd_label(Config) ->
    ...
    NewState = epm_state:command_parsed_args(State, GetOptResult),

    {ok, TestOpts} = epm_prv_common_test:prepare_tests(NewState),
...
cmd_include_dir(Config) ->
    ...
    Tests = epm_prv_common_test:prepare_tests(NewState),
    {ok, _} = epm_prv_common_test:compile(NewState, Tests),
...
cmd_sys_config(Config) ->
    ...
    {ok, _} = epm_prv_common_test:prepare_tests(State),
    ...
    EpmConfig = [{ct_opts, [{sys_config, CfgFile}]}],
    {ok, State1} = epm_test_utils:run_and_check(Config, EpmConfig, ["as", "test", "lock"], return),

    {ok, _} = epm_prv_common_test:prepare_tests(State1),
    ...
    {ok, _} = epm_prv_common_test:prepare_tests(State2),

    ?assertEqual({ok ,cmd_value}, application:get_env(AppName, key)).


cfg_opts(Config) ->
    C = epm_test_utils:init_epm_state(Config, "ct_cfg_opts_"),

    AppDir = ?config(apps, C),

    Name = epm_test_utils:create_random_name("ct_cfg_opts_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),

    EpmConfig = [{ct_opts, [{label, "this_is_a_label"}, {decrypt_file, "../keyfile.pem"}]}],

    {ok, State} = epm_test_utils:run_and_check(C, EpmConfig, ["as", "test", "lock"], return),

    {ok, TestOpts} = epm_prv_common_test:prepare_tests(State),
...
cfg_arbitrary_opts(Config) ->
    C = epm_test_utils:init_epm_state(Config, "ct_cfg_arbitrary_opts_"),

    AppDir = ?config(apps, C),

    Name = epm_test_utils:create_random_name("ct_cfg_arbitrary_opts_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),

    EpmConfig = [{ct_opts, [{foo, 1}, {bar, 2}, {baz, 3}]}],
...
cfg_cover_spec(Config) ->
    C = epm_test_utils:init_epm_state(Config, "ct_cfg_cover_spec_opts_"),

    AppDir = ?config(apps, C),

    Name = epm_test_utils:create_random_name("ct_cfg_cover_spec_opts_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),

    EpmConfig = [{ct_opts, [Opt = {cover, "spec/foo.spec"}]}],
...
cfg_atom_suites(Config) ->
    C = epm_test_utils:init_epm_state(Config, "ct_cfg_atom_suites_"),

    AppDir = ?config(apps, C),

    Name = epm_test_utils:create_random_name("ct_cfg_atom_suites_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),

    EpmConfig = [{ct_opts, [{suite, [foo, bar, baz]}]}],
...
cover_export_name(Config) ->
    ...
    Tests = epm_prv_common_test:prepare_tests(NewState),
    {ok, _} = epm_prv_common_test:compile(NewState, Tests),
    epm_prv_common_test:maybe_write_coverdata(NewState),

    Name = ?config(name, Config),
    Mod = list_to_atom(Name),
    {file, _} = cover:is_compiled(Mod),

    Dir = epm_dir:profile_dir(epm_state:opts(NewState), [default, test]),
    ct:pal("DIR ~s", [Dir]),
...
misspecified_ct_opts(Config) ->
    ...
    EpmConfig = [{ct_opts, {basic_html, false}}],

    {ok, State} = epm_test_utils:run_and_check(C, EpmConfig, ["as", "test", "lock"], return),

    {error, {epm_prv_common_test, Error}} = epm_prv_common_test:prepare_tests(State),
...
misspecified_ct_compile_opts(Config) ->
    ...
    EpmConfig = [{ct_compile_opts, {d, whatever}}],

    {ok, State} = epm_test_utils:run_and_check(C, EpmConfig, ["as", "test", "lock"], return),

    Tests = epm_prv_common_test:prepare_tests(State),
    {error, {epm_prv_common_test, Error}} = epm_prv_common_test:compile(State, Tests),
...
misspecified_ct_first_files(Config) ->
    ...
    EpmConfig = [{ct_first_files, some_file}],
...
testspec(Config) ->
    C = epm_test_utils:init_epm_state(Config, "ct_testspec_"),

    AppDir = ?config(apps, C),

    Name = epm_test_utils:create_random_name("ct_testspec_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),
    ...
    {ok, State} = epm_test_utils:run_and_check(C,
                                                 [],
                                                 ["as", "test", "lock"],
                                                 return),
    ...
    State1 = epm_state:command_parsed_args(State, GetOptResult1),
    Tests1 = epm_prv_common_test:prepare_tests(State1),
    {ok, NewState1} = epm_prv_common_test:compile(State1, Tests1),
    {ok, T1} = Tests1,
    Opts1= epm_prv_common_test:translate_paths(NewState1, T1),

    %% check that extra src dir is added
    [App1] = epm_state:project_apps(NewState1),
    ["test"] = epm_dir:extra_src_dirs(epm_app_info:opts(App1)),
    ...
    State2 = epm_state:command_parsed_args(State, GetOptResult2),
    Tests2 = {ok, T2} = epm_prv_common_test:prepare_tests(State2),
    {ok, NewState2} = epm_prv_common_test:compile(State2, Tests2),
    Opts2= epm_prv_common_test:translate_paths(NewState2, T2),

    %% check that extra src dirs are added
    [App2] = epm_state:project_apps(NewState2),
    ["specs","suites","test"] =
        lists:sort(epm_dir:extra_src_dirs(epm_app_info:opts(App2))),
...
testspec_at_root(Config) ->
    C = epm_test_utils:init_epm_state(Config, "ct_testspec_at_root_"),

    AppDir = ?config(apps, C),

    Name = epm_test_utils:create_random_name("ct_testspec_at_root_"),
    Vsn = epm_test_utils:create_random_vsn(),
    AppDir1 = filename:join([AppDir, "apps", Name]),
    epm_test_utils:create_app(AppDir1, Name, Vsn, [kernel, stdlib]),
    ...
    SpecArg1 = epm_string:join([Spec1,Spec2,Spec3],","),
    ...
    [ExtraDir,"suites","test"] =
        epm_dir:extra_src_dirs(epm_state:opts(NewState1)),
    ...
    %% check that extra src dir is added
    [ExtraDir,"suites","test"] =
        epm_dir:extra_src_dirs(epm_state:opts(NewState2)),
...
testspec_parse_error(Config) ->
    C = epm_test_utils:init_epm_state(Config, "ct_testspec_error"),

    AppDir = ?config(apps, C),

    Name = epm_test_utils:create_random_name("ct_testspec_error"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),
     ...
     {epm_prv_common_test,
      {error_reading_testspec,
       {Spec1,"no such file or directory"}}}} =
        epm_prv_common_test:compile(State1, Tests1),
     ...
     {epm_prv_common_test,
      {error_reading_testspec,
       {Spec2,"1: syntax error before: '.'"}}}} =
        epm_prv_common_test:compile(State2, Tests2),

    ok.

cmd_vs_cfg_opts(Config) ->
    C = epm_test_utils:init_epm_state(Config, "ct_cmd_vs_cfg_opts_"),

    AppDir = ?config(apps, C),

    Name = epm_test_utils:create_random_name("ct_cmd_vs_cfg_opts_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),

    EpmConfig = [{ct_opts, [{spec,"mytest.spec"},
                              {dir,"test"},
                              {suite,"some_SUITE"},
                              {group,"some_group"},
                              {testcase,"some_test"}]}],
    ...
    {ok, GetOptResult1} = getopt:parse(GetOptSpec, ["--spec","test/some.spec"]),
    State1 = epm_state:command_parsed_args(State, GetOptResult1),
    {ok, TestOpts1} = epm_prv_common_test:prepare_tests(State1),
    ...
    State2 = epm_state:command_parsed_args(State, GetOptResult2),
    {ok, TestOpts2} = epm_prv_common_test:prepare_tests(State2),
    ...
    State3 = epm_state:command_parsed_args(State, GetOptResult3),
    {ok, TestOpts3} = epm_prv_common_test:prepare_tests(State3),
...
single_testspec_in_ct_opts(Config) ->
    ...
    EpmConfig = [{ct_opts, [{spec,"test/some.spec"}]}],
    ...
    {ok, _NewState} = epm_prv_common_test:compile(State1, Tests1),

    ok = file:set_cwd(Wd),
    ok.

compile_only(Config) ->
    C = epm_test_utils:init_epm_state(Config, "compile_only_"),
    ...
    {ok, _State} = epm_test_utils:run_and_check(C, [], ["ct", "--compile_only"], {ok, [{app,Name}], "test"}).
