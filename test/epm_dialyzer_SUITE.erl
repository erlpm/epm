...
empty_base_plt(Config) ->
    ...
    EpmConfig = ?config(epm_config, Config),
    ...
    epm_test_utils:create_app(AppDir, Name, Vsn, [erts]),

    epm_test_utils:run_and_check(Config, EpmConfig, ["dialyzer"],
                                   {ok, [{app, Name}]}),
...
empty_app_plt(Config) ->
    ...
    Name = epm_test_utils:create_random_name("app1_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, []),
...
empty_app_succ_typings(Config) ->
    ...
    Name = epm_test_utils:create_random_name("app1_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_empty_app(AppDir, Name, Vsn, []),
...
build_release_plt(Config) ->
    ...
    epm_test_utils:create_app(filename:join([AppDir,"apps",Name1]), Name1, Vsn1,
                                [erts]),
    Name2 = epm_test_utils:create_random_name("relapp2_"),
    Vsn2 = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(filename:join([AppDir,"apps",Name2]), Name2, Vsn2,
...
plt_apps_option(Config) ->
    ...
    Vsn1 = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(filename:join([AppDir,"deps",Name1]), Name1, Vsn1,
                                []),
    App1 = ec_cnv:to_atom(Name1),

    Name2 = epm_test_utils:create_random_name("app2_"),
    Vsn2 = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(filename:join([AppDir,"deps",Name2]), Name2, Vsn2,
                                [App1]), % App2 depends on App1
    App2 = ec_cnv:to_atom(Name2),

    Name3 = epm_test_utils:create_random_name("app3_"), % the project application
    Vsn3 = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name3, Vsn3,
                                [App2]), % App3 depends on App2

    %% Dependencies settings
    State1 = epm_state:add_resource(State, {localfs, epm_localfs_resource}),
    Config1 = [{state, State1} | Config],
    EpmConfig1 = merge_config(
                     [{deps,
                       [
    ...
    epm_test_utils:run_and_check(Config1, EpmConfig1, ["dialyzer"],
                                   {ok, [{app, Name3}]}),
    ...
    EpmConfig2 = merge_config([{dialyzer, [{plt_apps, all_deps}]}],
                                EpmConfig1),
    epm_test_utils:run_and_check(Config1, EpmConfig2, ["dialyzer"],
                                   {ok, [{app, Name3}]}),
...
exclude_and_extra(Config) ->
    ...
    {value, {dialyzer, Opts}, Rest} = lists:keytake(dialyzer, 1, EpmConfig),
    % Remove erts => []
    ...
    Opts2 = [{exclude_apps, [erts]},
             ...
             Opts],
    EpmConfig2 = [{dialyzer, Opts2} | Rest],

    Name = epm_test_utils:create_random_name("app1_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [erts]),

    epm_test_utils:run_and_check(Config, EpmConfig2, ["dialyzer"],
                                   {ok, [{app, Name}]}),
...
cli_args(Config) ->
    AppDir = ?config(apps, Config),
    [{dialyzer, Opts}] = ?config(epm_config, Config),
    BasePlt = ?config(base_plt, Config),
    Plt = ?config(plt, Config),
...
single_app_succ_typing(Config) ->
    ...
    epm_test_utils:create_eunit_app(AppDir, Name, Vsn, []),
    ...
    EpmConfig2 = merge_config([{dialyzer, [{plt_apps, all_apps}]}],
        EpmConfig),
    {ok, _} =
        epm:run(epm_state:new(State, EpmConfig2, AppDir), ["dialyzer", "--succ-typings=false"]),
    ...
    % there are few warnings for generated test (see epm_test_utils:erl_eunit_suite_file/1)
    {error, {epm_prv_dialyzer, {dialyzer_warnings, _}}} =
        epm:run(epm_state:new(State, EpmConfig, AppDir), Command0),

    %% warnings from App
    Command1 = ["as", "test", "dialyzer", "--app=" ++ Name],
    % there are few warnings for generated test (see epm_test_utils:erl_eunit_suite_file/1)
    {error, {epm_prv_dialyzer, {dialyzer_warnings, _}}} =
        epm:run(epm_state:new(State, EpmConfig, AppDir), Command1),

    %% no warnings from App2
    Command2 = ["as", "test", "dialyzer", "--app=" ++ Name2],
    {ok, _} =
        epm:run(epm_state:new(State, EpmConfig, AppDir), Command2).
...
extra_src_dirs(Config) ->
    ...
    % there are few warnings for generated test (see epm_test_utils:erl_eunit_suite_file/1)
    {error, {epm_prv_dialyzer, {dialyzer_warnings, _}}} =
        epm:run(epm_state:new(State, EpmConfig, AppDir), Command).
...
merge_config(NewConfig, OldConfig) ->
    dict:to_list(
      epm_opts:merge_opts(dict:from_list(NewConfig),
...
get_apps_from_beam_files(BeamFiles) ->
    lists:usort(
      [begin
           ...
           [AppName | _] = epm_string:lexemes(AppNameVsn ++ "-", "-"),
