...
disable_app(Config) ->
    ...
    EpmConfig = [{excluded_apps, [list_to_atom(Name1)]}],
    %EpmConfig = [],
...
create_random_app(AppDir, Prefix) ->
    Name = epm_test_utils:create_random_name(Prefix),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_empty_app(filename:join([AppDir, "apps", Name]), Name, Vsn, [kernel, stdlib]),

    ModName = epm_test_utils:create_random_name("mod1_"),
