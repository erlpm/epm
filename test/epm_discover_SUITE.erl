...
all() ->
    [empty_app_src, bad_app_src, invalid_app_src, overwrite_extension].
     ...
     %% has epm just ignoring the directory as not OTP-related.


init_per_testcase(_, Config) ->
    NewConfig = epm_test_utils:init_epm_state(Config, "discover_app_"),
...
empty_app_src(Config) ->
    ...
    ?assertEqual(
       {error, {epm_app_discover, {cannot_read_app_file, AppSrc}}},
       epm_test_utils:run_and_check(Config, [], ["compile"], return)
    ),
    ok.
...
bad_app_src(Config) ->
    ...
    ?assertMatch(
       {error, {epm_app_discover, {bad_term_file, AppSrc, _}}},
       epm_test_utils:run_and_check(Config, [], ["compile"], return)
    ),
    ok.
...
overwrite_extension(Config) ->
    ...
    [App] = epm_state:project_apps(AfterCompileState),
    ?assertEqual("0.42.0", epm_app_info:vsn(App)),
    %% reverse order now, check that both are reported as conflicting
    {error, {epm_prv_app_discovery, {multiple_app_files, Conflicting}}} =
        epm_test_utils:run_and_check(
            Config,
