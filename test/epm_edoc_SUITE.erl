...
init_per_testcase(multiapp, Config) ->
    ...
    Name = epm_test_utils:create_random_name("multiapp"),
    AppsDir = filename:join([PrivDir, epm_test_utils:create_random_name(Name)]),
...
init_per_testcase(multiapp_macros, Config) ->
    ...
    DataDir = ?config(data_dir, Config),
    PrivDir = ?config(priv_dir, Config),
    Name = epm_test_utils:create_random_name("multiapp_macros"),
    AppsDir = filename:join([PrivDir, epm_test_utils:create_random_name(Name)]),
    ec_file:copy(filename:join([DataDir, "foo"]), AppsDir, [recursive]),
...
init_per_testcase(error_survival, Config) ->
    ...
    DataDir = ?config(data_dir, Config),
    PrivDir = ?config(priv_dir, Config),
    Name = epm_test_utils:create_random_name("error_survival"),
    AppsDir = filename:join([PrivDir, epm_test_utils:create_random_name(Name)]),
...
multiapp(Config) ->
    ...
    EpmConfig = [],
    epm_test_utils:run_and_check(Config, EpmConfig, ["edoc"], {ok, []}),
    ...
    %% Options such from epm.config in the app themselves are
    %% respected
    ?assert(file_content_matches(
...
multiapp_macros(Config) ->
    ...
    AppsDir = ?config(apps, Config),
    ct:pal("AppsDir: ~s", [AppsDir]),
    ok = file:write_file(filename:join([AppsDir, "apps", "bar1", "epm.config"]),
...
error_survival(Config) ->
    ...
    epm_test_utils:run_and_check(
        Config, EpmConfig, ["edoc"],
        {error,{epm_prv_edoc,{app_failed,"bad_bar2"}}}
    ),
    ok.
