-module(epm_eunit_SUITE).

-export([all/0, groups/0
    ,init_per_suite/1, end_per_suite/1
    ,init_per_group/2, end_per_group/2
    ,basic_app_compiles/1, basic_app_files/1
    ,basic_app_exports/1, basic_app_testset/1
    ,basic_app_eunit_macro/1
    ,multi_app_compiles/1, multi_app_files/1
    ,multi_app_exports/1, multi_app_testset/1
    ,multi_app_eunit_macro/1
    ,eunit_tests/1, eunit_opts/1, eunit_first_files/1
    ,single_application_arg/1, multi_application_arg/1, missing_application_arg/1
    ,single_module_arg/1, multi_module_arg/1, missing_module_arg/1
    ,single_suite_arg/1, multi_suite_arg/1, missing_suite_arg/1
    ,single_generator_arg/1, multi_generator_arg/1, missing_generator_arg/1
    ,single_file_arg/1, multi_file_arg/1, missing_file_arg/1
    ,single_dir_arg/1, multi_dir_arg/1, missing_dir_arg/1
    ,multiple_arg_composition/1, multiple_arg_errors/1
    ,misspecified_eunit_tests/1
    ,misspecified_eunit_compile_opts/1
    ,misspecified_eunit_first_files/1
    ,alternate_test_regex/1
    ,syscfg_app_opts/1]).
...
init_per_group(basic_app, Config) ->
    GroupState = epm_test_utils:init_epm_state(Config, "basic_app_"),
    ...
    EpmConfig = [{erl_opts, [{d, config_define}]}, {eunit_compile_opts, [{d, eunit_compile_define}]}],

    {ok, State} = epm_test_utils:run_and_check(GroupState, EpmConfig, ["as", "test", "lock"], return),

    [{result, State}|GroupState];
init_per_group(multi_app, Config) ->
    GroupState = epm_test_utils:init_epm_state(Config, "multi_app_"),
...
init_per_group(cmd_line_args, Config) ->
    GroupState = epm_test_utils:init_epm_state(Config, "cmd_line_args_"),
    ...
    EpmConfig = [{erl_opts, [{d, config_define}]},
    ...
    {ok, State} = epm_test_utils:run_and_check(GroupState, EpmConfig, ["eunit"], return),
...
basic_app_compiles(Config) ->
    ...
    {ok, _} = epm_prv_eunit:do(State),

    epm_test_utils:check_results(AppDir, [{app, "basic_app"}], "*").
...
basic_app_testset(Config) ->
    ...
    Set = epm_prv_eunit:prepare_tests(Result).
...
multi_app_compiles(Config) ->
    ...
    epm_test_utils:check_results(AppDir, [{app, "multi_app_bar"}, {app, "multi_app_baz"}], "*").
...
%% no explicit test for cmd line args taking precedence over the epm.config since
%% almost every single test implies it

%% check tests in the epm.config are run if no cmd line opts are specified
eunit_tests(Config) ->
    State = ?config(result, Config),

    Expect = {ok, [{test, multi_app_bar, sanity_test}, {test, multi_app_baz, sanity_test}]},
    Expect = epm_prv_eunit:prepare_tests(State).

%% check eunit_opts from the epm.config are respected
eunit_opts(Config) ->
    State = ?config(result, Config),

    Apps = epm_state:project_apps(State),
    lists:foreach(fun(App) -> [verbose] = epm_app_info:get(App, eunit_opts) end,
                  Apps).

%% check eunit_first_files from the epm.config are respected
eunit_first_files(Config) ->
    ...
    Apps = epm_state:project_apps(State),
    lists:foreach(fun(App) -> FirstFiles = epm_app_info:get(App, eunit_first_files) end,
                  Apps).
...
single_application_arg(Config) ->
    ...
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--application=multi_app_bar"]),
    State = epm_state:command_parsed_args(S, Args),

    {ok, [{application, multi_app_bar}]} = epm_prv_eunit:prepare_tests(State).

multi_application_arg(Config) ->
    S = ?config(result, Config),
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--application=multi_app_bar,multi_app_baz"]),
    State = epm_state:command_parsed_args(S, Args),

    {ok, [{application, multi_app_bar}, {application, multi_app_baz}]} = epm_prv_eunit:prepare_tests(State).

%% check that an invalid --application cmd line opt generates an error
missing_application_arg(Config) ->
    S = ?config(result, Config),
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--application=missing_app"]),
    State = epm_state:command_parsed_args(S, Args),

    Error = {error, {epm_prv_eunit, {eunit_test_errors, ["Application `missing_app' not found in project."]}}},
    Error = epm_prv_eunit:validate_tests(State, epm_prv_eunit:prepare_tests(State)).
...
single_module_arg(Config) ->
    ...
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--module=multi_app_bar"]),
    State = epm_state:command_parsed_args(S, Args),

    {ok, [{module, multi_app_bar}]} = epm_prv_eunit:prepare_tests(State),
...
multi_module_arg(Config) ->
    ...
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--module=multi_app_bar,multi_app_baz"]),
    State = epm_state:command_parsed_args(S, Args),

    {ok, [{module, multi_app_bar}, {module, multi_app_baz}]} = epm_prv_eunit:prepare_tests(State),
...
missing_module_arg(Config) ->
    ...
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--module=missing_app"]),
    State = epm_state:command_parsed_args(S, Args),

    T = epm_prv_eunit:prepare_tests(State),
    Tests = epm_prv_eunit:validate_tests(S, T),

    Error = {error, {epm_prv_eunit, {eunit_test_errors, ["Module `missing_app' not found in project."]}}},
    Error = Tests.
...
single_suite_arg(Config) ->
    ...
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--suite=multi_app_bar"]),
    State = epm_state:command_parsed_args(S, Args),
...
multi_suite_arg(Config) ->
    ...
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--suite=multi_app_bar,multi_app_baz"]),
    State = epm_state:command_parsed_args(S, Args),
...
missing_suite_arg(Config) ->
    ...
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--suite=missing_app"]),
    State = epm_state:command_parsed_args(S, Args),
...
single_generator_arg(Config) ->
    ...
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--generator=module_name:function_name"]),
    State = epm_state:command_parsed_args(S, Args),

    {ok, [{generator, module_name, function_name}]} = epm_prv_eunit:prepare_tests(State).

multi_generator_arg(Config) ->
    S = ?config(result, Config),

    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--generator=module1:func1;func2,module2:func1;func2"]),
    State = epm_state:command_parsed_args(S, Args),
    ...
    {ok, Generators} = epm_prv_eunit:prepare_tests(State).

%% check that an invalid --suite cmd line opt generates an error
missing_generator_arg(Config) ->
    S = ?config(result, Config),

    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--generator=missing_module:func1"]),
    State = epm_state:command_parsed_args(S, Args),

    Error = {error, {epm_prv_eunit, {eunit_test_errors, ["Module `missing_module' not found in project."]}}},
    Error = epm_prv_eunit:validate_tests(State, epm_prv_eunit:prepare_tests(State)).
...
single_file_arg(Config) ->
    ...
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--file=" ++ Path]),
    State = epm_state:command_parsed_args(S, Args),

    {ok, [{file, Path}]} = epm_prv_eunit:prepare_tests(State).
...
multi_file_arg(Config) ->
    ...
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--file=" ++ BarPath ++ "," ++ BazPath]),
    State = epm_state:command_parsed_args(S, Args),

    {ok, [{file, BarPath}, {file, BazPath}]} = epm_prv_eunit:prepare_tests(State).
...
missing_file_arg(Config) ->
    ...
    Error = {error, {epm_prv_eunit, {eunit_test_errors, ["File `" ++ Path ++"' not found."]}}},
    Error = epm_prv_eunit:validate_tests(State, epm_prv_eunit:prepare_tests(State)).
...
single_dir_arg(Config) ->
    ...
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--dir=" ++ Path]),
    State = epm_state:command_parsed_args(S, Args),

    {ok, [{dir, Path}]} = epm_prv_eunit:prepare_tests(State).
...
multi_dir_arg(Config) ->
    ...
    {ok, Args} = getopt:parse(epm_prv_eunit:eunit_opts(S), ["--dir=" ++ BarPath ++ "," ++ BazPath]),
    State = epm_state:command_parsed_args(S, Args),

    {ok, [{dir, BarPath}, {dir, BazPath}]} = epm_prv_eunit:prepare_tests(State).
...
missing_dir_arg(Config) ->
    ...
    Error = {error, {epm_prv_eunit, {eunit_test_errors, ["Directory `" ++ Path ++"' not found."]}}},
    Error = epm_prv_eunit:validate_tests(State, epm_prv_eunit:prepare_tests(State)).
...
multiple_arg_composition(Config) ->
    ...
    {ok, Expect} = epm_prv_eunit:prepare_tests(State),
...
multiple_arg_errors(Config) ->
    ...
    {error, {epm_prv_eunit, {eunit_test_errors, Expect}}} = Tests.

misspecified_eunit_tests(Config) ->
    State = epm_test_utils:init_epm_state(Config, "basic_app_"),
    ...
    EpmConfig = [{eunit_tests, {dir, "test"}}|BaseConfig],

    {error, {epm_prv_eunit, Error}} = epm_test_utils:run_and_check(State, EpmConfig, ["eunit"], return),
...
misspecified_eunit_first_files(Config) ->
    ...
    EpmConfig = [{eunit_first_files, some_file}|BaseConfig],
...
alternate_test_regex(Config) ->
    State = epm_test_utils:init_epm_state(Config, "alternate_test_regex_"),
    ...
    EpmConfig = [{eunit_test_regex, "basic_app_tests.erl"}|BaseConfig],

    {ok, S} = epm_test_utils:run_and_check(State, EpmConfig, ["as", "test", "lock"], return),

    Set = {ok, [{application, basic_app},
                {module, basic_app_tests}]},
    Set = epm_prv_eunit:prepare_tests(S).

%% check that sys_config files go through
syscfg_app_opts(Config) ->
    State = epm_test_utils:init_epm_state(Config, "syscfg_"),
