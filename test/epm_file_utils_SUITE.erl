...
init_per_group(reset_dir, Config) ->
    TmpDir = epm_file_utils:system_tmpdir(["epm_file_utils_SUITE", "resetable"]),
...
raw_tmpdir(_Config) ->
    case epm_file_utils:system_tmpdir() of
        ?TMPDIR -> ok;
        "./tmp" -> ok
    end.
...
simple_tmpdir(_Config) ->
    case epm_file_utils:system_tmpdir(["test"]) of
        ?TMPDIR ++ "/test" -> ok;
        "./tmp/test"       -> ok
    end.

multi_tmpdir(_Config) ->
    case epm_file_utils:system_tmpdir(["a", "b", "c"]) of
        ?TMPDIR ++ "/a/b/c" -> ok;
        "./tmp/a/b/c"       -> ok
    end.
...
reset_nonexistent_dir(Config) ->
    ...
    ok = epm_file_utils:reset_dir(TmpDir),
    ?assert(filelib:is_dir(TmpDir)),
    {ok, []} = epm_utils:list_dir(TmpDir).
...
path_from_ancestor(_Config) ->
    ?assertEqual({ok, "foo/bar/baz"}, epm_file_utils:path_from_ancestor("/foo/bar/baz", "/")),
    ?assertEqual({ok, "bar/baz"}, epm_file_utils:path_from_ancestor("/foo/bar/baz", "/foo")),
    ?assertEqual({ok, "bar"}, epm_file_utils:path_from_ancestor("foo/bar", "foo")),
    ?assertEqual({ok, "bar"}, epm_file_utils:path_from_ancestor("foo/bar/", "foo/")),
    ?assertEqual({error, badparent}, epm_file_utils:path_from_ancestor("/foo/bar/baz", "/qux")),
    ?assertEqual({error, badparent}, epm_file_utils:path_from_ancestor("/foo/bar/baz", "/foo/bar/baz/qux")).
...
canonical_path(_Config) ->
    ...
    ?assertEqual(filename:nativename(Root), epm_file_utils:canonical_path("/")),
    ?assertEqual(filename:nativename(Root), epm_file_utils:canonical_path("/../../..")),
    ?assertEqual(Root ++ "foo", epm_file_utils:canonical_path("/foo/bar/..")),
    ?assertEqual(Root ++ "foo", epm_file_utils:canonical_path("/foo/../foo")),
    ?assertEqual(Root ++ "foo", epm_file_utils:canonical_path("/foo/.")),
    ?assertEqual(Root ++ "foo", epm_file_utils:canonical_path("/foo/./.")),
    ?assertEqual(filename:nativename(Root ++ "foo/bar"),
                 epm_file_utils:canonical_path("/foo/./bar")).
...
absolute_path(_Config) ->
    ...
    ?assertEqual(filename:absname(Root), epm_file_utils:absolute_path("/")),
    ?assertEqual(Root ++ "foo", epm_file_utils:absolute_path("/foo")),
    % casoscon ../ o ./
    {ok, Cwd} = file:get_cwd(),
    ?assertEqual(Cwd, epm_file_utils:absolute_path("./")),
    ?assertEqual(Cwd ++ "/foo", epm_file_utils:absolute_path("./foo")),
    ?assertEqual(Cwd ++ "/foo/bar", epm_file_utils:absolute_path("foo/bar")).
...
normalized_path(_Config) ->
    ...
    ?assertEqual(Root, epm_file_utils:normalized_path("/")),
    ?assertEqual("../..", epm_file_utils:normalized_path("/../../..")),
    ?assertEqual(Root ++ "foo", epm_file_utils:normalized_path("/foo/bar/..")),
    ?assertEqual(Root ++ "foo", epm_file_utils:normalized_path("/foo/../foo")),
    ?assertEqual(Root ++ "foo", epm_file_utils:normalized_path("/foo/.")),
    ?assertEqual(Root ++ "foo", epm_file_utils:normalized_path("/foo/./.")),
    ?assertEqual(Root ++ "foo/bar",
                 epm_file_utils:normalized_path("/foo/./bar")).

resolve_link(_Config) ->
    TmpDir = epm_file_utils:system_tmpdir(
            ["epm_file_utils_SUITE", "resolve_link"]),
    ...
    ?assertEqual(Target, epm_file_utils:resolve_link(Link)).

split_dirname(_Config) ->
    ?assertEqual({".", ""}, epm_file_utils:split_dirname("")),
    ?assertEqual({"/", ""}, epm_file_utils:split_dirname("/")),
    ?assertEqual({"/", "foo"}, epm_file_utils:split_dirname("/foo")),
    ?assertEqual({".", "foo"}, epm_file_utils:split_dirname("foo")),
    ?assertEqual({"/foo", "bar"}, epm_file_utils:split_dirname("/foo/bar")),
    ?assertEqual({"foo", "bar"}, epm_file_utils:split_dirname("foo/bar")).

mv_warning_is_ignored(_Config) ->
    meck:new(epm_utils, [passthrough]),
    meck:expect(epm_utils, sh, fun("mv ding dong", _) -> {ok, "Warning"}  end),
    ok = epm_file_utils:mv("ding", "dong"),
...
mv_dir(Config) ->
    ...
    ?assertEqual(ok, epm_file_utils:mv(SrcDir, DstDir1)),
    ...
    ?assertEqual(ok, epm_file_utils:mv(SrcDir, DstDir2)),
    ...
    ?assertEqual(ok, epm_file_utils:mv(SrcDir, DstDir3)),
...
mv_file_same(Config) ->
    ...
    ?assertEqual(ok, epm_file_utils:mv(F, Dst)),
...
mv_file_dir_same(Config) ->
    ...
    ?assertEqual(ok, epm_file_utils:mv(F, DstDir)),
    ?assert(filelib:is_file(Dst)),
    ?assertNot(filelib:is_file(F)),
    ok.
