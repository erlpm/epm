...
init_per_testcase({mixed, Case}, Config) ->
    ...
    | setup_project(Case, Config, epm_test_utils:expand_deps(mixed, Deps))];
init_per_testcase({DepsType, Case}, Config) ->
...
format_expected_mixed_warnings(Warnings) ->
    [case W of
        {N, Vsn} when hd(N) >= $a, hd(N) =< $z -> {pkg, epm_string:uppercase(N), Vsn};
        {N, Vsn} when hd(N) >= $A, hd(N) =< $Z -> {git, N, Vsn};
        N when hd(N) >= $a, hd(N) =< $z -> {pkg, epm_string:uppercase(N), "0.0.0"};
...
setup_project(nondefault_profile, Config0, Deps) ->
    ...
    epm_test_utils:create_app(AppDir, "A", "0.0.0", [kernel, stdlib]),
    TopDeps = epm_test_utils:top_level_deps(Deps),
    EpmConf = epm_test_utils:create_config(AppDir, [{profiles, [
                                                            {nondef, [{deps, TopDeps}]}
                                                       ]}]),
    {SrcDeps, PkgDeps} = epm_test_utils:flat_deps(Deps),
    mock_git_resource:mock([{deps, SrcDeps}]),
...
setup_project(nondefault_pick_highest, Config0, _) ->
    ...
    AppDir = ?config(apps, Config),
    epm_test_utils:create_app(AppDir, "A", "0.0.0", [kernel, stdlib]),
    DefaultDeps = epm_test_utils:expand_deps(DepsType, [{"B", [{"C", "1.0.0", []}]}]),
    ProfileDeps = epm_test_utils:expand_deps(DepsType, [{"C", "2.0.0", []}]),
    DefaultTop = epm_test_utils:top_level_deps(DefaultDeps),
    ProfileTop = epm_test_utils:top_level_deps(ProfileDeps),
    EpmConf = epm_test_utils:create_config(
            AppDir,
            [{deps, DefaultTop},
    ...
    case DepsType of
        git ->
            {SrcDeps, _} = epm_test_utils:flat_deps(DefaultDeps++ProfileDeps),
            mock_git_resource:mock([{deps, SrcDeps}]);
        pkg ->
            {_, PkgDeps} = epm_test_utils:flat_deps(DefaultDeps++ProfileDeps),
            mock_pkg_resource:mock([{pkgdeps, PkgDeps}])
    end,
    [{epmconfig, EpmConf} | Config];
...
fail_conflict(Config) ->
    ...
    epm_test_utils:run_and_check(
        Config, EpmConfig, ["lock"], ?config(expect, Config)
    ),
...
default_profile(Config) ->
    ...
    epm_test_utils:run_and_check(
        Config, EpmConfig, ["lock"], Expect
    ),
    epm_test_utils:run_and_check(
        Config, EpmConfig, ["as", "profile", "lock"], Expect
    ),
    check_warnings(error_calls(), ?config(warnings, Config), ?config(deps_type, Config)),
    ...
    epm_test_utils:run_and_check(
        Config, EpmConfig, ["as", "other", "lock"], Expect
    ),
    [?assertMatch({ok, #file_info{type=directory}}, % somehow symlinks return dirs
...
nondefault_profile(Config) ->
    ...
    try
        epm_test_utils:run_and_check(
            Config, EpmConfig, ["as", "nondef", "lock"], {ok, AppLocks}
        ),
    ...
    Expect = {ok, Apps},
    epm_test_utils:run_and_check(
        Config, EpmConfig, ["as", "nondef", "lock"], Expect
    ),
    check_warnings(error_calls(), ?config(warnings, Config), ?config(deps_type, Config)),
    BuildDir = filename:join([AppDir, "_build"]),
...
nondefault_pick_highest(Config) ->
    ...
    epm_test_utils:run_and_check(
        Config, EpmConfig, ["as", "nondef", "lock"],
...
error_calls() ->
    History = meck:history(epm_log),
    [{Str, Args} || {_, {epm_log, log, [error, Str, Args]}, _} <- History].
