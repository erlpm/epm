...
%% implementing the deprecated epm_resource instead of v2
%%
%% ```
%% {deps, [
%%     %% Application files are copied from "/path/to/app_name"
%%     {app_name, {local fs, "/path/to/app_name", undefined}}
%% ]}.
%% '''
-module(epm_localfs_resource).

-behaviour(epm_resource).
...
download(AppDir, {localfs, Path}, _State) ->
    ok = epm_file_utils:cp_r(filelib:wildcard(Path ++ "/*"), AppDir),
    {ok, undefined}.
