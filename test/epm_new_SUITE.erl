...
init_per_testcase(plugin_tpl, Config) ->
    ...
    Name = epm_test_utils:create_random_name("plugin_tpl"),
...
mock_home_dir(Path) ->
    ...
    meck:expect(epm_dir, template_dir, fun(_) -> Path end).
...
mock_empty_escript_templates() ->
    ...
    meck:expect(epm_utils, escript_foldl, fun(_,_,_) -> {ok, []} end).

app_git_user(Config) ->
    meck:expect(epm_utils, sh, fun("git config --global user.name", _) -> {ok, "gitname"};
...
app_with_fallbacks(Config) ->
    meck:expect(epm_utils, sh, fun(_, _) -> {error, fallback} end),
...
app_hg_user(Config) ->
    meck:expect(epm_utils, sh, fun("hg showconfig ui.username", _) -> {ok, "hgname <hg@email.com>"};
                                    (_, _) -> {error, fallback}
                                 end),
