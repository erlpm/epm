...
init_per_testcase(Case, Config) ->
    ...
    TopApp1 = epm_app_info:applications(TopApp0, ['rp_a', 'rp_b']),

    S0 = epm_state:new(),
    S1 = epm_state:all_deps(S0, [ADep, BDep, CDep, DDep, RelxDep]),
    S2 = epm_state:all_plugin_deps(S1, [APlug, RelxPlug]),
    S3 = epm_state:project_apps(S2, [TopApp1]),
    S4 = epm_state:code_paths(S3, default, code:get_path()),
    S5 = epm_state:code_paths(
        S4,
        all_deps,
        [epm_app_info:ebin_dir(A) || A <- [ADep, BDep, CDep, DDep, RelxDep]]
    ),
    S6 = epm_state:code_paths(
        S5,
        all_plugin_deps,
        [epm_app_info:ebin_dir(A) || A <- [APlug, RelxPlug, EPlug]]
    ),
    [{base_paths, BasePaths}, {root_dir, Dir}, {state, S6} | Config].

end_per_testcase(_, Config) ->
    %% this is deeply annoying because we interfere with epm own
    %% path handling!
    epm_paths:unset_paths([plugins, deps], ?config(state, Config)),
    Config.

fake_app(Name, Vsn, OutDir) ->
    {ok, App} = epm_app_info:new(Name, Vsn, OutDir),
...
compile_fake_appmod(App, Apps) ->
    OutDir = epm_app_info:ebin_dir(App),
...
clashing_apps(Config) ->
    Clashes = epm_paths:clashing_apps([deps, plugins],
                                        ?config(state, Config)),
...
set_paths(Config) ->
    ...
    epm_paths:set_paths([runtime], State),
...
misloaded_mods(_Config) ->
    Res = epm_paths:misloaded_modules(
      ["/1/2/3/4",
       "/1/2/4",
       "/2/1/1",
       "/3/4/5"],
