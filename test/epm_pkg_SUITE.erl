%% Test suite for the epm pkg index caching and decompression
%% mechanisms.
-module(epm_pkg_SUITE).
-compile(export_all).
...
good_uncached(Config) ->
    ...
    ?assertEqual(ok,
                 epm_pkg_resource:download(Tmp, {pkg, Pkg, Vsn, ?good_checksum, ?good_checksum, #{}}, State, #{}, true)),
...
badindexchk(Config) ->
    ...
    {Pkg,Vsn} = ?config(pkg, Config),
    State = ?config(state, Config),
    ?assertMatch({error, {epm_pkg_resource, {bad_registry_checksum, _, _, _, _}}},
                 epm_pkg_resource:download(Tmp, {pkg, Pkg, Vsn, ?bad_checksum, ?bad_checksum, #{}}, State, #{}, true)),
...
badpkg(Config) ->
    ...
    epm_pkg_resource:store_etag_in_cache(ETagPath, ?BADPKG_ETAG),
    ?assertMatch({error, {hex_tarball, {tarball, {inner_checksum_mismatch, _, _}}}},
                 epm_pkg_resource:download(Tmp, {pkg, Pkg, Vsn, ?badpkg_checksum, ?badpkg_checksum, #{}}, State, #{}, false)),
...
good_disconnect(Config) ->
    ...
    ?assert(filelib:is_regular(CachedFile)),
    {ok, Content} = file:read_file(CachedFile),
    epm_pkg_resource:store_etag_in_cache(ETagFile, ?BADPKG_ETAG),
    ?assertEqual(ok,
                 epm_pkg_resource:download(Tmp, {pkg, Pkg, Vsn, ?good_checksum, ?good_checksum, #{}}, State, #{}, true)),
    {ok, Content} = file:read_file(CachedFile).
...
pkgs_provider(Config) ->
    Config1 = epm_test_utils:init_epm_state(Config),
...
find_highest_matching(_Config) ->
    ...
    ?assertEqual(none, epm_packages:find_highest_matching_(<<"goodpkg">>, ec_semver:parse(<<"5.0">>),
                                                             #{name => <<"hexpm">>}, ?PACKAGE_TABLE, State)),


    {ok, Vsn3} = epm_packages:find_highest_matching_(<<"goodpkg">>, ec_semver:parse(<<"3.0.0-rc.0">>),
                                                       #{name => <<"hexpm">>}, ?PACKAGE_TABLE, State),
...
mock_config(Name, Config) ->
    ...
    Tid = ets:new(registry_table, [public]),
    AllDeps = [
        {{<<"badindexchk">>,<<"1.0.0">>}, [[], ?bad_checksum, ?bad_checksum, [<<"epm">>]]},
        {{<<"goodpkg">>,<<"1.0.0">>}, [[], ?good_checksum, ?good_checksum, [<<"epm">>]]},
        {{<<"goodpkg">>,<<"1.0.1">>}, [[], ?good_checksum, ?good_checksum, [<<"epm">>]]},
        {{<<"goodpkg">>,<<"1.1.1">>}, [[], ?good_checksum, ?good_checksum, [<<"epm">>]]},
        {{<<"goodpkg">>,<<"2.0.0">>}, [[], ?good_checksum, ?good_checksum, [<<"epm">>]]},
        {{<<"goodpkg">>,<<"3.0.0-rc.0">>}, [[], ?good_checksum, ?good_checksum, [<<"epm">>]]},
        {{<<"badpkg">>,<<"1.0.0">>}, [[], ?badpkg_checksum, ?badpkg_checksum, [<<"epm">>]]}
    ],
    ...
    meck:new(epm_state, [passthrough]),
    meck:expect(epm_state, get,
                fun(_State, epm_packages_cdn, _Default) ->
                        "http://test.com/";
                   (_, _, Default) ->
                        Default
                end),
    meck:expect(epm_state, resources,
                fun(_State) ->
                        DefaultConfig = erlpm_core:default_config(),
                        [epm_resource_v2:new(pkg, epm_pkg_resource,
    ...
    meck:expect(erlpm_repo, get_tarball, fun(_, _, _) when GoodCache ->
