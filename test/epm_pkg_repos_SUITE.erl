...
default_repo(_Config) ->
    ...
    MergedRepos = epm_hex_repos:repos([{repos, [Repo1]}]),

    ?assertMatch([#{name := <<"hexpm">>,
                    api_key := <<"asdf">>,
                    api_url := <<"https://erl.pm/api">>}], MergedRepos).
...
repo_merging(_Config) ->
    ...
    Repo2 = #{name => <<"repo-2">>,
              ...
              repo_verify => false},
    Result = epm_hex_repos:merge_repos([Repo1, Repo2,
                                             #{name => <<"repo-2">>,
                                               api_url => <<"repo-2/api">>,
                                               repo_url => <<"bad url">>,
                                               repo_verify => true},
...
repo_replacing(_Config) ->
    ...
    ?assertMatch([Repo1, Repo2, #{name := <<"hexpm">>}],
                 epm_hex_repos:repos([{repos, [Repo1]},
                                        {repos, [Repo2]}])),
    ...
    ?assertMatch([Repo1],
                 epm_hex_repos:repos([{repos, replace, [Repo1]},
                                        {repos, [Repo2]}])).
...
auth_merging(_Config) ->
    ...
    State = epm_state:new([{hex, [{repos, [Repo1, Repo2]}]}]),
    ...
    ?assertMatch({ok,
                 ...
                 epm_pkg_resource:init(pkg, State)),
...
auth_config_errors(_Config) ->
    ...
    ?assertThrow(epm_abort, epm_pkg_resource:init(pkg, State)),
    meck:expect(file, consult,
                fun(_) ->
                        {error, enoent}
                end),
...
auth_read_write_read(_Config) ->
    ...
    epm_hex_repos:update_auth_config(#{<<"foo">> => <<200>>}, State).
...
auth_remove_from_config(_Config) ->
    ...
    epm_hex_repos:remove_from_auth_config(<<"hexpm:repo-1">>, State).
...
use_first_repo_match(Config) ->
    ...
    ?assertMatch({ok,{package,{<<"B">>, {{2,0,0}, {[],[]}}, Repo2},
                  ...
                  #{name := Repo2,
                    http_adapter := {epm_httpc_adapter, #{profile := epm}}}},
                 epm_packages:resolve_version(<<"B">>, <<"> 1.4.0">>, undefined, undefined,
                                                ?PACKAGE_TABLE, State)),
...
use_exact_with_hash(Config) ->
    ...
    ?assertMatch({ok,{package,{<<"C">>, {{1,3,1}, {[],[]}}, Repo2},
                  ...
                  #{name := Repo2,
                    http_adapter := {epm_httpc_adapter, #{profile := epm}}}},
                 epm_packages:resolve_version(<<"C">>, <<"1.3.1">>, <<"inner checksum">>, <<"good outer checksum">>,
                                                ?PACKAGE_TABLE, State)).
...
ignore_match_in_excluded_repo(Config) ->
    ...
    ?assertMatch({ok,{package,{<<"A">>, {{0,1,1}, {[],[]}}, Repo2},
                  ...
                  #{name := Repo2,
                    http_adapter := {epm_httpc_adapter, #{profile := epm}}}},
                 epm_packages:resolve_version(<<"A">>, <<"0.1.1">>, <<"inner checksum">>, <<"good outer checksum">>,
                                                ?PACKAGE_TABLE, State)).
...
optional_prereleases(Config) ->
    ...
    ?assertMatch({ok,{package,{<<"B">>, {{1,5,0}, {[],[]}}, Hexpm},
                  ...
                  #{name := Hexpm,
                    http_adapter := {epm_httpc_adapter, #{profile := epm}}}},
                 epm_packages:resolve_version(<<"B">>, <<"~> 1.5.0">>, undefined, undefined,
                                                ?PACKAGE_TABLE, State)),
    ...
    ?assertMatch({ok,{package,{<<"B">>, {{1,5,6}, {[<<"rc">>,0],[]}}, Hexpm},
                  ...
                  #{name := Hexpm,
                    http_adapter := {epm_httpc_adapter, #{profile := epm}}}},
                 epm_packages:resolve_version(<<"B">>, <<"1.5.6-rc.0">>, <<"inner checksum">>, <<"outer checksum">>,
                                                ?PACKAGE_TABLE, State)),

    %% allow prerelease through configuration
    State1 = epm_state:set(State, deps_allow_prerelease, true),
    ?assertMatch({ok,{package,{<<"B">>, {{1,5,6}, {[<<"rc">>,0],[]}}, Hexpm},
                      <<"inner checksum">>,<<"outer checksum">>, true, []},
                  #{name := Hexpm,
                    http_adapter := {epm_httpc_adapter, #{profile := epm}}}},
                 epm_packages:resolve_version(<<"B">>, <<"~> 1.5.0">>, <<"inner checksum">>, <<"outer checksum">>,
...
setup_deps_and_repos(Deps, Repos) ->
    ...
    true = epm_packages:new_package_table(),
    insert_deps(Deps),
    State = epm_state:new([{hex, [{repos, [#{name => R} || R <- Repos]}]}]),
    epm_state:create_resources([{pkg, epm_pkg_resource}], State).


insert_deps(Deps) ->
    lists:foreach(fun({Name, Version, Repo, Retired}) ->
                          ets:insert(?PACKAGE_TABLE, #package{key={epm_utils:to_binary(Name),
                                                                   ec_semver:parse(Version),
                                                                   epm_utils:to_binary(Repo)},
                                                              dependencies=[],
                                                              retired=Retired,
