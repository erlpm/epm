...
init_per_testcase(_, Config) ->
    epm_test_utils:init_epm_state(Config, "profiles_").
...
profile_new_key(Config) ->
    ...
    AllDeps = epm_test_utils:expand_deps(git, [{"a", "1.0.0", []}
                                                ,{"b", "1.0.0", []}]),
    {SrcDeps, []} = epm_test_utils:flat_deps(AllDeps),
    mock_git_resource:mock([{deps, SrcDeps}]),

    Name = epm_test_utils:create_random_name("profile_new_key_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),

    Deps = epm_test_utils:top_level_deps(
             epm_test_utils:expand_deps(git, [{"a", "1.0.0", []}
                                               ,{"b", "1.0.0", []}])),
    ct:pal("Deps ~p", [Deps]),
    EpmConfig = [{profiles,
                   [{ct,
...
profile_merge_keys(Config) ->
    ...
    Deps = epm_test_utils:top_level_deps(
             epm_test_utils:expand_deps(git, [{"a", "1.0.0", []}
                                               ,{"b", "1.0.0", []}])),
    ProfileDeps = epm_test_utils:top_level_deps(
                    epm_test_utils:expand_deps(git, [{"b", "2.0.0", []}])),
...
profile_merge_umbrella_keys(Config) ->
    ...
    Name = epm_test_utils:create_random_name("profile_merge_umbrella_keys"),
    Vsn = epm_test_utils:create_random_vsn(),
    SubAppDir = filename:join([AppDir, "apps", Name]),

    EpmConfig = [{vals, [{a,1},{b,1}]},
                   {profiles,
                    [{ct,
                      [{vals, [{a,1},{b,2}]}]}]}],
    ...
    epm_test_utils:create_app(SubAppDir, Name, Vsn, [kernel, stdlib]),
    epm_test_utils:create_config(SubAppDir, SubEpmConfig),
    {ok, EpmConfigRead} = file:consult(epm_test_utils:create_config(AppDir, EpmConfig)),

    {ok, State} = epm_test_utils:run_and_check(
        Config, EpmConfigRead, ["as", "ct", "compile"], return
    ),

    [ProjectApp] = epm_state:project_apps(State),
    ?assertEqual(Name, binary_to_list(epm_app_info:name(ProjectApp))),
    Opts = epm_app_info:opts(ProjectApp),
...
explicit_profile_deduplicate_deps(Config) ->
    ...
    Name = epm_test_utils:create_random_name("explicit_profile_deduplicate_deps_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),

    FooDeps = epm_test_utils:top_level_deps(
                    epm_test_utils:expand_deps(git, [{"a", "1.0.0", []},
                                                       {"b", "2.0.0", []}])),
    BarDeps = epm_test_utils:top_level_deps(
...
implicit_profile_deduplicate_deps(Config) ->
    ...
    Name = epm_test_utils:create_random_name("implicit_profile_deduplicate_deps_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),

    TestDeps = epm_test_utils:top_level_deps(
                    epm_test_utils:expand_deps(git, [{"a", "1.0.0", []},
                                                       {"b", "2.0.0", []}])),
    ProfileDeps = epm_test_utils:top_level_deps(
                    epm_test_utils:expand_deps(git, [{"b", "1.0.0", []}])),
...
all_deps_code_paths(Config) ->
    ...
    Name = epm_test_utils:create_random_name("all_deps_code_paths"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),
    ...
    EpmConfig = [{deps, Deps},
                   {profiles,
                    [{all_deps_test,
                      [{deps, ProfileDeps}]}]}],
    os:putenv("EPM_PROFILE", "all_deps_test"),
    {ok, State} = epm_test_utils:run_and_check(Config, EpmConfig,
                                   ["compile"], {ok, [{app, Name}
                                                     ,{dep, "a", "1.0.0"}
                                                     ,{dep, "b", "2.0.0"}]}),
    os:putenv("EPM_PROFILE", ""),

    Paths = epm_state:code_paths(State, all_deps),
...
profile_merges(_Config) ->
    ...
    State = epm_state:new(EpmConfig),
    State1 = epm_state:apply_profiles(State, [profile1, profile2]),

    %% Combine lists
    ?assertEqual(lists:sort([key1, key2, {key1, 1, 2}, {key3, 5}]),
                 lists:sort(epm_state:get(State1, test1))),

    %% Use new value for strings
    "goodbye" = epm_state:get(State1, test2),

    %% Check that a newvalue of []/"" doesn't override non-string oldvalues
    [key3] = epm_state:get(State1, test3),
    [] = epm_state:get(State1, test4),
    [{key5, false}, {key5, true}] = epm_state:get(State1, test5),
    [{key6, true}, {key6, false}] = epm_state:get(State1, test6).
...
same_profile_deduplication(_Config) ->
    ...
    State = epm_state:new(EpmConfig),
    State1 = epm_state:apply_profiles(State, [profile1, profile1, profile1]),

    ?assertEqual([default, profile1], epm_state:current_profiles(State1)),
    Test1 = epm_state:get(State1, test1),
    ...
    %% Check that a newvalue of []/"" doesn't override non-string oldvalues
    ?assertEqual([key3], epm_state:get(State1, test3)),
    ?assertEqual([bar, foo], epm_state:get(State1, test2)).
...
stack_deduplication(_Config) ->
    ...
    State = epm_state:new(EpmConfig),
    State1 = epm_state:apply_profiles(State, [a, b, c, d, e, a, e, b]),
    ?assertEqual(b, epm_state:get(State1, test_key)),

    TestList = epm_state:get(State1, test_list),
    ?assertEqual(
...
add_to_profile(_Config) ->
    EpmConfig = [{foo, true}, {bar, false}],
    State = epm_state:new(EpmConfig),
    State1 = epm_state:add_to_profile(State, test, [{foo, false}]),
    State2 = epm_state:apply_profiles(State1, test),

    Opts = epm_state:opts(State2),
    lists:map(fun(K) -> false = dict:fetch(K, Opts) end, [foo, bar]).

add_to_existing_profile(_Config) ->
    EpmConfig = [{foo, true}, {bar, false}, {profiles, [
        {test, [{foo, false}]}
    ]}],
    State = epm_state:new(EpmConfig),
    State1 = epm_state:add_to_profile(State, test, [{baz, false}]),
    State2 = epm_state:apply_profiles(State1, test),
...
profiles_remain_applied_with_config_present(Config) ->
    ...
    Name = epm_test_utils:create_random_name("profiles_remain_applied_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),

    EpmConfig = [{erl_opts, []}, {profiles, [
...
deduplicated_paths(Config) ->
    ...
    Name = epm_test_utils:create_random_name("deduplicated_paths_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),
...
test_profile_applied_at_completion(Config) ->
    ...
    Name = epm_test_utils:create_random_name("test_profile_at_completion_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_eunit_app(AppDir, Name, Vsn, [kernel, stdlib]),

    EpmConfig = [{erl_opts, [{d, some_define}]}],
    epm_test_utils:create_config(AppDir, EpmConfig),

    {ok, State} = epm_test_utils:run_and_check(Config,
                                                 EpmConfig,
                                                 ["eunit"],
                                                 return),

    [App] = epm_state:project_apps(State),
    ErlOpts = epm_app_info:get(App, erl_opts),
    true = lists:member({d, 'TEST'}, ErlOpts).

test_profile_applied_before_compile(Config) ->
    AppDir = ?config(apps, Config),

    Name = epm_test_utils:create_random_name("test_profile_before_compile_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_eunit_app(AppDir, Name, Vsn, [kernel, stdlib]),
    ...
    {ok, State} = epm_test_utils:run_and_check(Config, EpmConfig, ["eunit"], {ok, [{app, Name}]}),
...
test_profile_applied_before_eunit(Config) ->
    ...
    Name = epm_test_utils:create_random_name("test_profile_before_eunit_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_eunit_app(AppDir, Name, Vsn, [kernel, stdlib]),
...
test_profile_applied_to_apps(Config) ->
    ...
    Name = epm_test_utils:create_random_name("test_profile_applied_to_apps_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_eunit_app(AppDir, Name, Vsn, [kernel, stdlib]),
...
test_erl_opts_debug_info(_Config) ->
    ToOpts = fun(List) -> epm_opts:erl_opts(dict:from_list([{erl_opts, List}])) end,
...
test_profile_erl_opts_precedence(Config) ->
    ...
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),
    EpmConfig = [{erl_opts, [no_debug_info]},
                   {profiles, [
                     {test, [{erl_opts, [debug_info, {d,'HI'}]}]},
                     {other, [{erl_opts, [debug_info, {d,'HI'}]}]}
                   ]}],
    {ok, State1} = epm_test_utils:run_and_check(
      Config, EpmConfig, ["as", "test", "compile"], return
    ),
    {ok, State2} = epm_test_utils:run_and_check(
      Config, EpmConfig, ["as", "other", "compile"], return
    ),
    {ok, State3} = epm_test_utils:run_and_check(
      Config, EpmConfig, ["compile"], return
    ),
    Opts1 = epm_state:opts(State1),
    Opts2 = epm_state:opts(State2),
    Opts3 = epm_state:opts(State3),
    ErlOpts1 = epm_opts:erl_opts(Opts1),
    ErlOpts2 = epm_opts:erl_opts(Opts2),
    ErlOpts3 = epm_opts:erl_opts(Opts3),
...
first_files_exception(_Config) ->
    EpmConfig = [{erl_first_files, ["c","a","b"]},
    ...
    State = epm_state:new(EpmConfig),
    State1 = epm_state:apply_profiles(State, [profile2]),

    %% Combine lists
    ?assertEqual(["a","b","c","e"], epm_state:get(State1, other)),
    %% there is no specific reason not to dedupe "a" here aside from "this is how it is"
    ?assertEqual(["c","a","b","a","e"], epm_state:get(State1, erl_first_files)),
    ?assertEqual(["c","a","b","a","e"], epm_state:get(State1, mib_first_files)),
    ok.
...
get_compiled_profile_erl_opts(Profiles, Config) ->
    ...
    Name = epm_test_utils:create_random_name(
    ...
    Command = case Profiles of
        ...
        [default] ->
            ["compile"];
        _ ->
            ["as", epm_string:join(PStrs, ","), "compile"]
    end,
    {ok, State} = epm_test_utils:run_and_check(
        Config, EpmConfig, Command, {ok, [{app, Name}]}),
    code:add_paths(epm_state:code_paths(State, all_deps)),
