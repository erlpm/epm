...
init_per_group(all, Config) ->
    State = epm_state:resources(epm_state:new(), [{git, epm_git_resource},
...
init_per_testcase(change_type_upgrade, Config) ->
    ...
    {ok, AppInfo} = epm_app_info:new(test_app, "0.0.1", DirName),
    AppInfo1 = epm_app_info:source(AppInfo, ?config(resource, Config)),
...
change_type_upgrade(Config) ->
    ?assert(epm_fetch:needs_update(?config(app, Config),
                                     ?config(state, Config))).
