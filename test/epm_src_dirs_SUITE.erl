...
src_dirs_at_root(Config) ->
    ...
    EpmConfig = [{src_dirs, ["foo", "bar", "baz"]}],
...
extra_src_dirs_at_root(Config) ->
    ...
    EpmConfig = [{extra_src_dirs, ["foo", "bar", "baz"]}],
...
src_dirs_at_root_and_in_erl_opts(Config) ->
    ...
    Name = epm_test_utils:create_random_name("src_dirs_root_erlopts_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),

    EpmConfig = [{erl_opts, [{src_dirs, ["foo", "bar"]}]}, {src_dirs, ["baz", "qux"]}],

    %% move the .app.src file to one of the subdirs, out of src/
    filelib:ensure_dir(filename:join([AppDir, "qux", "fake"])),
    epm_file_utils:mv(filename:join([AppDir, "src", Name ++ ".app.src"]),
...
dupe_src_dirs_at_root_and_in_erl_opts(Config) ->
    ...
    Name = epm_test_utils:create_random_name("dupe_src_dirs_root_erlopts_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, Name, Vsn, [kernel, stdlib]),
    ...
    {error, {epm_prv_app_discovery, {multiple_app_files, [Src2, Src1]}}} =
      epm_test_utils:run_and_check(Config, EpmConfig, ["compile"], return),
...
extra_src_dirs_at_root_and_in_erl_opts(Config) ->
    ...
    EpmConfig = [{erl_opts, [{extra_src_dirs, ["foo", "bar"]}]}, {extra_src_dirs, ["baz", "qux"]}],

    {ok, State} = epm_test_utils:run_and_check(Config, EpmConfig, ["compile"], return),

    ["bar", "baz", "foo", "qux"] = epm_dir:extra_src_dirs(epm_state:opts(State), []).
...
src_dir_takes_precedence_over_extra(Config) ->
    ...
    EpmConfig = [{erl_opts, [{src_dirs, ["src", "extra"]}, {extra_src_dirs, ["extra"]}]}],
...
src_dir_checkout_dep(Config) ->
    AppDir = ?config(apps, Config),
    AppName = epm_test_utils:create_random_name("src_dir_checkout_app"),
    DepName = epm_test_utils:create_random_name("src_dir_checkout_dep"),
    AtomDep = list_to_atom(DepName),

    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_app(AppDir, AppName, Vsn, [kernel, stdlib]),
    EpmConfig = [{deps, [AtomDep]}],

    DepDir = filename:join([?config(checkouts, Config), DepName]),
    ct:pal("checkouts dir: ~p", [DepDir]),
    epm_test_utils:create_app(DepDir, DepName, Vsn, [kernel, stdlib]),


    %% move the .app.src file to one of the subdirs, out of src/
    epm_file_utils:mv(filename:join([DepDir, "src"]),
                        filename:join([DepDir, "qux"])),
    DepEpmConfig = [{erl_opts, [{src_dirs, ["foo", "bar"]}]},
                      {src_dirs, ["baz", "qux"]}],
    file:write_file(filename:join([DepDir, "epm.config"]),
                    io_lib:format("~p.~n~p.~n", DepEpmConfig)),
...
app_src_info(Config) ->
    ...
    AppName1 = epm_test_utils:create_random_name("app_src_info"),
    AppDir1 = filename:join(PrivDir, AppName1),
    {ok, Info1} = epm_app_info:new(AppName1, "1.0.0", AppDir1),
    AppSrc1 = filename:join([AppDir1, "src", AppName1 ++ ".app.src"]),
    ok = filelib:ensure_dir(AppSrc1),
    ok = file:write_file(AppSrc1, "[]."),
    ?assertEqual(AppSrc1, epm_app_info:app_file_src(Info1)),

    AppName2 = epm_test_utils:create_random_name("app_src_info"),
    AppDir2 = filename:join(PrivDir, AppName2),
    {ok, Info2Tmp} = epm_app_info:new(AppName2, "1.0.0", AppDir2),
    Info2 = epm_app_info:set(Info2Tmp, src_dirs, ["foo", "bar", "baz"]),
    AppSrc2 = filename:join([AppDir2, "bar", AppName2 ++ ".app.src"]),
    ok = filelib:ensure_dir(AppSrc2),
    ok = file:write_file(AppSrc2, "[]."),
    ?assertEqual(AppSrc2, epm_app_info:app_file_src(Info2)),
    ok.
