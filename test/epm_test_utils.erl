-module(epm_test_utils).
-include_lib("common_test/include/ct.hrl").
-include_lib("eunit/include/eunit.hrl").
-export([init_epm_state/1, init_epm_state/2, run_and_check/3, run_and_check/4,
         check_results/3, check_results/4
    ,expand_deps/2, flat_deps/1, top_level_deps/1
    ,create_app/4, create_plugin/4, create_eunit_app/4, create_empty_app/4,
         create_config/2, create_config/3, package_app/4
    ,create_random_name/1, create_random_vsn/0, write_src_file/2,
         random_element/1]).
...
%% @doc {@see init_epm_state/2}
init_epm_state(Config) -> init_epm_state(Config, "apps_dir1_").

%% @doc Takes a common test config and a name (string) and sets up
%% a basic OTP app directory with a pre-configured epm state to
%% run tests with.
init_epm_state(Config, Name) ->
    ...
    Verbosity = epm:log_level(),
    epm_log:init(command_line, Verbosity),
    GlobalDir = filename:join([DataDir, "cache"]),
    Repos = proplists:get_value(repos, Config, []),
    State = epm_state:new([{base_dir, filename:join([AppsDir, "_build"])}
                            ,{global_epm_dir, GlobalDir}
                            ,{hex, [{repos, [#{name => R} || R <- Repos]}]}
                            ,{root_dir, AppsDir}]),
    [{apps, AppsDir}, {checkouts, CheckoutsDir}, {state, State} | Config].

%% @doc Takes common test config, a epm config ([] if empty), a command to
...
%%   dependency.
%%
%% This function assumes `init_epm_state/1-2' has run before, in order to
%% fetch the `apps' and `state' values from the CT config.
run_and_check(Config, EpmConfig, Command, Expect) ->
    %% Assumes init_epm_state has run first
    AppDir = ?config(apps, Config),
    State = ?config(state, Config),
    try
        Res = epm:run(epm_state:new(State, EpmConfig, AppDir), Command),
        case Expect of
            {error, Reason} ->
        ...
        epm_abort when Expect =:= epm_abort -> epm_abort
    end.
...
%% - src/<file>.erl
%% - src/<file>.app.src
%% And returns a `epm_app_info' object.
create_app(AppDir, Name, Vsn, Deps) ->
    ...
    epm_app_info:new(Name, Vsn, AppDir, Deps).
...
%% @doc Creates a epm.config file. The function accepts a list of terms,
...
create_config(AppDir, Contents) ->
    ConfFilename = filename:join([AppDir, "epm.config"]),
...
expand_deps(mixed, [{Name, Deps} | Rest]) ->
    Dep = if hd(Name) >= $a, hd(Name) =< $z ->
            {pkg, epm_string:uppercase(Name), "0.0.0", undefined, undefined}
           ; hd(Name) >= $A, hd(Name) =< $Z ->
...
expand_deps(mixed, [{Name, Vsn, Deps} | Rest]) ->
    Dep = if hd(Name) >= $a, hd(Name) =< $z ->
            {pkg, epm_string:uppercase(Name), Vsn, undefined, undefined}
           ; hd(Name) >= $A, hd(Name) =< $Z ->
...
check_results(AppDir, Expected, ProfileRun, State) ->
    ...
    Locks = lists:flatten(epm_config:consult_lock_file(LockFile)),

    InvalidApps = epm_app_discover:find_apps(BuildDirs, invalid, State),
    ValidApps = epm_app_discover:find_apps(BuildDirs, valid, State),

    InvalidDepsNames = [{ec_cnv:to_list(epm_app_info:name(App)), App} || App <- InvalidApps],
    ValidDepsNames = [{ec_cnv:to_list(epm_app_info:name(App)), App} || App <- ValidApps],

    Deps = epm_app_discover:find_apps(BuildDirs, all, State),
    SubDeps = epm_app_discover:find_apps(BuildSubDirs, all, State),
    DepsNames = [{ec_cnv:to_list(epm_app_info:name(App)), App} || App <- Deps],
    SubDirDepsNames = [{ec_cnv:to_list(epm_app_info:name(App)), App} || App <- SubDeps],
    Checkouts = epm_app_discover:find_apps(CheckoutsDirs, all, State),
    CheckoutsNames = [{ec_cnv:to_list(epm_app_info:name(App)), App} || App <- Checkouts],
    Plugins = epm_app_discover:find_apps(PluginDirs, all, State),
    PluginsNames = [{ec_cnv:to_list(epm_app_info:name(App)), App} || App <- Plugins],
    GlobalPlugins = epm_app_discover:find_apps(GlobalPluginDirs, all, State),
    GlobalPluginsNames = [{ec_cnv:to_list(epm_app_info:name(App)), App} || App <- GlobalPlugins],
    ...
    lists:foreach(
                ...
                case lists:keyfind(Name, 1, DepsNames) of
                    ...
                    {Name, App} ->
                        ?assertEqual(iolist_to_binary(Vsn),
                                     iolist_to_binary(epm_app_info:original_vsn(App)))
                end
        ;  ({subdir_dep, Name}) ->
        ...
        ;  ({release, Name, Vsn, ExpectedDevMode}) ->
                ...
                try
                    ...
                    {ok, RelLibs} = epm_utils:list_dir(LibDir),
                    IsSymLinkFun =
                        fun(X) ->
...
package_app(AppDir, DestDir, PkgName, PkgVsn) ->
    ...
    {ok, Fs} = epm_utils:list_dir(AppSrc),
    ...
    {ok, #{tarball := Tarball, outer_checksum := <<Checksum:256/big-unsigned-integer>>}} = erlpm_tarball:create(Metadata, Files),
