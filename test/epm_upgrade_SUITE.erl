...
init_per_testcase(novsn_pkg, Config0) ->
    Config = epm_test_utils:init_epm_state(Config0, "novsn_pkg_"),
    AppDir = ?config(apps, Config),
    EpmConf = epm_test_utils:create_config(AppDir, [{deps, [fakeapp]}]),
...
init_per_testcase(upgrade_no_args, Config0) ->
    epm_test_utils:init_epm_state(Config0, "upgrade_no_args_");
init_per_testcase(Case, Config) ->
    DepsType = ?config(deps_type, Config),
    {Deps, UpDeps, ToUp, Expectations} = upgrades(Case),
    Expanded = epm_test_utils:expand_deps(DepsType, Deps),
    UpExpanded = epm_test_utils:expand_deps(DepsType, UpDeps),
...
setup_project(Case=umbrella_config, Config0, Deps, UpDeps) ->
    ...
    Config = epm_test_utils:init_epm_state(Config0, NameRoot++"_"),
    AppDir = filename:join([?config(apps, Config), "apps", NameRoot]),
    epm_test_utils:create_app(AppDir, "Root", "0.0.0", [kernel, stdlib]),
    TopDeps = epm_test_utils:top_level_deps(Deps),
    TopConf = epm_test_utils:create_config(AppDir, [{deps, []}]),
    EpmConf = epm_test_utils:create_config(AppDir, [{deps, TopDeps}]),
    [{epmconfig, TopConf},
     {epmumbrella, EpmConf},
     {next_top_deps, epm_test_utils:top_level_deps(UpDeps)} | Config];
setup_project(Case, Config0, Deps, UpDeps) when Case == profiles;
    ...
    AppDir = filename:join([?config(apps, Config), "apps", NameRoot]),
    epm_test_utils:create_app(AppDir, "Root", "0.0.0", [kernel, stdlib]),
    [Top|ProfileDeps] = epm_test_utils:top_level_deps(Deps),
    EpmConf = epm_test_utils:create_config(AppDir, [
        {deps, [Top]},
        {profiles, [{fake, [{deps, ProfileDeps}]}]}
    ]),
    [NextTop|NextPDeps] = epm_test_utils:top_level_deps(UpDeps),
...
upgrades(top_b) ->
    ...
    {[{"A", "1.0.0", [{"B", [{"D", "1.0.0", []}]},
     ...
     ["A","B","D"],
     %% upgrade vs. new tree
     {"B", {error, {epm_prv_upgrade, {transitive_dependency, <<"B">>}}}}};
upgrades(top_c) ->
     %% Original tree
    {[{"A", "1.0.0", [{"B", [{"D", "1.0.0", []}]},
                  {"C", [{"D", "2.0.0", []}]}]}
     ...
     ["A","B","D"],
     %% upgrade vs. new tree
     {"C", {error, {epm_prv_upgrade, {transitive_dependency, <<"C">>}}}}};
upgrades(top_d1) ->
     %% Original tree
    {[{"A", "1.0.0", [{"B", [{"D", "1.0.0", []}]},
                  {"C", [{"D", "2.0.0", []}]}]}
     ...
     ["A","B","D"],
     %% upgrade vs. new tree
     {"D", {error, {epm_prv_upgrade, {transitive_dependency, <<"D">>}}}}};
upgrades(top_d2) ->
     %% Original tree
    {[{"A", "1.0.0", [{"B", [{"D", "1.0.0", []}]},
                  {"C", [{"D", "2.0.0", []}]}]}
...
upgrades(top_e) ->
    ...
    {[{"A", "1.0.0", [{"B", [{"D", "1.0.0", []}]},
     ...
     ["A","B","D"],
     %% upgrade vs. new tree
     {"E", {error, {epm_prv_upgrade, {unknown_dependency, <<"E">>}}}}};
upgrades(pair_a) ->
    {[{"A", "1.0.0", [{"C", "1.0.0", []}]},
      {"B", "1.0.0", [{"D", "1.0.0", []}]}
     ],
...
mock_deps(git, OldDeps, Deps, Upgrades) ->
    ...
    {SrcDeps, _} = epm_test_utils:flat_deps(Deps++OldDeps),
    mock_git_resource:mock([{deps, SrcDeps}, {upgrade, Upgrades}]);
mock_deps(pkg, OldDeps, Deps, Upgrades) ->
    catch mock_pkg_resource:unmock(),
    {_, PkgDeps} = epm_test_utils:flat_deps(Deps++OldDeps),
    mock_pkg_resource:mock([{pkgdeps, PkgDeps}, {upgrade, Upgrades}]).
...
delete_d(Config) ->
    ...
    run(Config),
    Infos = [{Str, Args}
            || {_, {epm_log, log, [info, Str, Args]}, _} <- meck:history(epm_log)],
    meck:unload(epm_log),
    ?assertNotEqual([],
...
stable_lock(Config) ->
    ...
    epm_test_utils:run_and_check(Config, EpmConfig, ["lock"], {ok, []}),
    ...
    NewEpmConf = epm_test_utils:create_config(?config(apps, Config),
                                                  [{deps, ?config(next_top_deps, Config)}]),
    {ok, NewEpmConfig} = file:consult(NewEpmConf),
    epm_test_utils:run_and_check(
        Config, NewEpmConfig, ["lock", App], Expectation
    ).
...
compile_upgrade_parity(Config) ->
    ...
    epm_test_utils:run_and_check(Config, EpmConfig, ["compile"], {ok, []}),
    {ok, CompileLockData1} = file:read_file(Lockfile),
    epm_test_utils:run_and_check(Config, EpmConfig, ["upgrade", "--all"], {ok, []}),
...
umbrella_config(Config) ->
    apply(?config(mock, Config), []),
    {ok, TopConfig} = file:consult(?config(epmconfig, Config)),
    %% Install dependencies before re-mocking for an upgrade
    epm_test_utils:run_and_check(Config, TopConfig, ["lock"], {ok, []}),
    {App, Unlocks} = ?config(expected, Config),
    ct:pal("Upgrades: ~p -> ~p", [App, Unlocks]),
    Expectation = case Unlocks of
        {error, Term} -> {error, Term};
    ...
    meck:new(epm_prv_upgrade, [passthrough]),
    meck:expect(epm_prv_upgrade, do, fun(S) ->
                                               apply(?config(mock_update, Config), []),
                                               meck:passthrough([S])
                                       end),
    _NewEpmConf = epm_test_utils:create_config(filename:dirname(?config(epmumbrella, Config)),
                                                   [{deps, ?config(next_top_deps, Config)}]),
...
profiles(Config) ->
    ...
    epm_test_utils:run_and_check(Config, TopConfig, ["as","fake","lock"], {ok, []}),
    {App, Unlocks} = ?config(expected, Config),
    ct:pal("Upgrades: ~p -> ~p", [App, Unlocks]),
    Expectation = case Unlocks of
        {error, Term} -> {error, Term};
    ...
    meck:new(epm_prv_app_discovery, [passthrough]),
    meck:expect(epm_prv_app_discovery, do, fun(S) ->
                                               apply(?config(mock_update, Config), []),
                                               meck:passthrough([S])
                                       end),
    NewEpmConf = epm_test_utils:create_config(?config(apps, Config),
    ...
    {ok, NewEpmConfig} = file:consult(NewEpmConf),
    epm_test_utils:run_and_check(
        Config, NewEpmConfig, ["as","fake","upgrade", App], Expectation
     ),
    meck:unload(epm_prv_app_discovery).

profiles_exclusion(Config) -> profiles(Config).

tree_migration(Config) ->
    apply(?config(mock, Config), []),
    ConfigPath = ?config(epmconfig, Config),
    {ok, EpmConfig} = file:consult(ConfigPath),
    %% Install dependencies before re-mocking for an upgrade
    epm_test_utils:run_and_check(Config, EpmConfig, ["lock"], {ok, []}),
    ...
    NewEpmConf = epm_test_utils:create_config(filename:dirname(ConfigPath),
                                                  [{deps, ?config(next_top_deps, Config)}]),
    {ok, NewEpmConfig} = file:consult(NewEpmConf),
    {ok, NewState} = epm_test_utils:run_and_check(
        Config, NewEpmConfig, ["upgrade", App], return
    ),
    meck:unload(epm_prv_upgrade),
    %% Check that the internal state properly has E with a lock-level
    %% of 1.
    Locks = epm_state:lock(NewState),
    [Locked] = [X || X <- Locks, epm_app_info:name(X) =:= <<"E">>],
    ?assertEqual(1, epm_app_info:dep_level(Locked)),
...
run(Config) ->
    ...
    {ok, NewEpmConfig} = file:consult(NewEpmConf),
    epm_test_utils:run_and_check(
        Config, NewEpmConfig, ["upgrade", App], Expectation
     ),
    meck:unload(epm_prv_upgrade).
...
novsn_pkg(Config) ->
    ...
    apply(?config(mock_update, Config), []),
    epm_test_utils:run_and_check(
        Config, EpmConfig, ["upgrade", "--all"], Expectation
    ),
    ok.
...
upgrade_no_args(Config) ->
    try epm_test_utils:run_and_check(Config, [], ["upgrade"], return)
    catch {error, {epm_prv_upgrade, no_arg}} ->
        ok
    end,
    ok.
