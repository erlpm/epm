...
init_per_testcase(xref_dep_hook, Config) ->
    ...
    ok = epm_file_utils:cp_r([Src], Dst),
    GlobalDir = filename:join([?config(priv_dir, Config), "cache"]),
    State = epm_state:new([{base_dir, filename:join([Dst, "_build"])}
                            ,{global_epm_dir, GlobalDir}
                            ,{root_dir, Dst}]),
    [{apps, Dst}, {state, State} | Config];
init_per_testcase(Case, Config) ->
    UpdConfig = epm_test_utils:init_epm_state(Config),
    AppDir = ?config(apps, UpdConfig),
    file:set_cwd(AppDir),
    Name = epm_test_utils:create_random_name("xrefapp_"),
    Vsn = epm_test_utils:create_random_vsn(),
    epm_test_utils:create_empty_app(AppDir, Name, Vsn, [kernel, stdlib]),
...
xref_test(Config) ->
    ...
    Result = epm:run(epm_state:new(State, EpmConfig, AppDir), ["xref"]),
...
xref_dep_hook(Config) ->
    epm_test_utils:run_and_check(Config, [], ["compile"], {ok, []}).
